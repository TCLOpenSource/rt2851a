...
static int rtk_snd_ai_hw_close(struct snd_pcm_substream *substream)
{
	...
	if (do_uninit) {
		...
		/* initial ring buffer NULL */
        ret = RPC_TOAGENT_INITRINGBUFFER_HEADER_SVC(dpcm->card->card, &buf);
        pr_info("[ALSA] AI init ring success\n");
    }
...
static void rtk_snd_pcm_freerun_timer_function(unsigned long data)
{
	...
	if (dpcm->freerun == 0) {
		...
		if ((dpcm->audiopath == AUDIO_PATH_DECODER_AO) || (dpcm->audiopath == AUDIO_PATH_AO)) {
			if (wp_next > wp || wp_next == 0) {
				offset = (size_t) (dpcm->ring_bak[0].writePtr - dpcm->ring_bak[0].beginAddr);
				map_size = (size_t) (adv_min * dpcm->output_frame_bytes);

				dma_addr_t phy_addr = dma_map_page(dev, phys_to_page(runtime->dma_addr), offset, map_size, DMA_TO_DEVICE);
				dma_unmap_page(dev, phy_addr, map_size, DMA_TO_DEVICE);
			} else {
				offset = (size_t) (dpcm->ring_bak[0].writePtr - dpcm->ring_bak[0].beginAddr);
				map_size = (size_t) ((runtime->buffer_size - wp) * dpcm->output_frame_bytes);
				...
				if (wp_next) {
					offset = 0;
					map_size = (size_t) (wp_next * dpcm->output_frame_bytes);

					dma_addr_t phy_addr1 = dma_map_page(dev, phys_to_page(runtime->dma_addr), offset, map_size, DMA_TO_DEVICE);
					dma_unmap_page(dev, phy_addr1, map_size, DMA_TO_DEVICE);
				}
			}
			dpcm->ring_bak[0].writePtr = dpcm->ring_bak[0].beginAddr + (dpcm->appl_ptr % runtime->buffer_size) * dpcm->output_frame_bytes;
			dpcm->ring[0].writePtr = htonl(dpcm->ring_bak[0].writePtr);
...
void COPY_FUNC_DATA_FROM_DMIC0_DMIC1(int n_frame, void *src, long *ring_rp[2])
{
	int i, temp;
	short *p = (short *) src;
...
void COPY_FUNC_DATA_FROM_DMIC0_DMIC1_32BIT(int n_frame, void *src, long *ring_rp[2])
{
	int i, temp;
	int *p = (int *) src;
	...
	long n_period, int write_zero)
{
	...
	if (is_far_field(substream->pcm->device)) {
		...
		if (write_zero == 0) {
			ring_rp[0] = (long *) (dpcm->ring_p + ntohl(dpcm->ring[0].readPtr[dpcm->rd_idx]) - ntohl(dpcm->ring[0].beginAddr));
			ring_limit[0] = (long) (dpcm->ring_p + ntohl(dpcm->ring[0].size));

			dmac_flush_range(dpcm->ring_p, ring_limit[0]);
		...
		if (write_zero == 0) {
			for (i = 0; i < 2; ++i) {
				ring_rp[i] = (long *) (phys_to_virt(ntohl(dpcm->ring[i].beginAddr)) + ntohl(dpcm->ring[i].readPtr[0]) - ntohl(dpcm->ring[i].beginAddr));
				ring_limit[i] = (long) (phys_to_virt(ntohl(dpcm->ring[i].beginAddr)) + ntohl(dpcm->ring[i].size));

				dmac_flush_range(phys_to_virt(ntohl(dpcm->ring[i].beginAddr)), ring_limit[i]);
...
static int rtk_snd_playback_mmap(struct snd_pcm_substream *substream, struct vm_area_struct *area)
{
       ...
       if (substream->pcm->device == AAUDIO_DEVICE) {
               pr_info("[ALSA] Do rtk_snd_ion_mmap\n");
               ret = rtk_snd_ion_mmap(substream, area);
       } else {
               pr_info("[ALSA] Do snd_pcm_lib_default_mmap\n");
               ret = snd_pcm_lib_default_mmap(substream, area);
       }
       return ret;
}
#endif
...
static int rtk_snd_playback_close(struct snd_pcm_substream *substream)
{
	...
	pr_info("[ALSA] playback del timer done\n");
...
static int rtk_snd_capture_close(struct snd_pcm_substream *substream)
{
	...
	pr_info("[ALSA] close a realtek pcm capture channel\n");
	/* Mars 1 for AI capture, Mars 2 for AO capture */
	if (!strcmp(dpcm->card->card->longname, "Mars_1")) {
