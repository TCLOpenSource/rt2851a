...
void __init rtk_reserve(void)
{
	...
	memblock_reserve(carvedout_buf[layout_idx][CARVEDOUT_VDEC_RINGBUF][0], carvedout_buf[layout_idx][CARVEDOUT_VDEC_RINGBUF][1]);   // need content protection
...
//		ret |= add_ban_info(carvedout_buf[layout_idx][CARVEDOUT_TP][0], carvedout_buf[layout_idx][CARVEDOUT_TP][1], BAN_NOT_USED);
...
#if 1 //debug
    {
        ...
        pr_info("carvedout vdec-comem start(%lx)..(%lx), size(%ld_KB))\n", addr, addr + size, (size/1024));
...
/* Panic hook for boot.reason */
static int boot_reason_panic(struct notifier_block *this, unsigned long ev, void *ptr)
{
    printk(KERN_EMERG, "Panic reason: %s \n", (unsigned char *)ptr);
    rtk_write_boot_reason(BREASON_KERNEL_PANIC);
...
static int __init check_boot_reason_panic_init(void)
{
	atomic_notifier_chain_register(&panic_notifier_list, &boot_reason_panic_block);
	return 0;
}

early_initcall(check_boot_reason_panic_init);

/* API for boot.reason */
void rtk_write_boot_reason(enum eBootreason pri_reason)
{
    printk(KERN_EMERG , "%s:%d  \n", __FUNCTION__, __LINE__ );
    rtd_outl(BOOTREASON_REG, (pri_reason)&BOOTREASON_PRI_mask);
}

bool rtk_is_panic_happen(void)
{
    return (rtd_inl(BOOTREASON_REG)&BOOTREASON_PRI_mask) == BREASON_KERNEL_PANIC ? true:false;
}
...
void rtk_machine_restart(enum reboot_mode mode, const char *cmd)
{
	...
	kill_watchdog ();
#endif
	printk(KERN_EMERG"reboot argu=%s,   reboot_mode=%d \n", cmd, mode);
    ...
    /* Android boot reason, if panic occurs before reboot, skip writing REBOOT to boot reason.*/
    if(!rtk_is_panic_happen())
        rtk_write_boot_reason(BREASON_REBOOT);
