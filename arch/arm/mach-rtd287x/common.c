...
extern void __init sb2_intr_setup(void);
extern void __init sb2_dbg_intr_setup(void);
extern void __init monitor_kcpu_text_phy(void);
extern void __init monitor_overrun_phy(void);
extern void __init monitor_dc_phy(void);
extern void __init monitor_dc2_phy(void);
extern void __init monitor_scpu_text_phy(void);
#endif
...
void __init rtk_reserve_cma(unsigned int dma_limit, unsigned int lowmem_limit)
{
	...
	if (lowmem_limit >= 0x20000000) {
		...
		dma_declare_contiguous(NULL, (unsigned int)size, (unsigned int)addr, min(dma_limit, lowmem_limit));
	} else {
        addr = 0x02800000;
        size = lowmem_limit - 72*_MB_;
		dma_declare_contiguous(NULL, size, addr, min(dma_limit, lowmem_limit));
		carvedout_buf[layout_idx][CARVEDOUT_CMA_LOW_LIMIT][1] = 0;
	}
    pr_info("reserve %08lx - %08lx for low memory cma...\n", addr, addr + size);

	size = carvedout_buf_query(CARVEDOUT_CMA_LOW_LIMIT, (void *)&addr);
	if (size) {
		memblock_reserve(addr, size);
		pr_info("reserve %08lx - %08lx for low-limit memory cma...\n", addr, addr + size);
	}
}
...
void __init rtk_reserve(void)
{
	unsigned int ret = 0;
	unsigned int layout_idx = carvedout_buf_get_layout_idx();
	unsigned long ib_start_addr = 0, ib_start_size = 0;
	unsigned long ib_end_addr = 0, ib_end_size = 0;
	...
	/* for ib boundary */
	ib_start_size = carvedout_buf_query(CARVEDOUT_IB_START_BOUNDARY, (void*)(&ib_start_addr));
	ib_end_size = carvedout_buf_query(CARVEDOUT_IB_END_BOUNDARY,  (void*)(&ib_end_addr));
	/* we only reserve the previous one page from the boundary */
	if (ib_start_size) {
		memblock_reserve(ib_start_addr - ib_start_size, ib_start_size);
		pr_debug("Reserve IB region Start boundary (%lx @ %lx)\n", ib_start_size, ib_start_addr - ib_start_size);
	}
	if (ib_end_size) {
		memblock_reserve(ib_end_addr - ib_end_size, ib_end_size);
		pr_debug("Reserve IB region End boundary (%lx @ %lx)\n", ib_end_size, ib_end_addr - ib_end_size);
	}
