...
static struct resource rtk_usb_xhci_resources[] = {
	[0] = {
		.start		= GET_MAPPED_RBUS_ADDR(0xb8050000),
		.end		= GET_MAPPED_RBUS_ADDR(0xb8050000 + 0x8fff),
		.flags		= IORESOURCE_MEM,
	},
...
static struct platform_device rtk_usb_xhci_device = {
	.name		= "xhci-hcd",
	.id		= -1,
	.dev = {
	...
	.num_resources	= ARRAY_SIZE(rtk_usb_xhci_resources),
	.resource	= rtk_usb_xhci_resources,
};
...
unsigned long carvedout_buf[MM_LAYOUT_CARVEDOUT_NUM][CARVEDOUT_NUM][2] = {
  // MM_LAYOUT_DEFAULT
  {
	...
	{0x15f00000,  1*_MB_}, // CARVEDOUT_K_BOOT
#ifdef CONFIG_OPTEE
...
#endif
#endif
	{RBUS_BASE_PHYS, 34*_MB_}, // CARVEDOUT_MAP_RBUS

#ifdef CONFIG_REALTEK_SECURE_DDK
	{0x1a300000 - RTK_GPU_FW_SIZE, RTK_GPU_FW_SIZE}, // CARVEDOUT_GPU_FW
#endif
	{0x1a300000, 13*_MB_}, // CARVEDOUT_AV_OS
	...
	{0x02800000, 128*_MB_/*CMA_LOW_RESERVED_SIZE*/}, // CARVEDOUT_CMA_LOW
	{0, _TBD_}, // CARVEDOUT_CMA_HIGH
	{0, _TBD_}, // CARVEDOUT_CMA_3
	{0, _TBD_}, // CARVEDOUT_CMA_LOW_LIMIT
	{0, _TBD_}, // CARVEDOUT_CMA_BW

	/* for ddr interleave bank */
	{_TBD_, _TBD_}, // CARVEDOUT_IB_START_BOUNDARY
	{_TBD_, _TBD_}, // CARVEDOUT_IB_END_BOUNDARY

	{0, CMA_HIGH_RESERVED_SIZE}, // CARVEDOUT_GPU_RESERVED
  },
...
{
    const char *str_RTD287X = "RTD287X";

    if (strncmp(str_RTD287X, str, strlen(str_RTD287X))==0)
    {
        platform=PLATFORM_RTD287X;
        pr_info("RTDd287X platform\n");
    }
    else
    {
...
#define VERSION_A_REG_CODE	0x65050000
...
static const char *carvedout_mem_dts_name[CARVEDOUT_NUM] = {
	...
	/* for ddr interleave bank */
	"ib_start_boundary", // CARVEDOUT_IB_START_BOUNDARY
	"ib_end_boundary", // CARVEDOUT_IB_END_BOUNDARY
...
unsigned long carvedout_buf_query(carvedout_buf_t what, void **addr)
{
	...
	switch (what)
	{
	// TODO:  IB boundary info will be got from IB APIs
	case CARVEDOUT_IB_START_BOUNDARY:
		if (layout_idx == MM_LAYOUT_DEFAULT) {
			address = (void *)0x20000000;
			size = 0x1000;  // 1 page
		} else {
			// Not reserve IB boundaries for 1 GB platform
			address = (void *)0x00000000;
			size = 0x0;
		}
		break;
	case CARVEDOUT_IB_END_BOUNDARY:
		if (layout_idx == MM_LAYOUT_DEFAULT) {
			address = (void *)0x60000000;
			size = 0x1000;  // 1 page
		} else {
			// Not reserve IB boundaries for 1 GB platform
			address = (void *)0x1ffff000;
			size = 0x0;
		}
		break;
...
int platform_support_vdec_dual_decoding(void)
{
	unsigned long vdec_size = 0;
	unsigned long vdec_addr = 0;

	vdec_size = carvedout_buf_query(CARVEDOUT_VDEC_VBM, (void **)&vdec_addr);

	if (vdec_size == VDEC_BUFFER_SIZE)
		return true;
	else
		return false;
}
