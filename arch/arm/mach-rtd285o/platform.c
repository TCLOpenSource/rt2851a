...
{
    const char *str_RTD285O = "RTD285O";

    if (strncmp(str_RTD285O, str, strlen(str_RTD285O))==0)
    {
        platform=PLATFORM_RTD285O;
        pr_info("RTDd285O platform\n");
    }
    else
    {
...
{
#if defined(CONFIG_ARCH_RTK285O)
#define CHIP_CODE (0x6543)
#else
#error "Lost definition of this chip"
#endif

#define CHIP_CODE_SHIFT (16)
#define CHIP_CODE_MASK (0xffff0000)
#define VERSION_CODE_SHIFT (0)
#define VERSION_CODE_MASK (0x0000ffff)
#define VERSION_A_REG_CODE (0x0000)
#define VERSION_B_REG_CODE (0x0010)

	unsigned int ver_code_reg;
	unsigned int chip;
	unsigned int ver;

	ver_code_reg = rtd_inl(STB_SC_VerID_reg);

	chip = (ver_code_reg & CHIP_CODE_MASK) >> CHIP_CODE_SHIFT;
	ver = (ver_code_reg & VERSION_CODE_MASK) >> VERSION_CODE_SHIFT;

	if (chip != CHIP_CODE)
	{
		pr_err("ERR: this chip id is not matched.\n");
		return VERSION_A; // if chip is not matched, return verA for bringup
	}

	if (ver == VERSION_B_REG_CODE)
		return VERSION_B;
	else if (ver == VERSION_A_REG_CODE)
		return VERSION_A;
	else
		return VERSION_B; // if version has updated, need to modify this return value
}
...
unsigned long carvedout_buf_query(carvedout_buf_t what, void **addr)
{
	...
	case CARVEDOUT_ZONE_ZRAM:
		// If DRAM size is 1.5GB, there will be some region set as zone ZRAM
		if (DRAM_size == 0x60000) {
			// TODO: bootcode may deliver unbalanced region size to kernel
			address = (void *)0x40000000;
			size = (DRAM_size - 0x40000) << PAGE_SHIFT;
			pr_info("[MEM] Set 0x%lx to 0x%lx as zone ZRAM\n", address, address + size);
		} else {
			address = (void *)0x40000000;
			size = 0;
			pr_info("[MEM] No zone ZRAM\n");
		}
		break;
#endif
...
#define SVP_PROT_ALIGN 6*1024
	...
	case CARVEDOUT_SCALER_OD:
		...
		pr_debug("[MEM] carvedout query secure addr=%p size=%lx\n", address, size);
		break;
	default:
#ifndef CONFIG_ARM64
		address = (void *)carvedout_buf[layout_idx][what][0];
...
int carvedout_buf_query_is_in_range(unsigned long in_addr, void **start, void **end)
{
	...
	carvedout_buf_t idx;
#ifdef CONFIG_REALTEK_LOGBUF
	carvedout_buf_t whitelist[] = {CARVEDOUT_BOOTCODE, CARVEDOUT_VDEC_RINGBUF, CARVEDOUT_LOGBUF, CARVEDOUT_VDEC_VBM, CARVEDOUT_TP};
#else
