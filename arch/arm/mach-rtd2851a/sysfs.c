...
void dump_parent_info(char * upload_buf)
{
	...
	free_size = free_size - strlen(buf);
        ...
        if(upload_buf)
        {
            snprintf(upload_buf, free_size, "%s",buf);
        }
        pr_err("%s", buf);
...
static ssize_t apk_boost_mode_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
	...
	extern void rtk_printk_cpufreq_load(void);
	rtk_printk_cpufreq_load();
#endif
...
static ssize_t sched_log_name_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
	...
	if(page) {
		strncpy(platform_info.sched_log_name, page, min(strlen(page)-1, name_max_len-1));
...
static ssize_t arm_wrapper_store (struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
    ...
    if (!arm_wrapper)
    {
        ...
        if (!arm_wrapper)
        {
            pr_err("%s %d  memory allocate fail\n",__func__,__LINE__);
            return count;
        }

        addr_s=virt_to_phys(arm_wrapper);
        addr_e =virt_to_phys(arm_wrapper)+sizeof(arm_wrapper);
    }
    ...
    if (strncmp(page,"dbg_show",strlen("dbg_show"))==0)
    {
        ...
        pr_err("echo dbg_flush_cache >/sys/realtek_boards/arm_wrapper   (flush cache by copy lots of data)\n\n");
    }
...
static ssize_t stravnoreload_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
	...
	return snprintf(page, PAGE_SIZE-1, "stravnoreload_show:%d\n", stravnoreload);
}
...
#define POWER_ON_RESOURCE_ADDR                      (0x04eff000)     // 0C6ff000~0C70_0000
static  unsigned long rtk_resource_addr=POWER_ON_RESOURCE_ADDR;
...
typedef struct
{
        ...
        unsigned int cpu_resource_offset;
...
void * __init get_platform_resource(void)
{
        ...
        if(pAnimation)
        {
                resource_table_virt_addr = pAnimation + rtk_resource_addr - reserved_dvr_start;
                cal_sum = resource_table_virt_addr->magic_num + resource_table_virt_addr->scaler_resource_offset + resource_table_virt_addr->throttle_resource_offset + resource_table_virt_addr->cpu_resource_offset;
                ...
                vsc_resource = (VSC_RESOURCE *)((unsigned long)resource_table_virt_addr + resource_table_virt_addr->scaler_resource_offset);
        ...
        {
                printk("%s pAnimation(0x%lx) is invalid\n", __FUNCTION__, (unsigned long)pAnimation);
                goto error;
        }

        printk("vsc_resource.xml is valid\n");
...
#if 0
void dump_cpu_resource_log(CPU_CONTENT_T *cpu_content)
{
        int i = 0;
        int j = 0;
        
        for(i=0; i<CPU_LINE_MAX; i++)
        {
                if(!strlen(cpu_content[i].module_name_content))
                        continue;
                
                printk("%s ", cpu_content[i].module_name_content);
                
                for(j=CPU_ENABLE_KEY_E; j<CPU_MAX_KEY_E; j++)
                        printk("%d ", cpu_content[i].content[j-1]);
                printk("\n");
        }
}
#endif
...
void * __init get_platform_throttle_resource(void)
{
        ...
        if(pAnimation)
        {
                ...
                throttle_resource = (THROTTLE_RESOURC_T *)((unsigned long)resource_table_virt_addr + resource_table_virt_addr->throttle_resource_offset);
        ...
        printk("throttle_resource.xml is valid\n");
...
void * __init get_platform_cpu_resource(void)
{
        RESOURCE_TABLE *resource_table_virt_addr =  NULL;
        int cal_sum = 0;
        CPU_RESOURC_T *cpu_resource = NULL;
        int *p_tmp = NULL;
        int i = 0;
        ...
        if(pAnimation)
        {
                ...
                cpu_resource = (CPU_RESOURC_T *)((unsigned long)resource_table_virt_addr + resource_table_virt_addr->cpu_resource_offset);
                if((cpu_resource->magic_num != RESOURCE_MAGIC_NUM) || (!cpu_resource->valid))
                {
                        printk("%s cpu_resource is invalid; magic_num(0x%x) != 0x%x, valid(0x%x)\n", __FUNCTION__, cpu_resource->magic_num, RESOURCE_MAGIC_NUM, cpu_resource->valid);
                        goto error;
                 }

                 p_tmp = (int *)(cpu_resource->cpu_content);
                 cal_sum = 0;
                 for(i=0; i<sizeof(cpu_resource->cpu_content)/4; i++)
                 {
                        cal_sum += p_tmp[i];
                 }

                 if(cal_sum != cpu_resource->check_sum)
                 {
                        printk("%s cpu_resource->check_sum(0x%x) != cal_sum(0x%x)\n", __FUNCTION__, cpu_resource->check_sum, cal_sum);
                        goto error;
                 }
        }
        else
        ...
        printk("cpu_resource.xml is valid\n");

        //dump_cpu_resource_log((CPU_CONTENT_T*)(cpu_resource->cpu_content));
        
        return (void *)(cpu_resource->cpu_content);
...
//	unsigned int *unlzma = (unsigned int *)phys_to_virt(UNLZMA_SYNC_ADDR);
	...
	pr_info("[POV] cmd = %d, pAnimation = %p\n", cmd, pAnimation);
	 ...
	 */
	pr_info("[POV] cmd = %d, pLastImage = %p\n", cmd, pLastImage);
	if ((cmd == 2) && (pLastImage)) {
		dvr_free(pLastImage);
		pLastImage = 0;
        printk("\033[1;44m [%s] cmd=%d...\033[0m\n", __FUNCTION__, cmd);
...
{
	return snprintf(page, PAGE_SIZE-1, "Please add command as parameter...\n");
}

static ssize_t dvrfs_buffer_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
	char buffer[30] = {0}, *s_dup = NULL, *s_tmp = NULL;
	int cmd, err;
	unsigned int addr = 0, size = 0;
	const char delim[] = " ";
	strncpy(buffer, page, 29);
	s_dup = kstrdup(page, GFP_KERNEL);
	s_tmp = strsep(&s_dup, delim);
	err = kstrtoint(s_tmp, 10, &cmd);
	if(err){
		printk("get cmd fail\n");
		goto out;
	}
	s_tmp = strsep(&s_dup, delim);
	err = kstrtouint(s_tmp, 16, &addr);
	if(err){
		goto out;
	}
	s_tmp = strsep(&s_dup, delim);
	err = kstrtouint(s_tmp, 16, &size);
	if(err){
		goto out;
	}
out:
...
//		printk("%s\n", token);
  ...
  if (pcb_enum_count == 0) return 0;
	...
	{
		pr_err("allocate pcb_enum_count failed\n");
		return 0;
	}
...
//mac5p can not adjust scpu voltage by software, so return the const voltage value directly. said by clamp
...
static ssize_t pll_info_show(struct kobject * kobj, struct kobj_attribute * attr, char * page)
{
    ...
    msg[sizeof(msg)-1] = 0;
	strncat(page_info, msg, sizeof(page_info) - sizeof(msg)-1);
	...
	if (freq != 0)
		...
		snprintf( msg, sizeof(msg)-1, "DDR    PLL       = --- MHz (DC1)\n");
        msg[sizeof(msg)-1] = 0;
	strcat(page_info, msg);
...
static ssize_t fw_git_version_show(struct kobject * kobj, struct kobj_attribute * attr, char * page)
{
	...
	memset(msg, 0, sizeof(msg));
#if (defined CONFIG_RTK_KDRV_VDEC) && (!defined CONFIG_ARM64)
	...
	memset(msg, 0, sizeof(msg));
#if (defined CONFIG_RTK_KDRV_AUDIO) && (!defined CONFIG_ARM64)
...
#if defined(CONFIG_RTK_KDRV_CPU_FREQUENCY)
...
static int __init realtek_sysfs_rtk_boost_init(struct kobject *parent_kobj)
{
    int err = 0;

    err = sysfs_create_group(parent_kobj, &rtk_boost_attr_group);
    if(err) {
        printk("Realtek: sysfs init rtk_boost failed\n");
        return err;
    }
...
static __init int early_parse_rtk_rcs (char *str)
{
    if(str) {
	rtk_resource_addr= simple_strtoull(str, NULL, 16);
	pr_info("rtk_resource_addr:%lx\n",rtk_resource_addr);
    }

    return 0 ;
}

early_param("rtk_rcs", early_parse_rtk_rcs);
