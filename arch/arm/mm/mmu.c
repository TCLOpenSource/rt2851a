...
{
    struct map_desc map;
    unsigned long addr = 0x1ca00000;
    unsigned long size = 0;
    unsigned long start_addr = 0;

    size = rtdlog_get_buffer_size();
    start_addr = rtd_inl(STB_SW_DATA15_reg);
    ...
    if(((start_addr & 0xFFFF) == 0xCDEF) && (start_addr < 0x20000000)){
	addr = (start_addr & 0xFFFF0000);
    }
