...
#endif
			if (carvedout_end == (max_high * PAGE_SIZE)) { // carvedout is in last
				if (zone_size[ZONE_HIGHMEM] < (carvedout_high_size / PAGE_SIZE))
					BUG();
				zone_size[ZONE_MOVABLE] += (carvedout_high_size / PAGE_SIZE);
				zone_size[ZONE_HIGHMEM] -= (carvedout_high_size / PAGE_SIZE);
			}
#else
...
//	dma_contiguous_reserve(arm_dma_limit);
	...
	dma_contiguous_reserve(arm_dma_limit);
#else
	rtk_reserve_cma((unsigned int)arm_dma_limit, (unsigned int)arm_lowmem_limit);
#endif
		...
		if (hcnt > 0) {
			...
			if (hcnt > 1) {
				if (carvedout_high_size + carvedout_cma_high_size > mi->bank[last].size) { // No highmem, only carvedout
					...
					if (carvedout_end == (mi->bank[last].start + mi->bank[last].size)) { // carvedout is in last
						cma_highmem_start = (mi->bank[last].start + mi->bank[last].size) - carvedout_high_size - cma_highmem_size;
					}
					#endif
			...
			if (ret)
				...
				carvedout_buf[layout_idx][CARVEDOUT_CMA_BW][0] = cma_highmem_bw_start;
				carvedout_buf[layout_idx][CARVEDOUT_CMA_BW][1] = cma_highmem_bw_size;
			}
#endif
