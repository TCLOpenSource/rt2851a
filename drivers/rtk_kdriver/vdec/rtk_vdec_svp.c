...
int rtkvdec_svp_laycout_sync_tee()
{
	...
	layout.tzram_size = tzsize;
        ...
        pr_err("layout index=%d svp_base=%x, svp_size=%x tzbase=%x tzsize=%x\n", layout.mem_layout_index,
        ...
        if (shm_buffer == NULL) {
                pr_err("%s %d svp layut: no shm_buffer\n", __func__, __LINE__);
                ret = -ENOMEM;
                goto out;
        }
...
rtkvdec_svp_enable_cpb_protection (unsigned int addr, unsigned int size)
{
  ...
  if (rc || arg.ret) {
      pr_err("svp-open: invoke failed ret %x arg.ret %x/%x\n", rc, arg.ret,
             arg.ret_origin);
      ret = -EINVAL;
      goto out;
  }
...
rtkvdec_svp_disable_cpb_protection (unsigned int addr, unsigned int size)
{
  ...
  rc = tee_client_invoke_func(svp_ta.ctx, &arg, &param[0]);
  if (rc || arg.ret) {
      pr_err("svp-close: invoke failed ret %x arg.ret %x/%x\n", rc, arg.ret,
             arg.ret_origin);
      ret = -EINVAL;
      goto out;
  }
