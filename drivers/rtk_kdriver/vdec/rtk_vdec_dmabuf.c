...
#define RTK_VDEC_DMABUF_CREATE_BUF(privdata, dmabuf, length)                   \
	...
	if (IS_ERR_OR_NULL(dmabuf)) {                                          \
		ret = -EFAULT;                                                 \
		pr_err("[rtk_vdec_dmabuf:%d] buf export err\n", __LINE__);     \
		up(&DmabufLock);                                              \
		goto err;                                                      \
	}                                                                      \
	up(&DmabufLock);
...
#define RTK_VDEC_DMABUF_CHECK_ADDRESS(addr)                                    \
	if (!addr) {                                                           \
		pr_err("[rtk_vdec_dmabuf:%d] Didn't allocate buffer\n",        \
		       __LINE__);                                              \
		goto err;                                                      \
	}
...
{
	uint32_t svpmem_addr = 0, header_addr = 0, cma_addr = 0;
				      ...
				      struct rtk_vdec_dmabuf_mem_addr *addr)
{
	...
	addr->virtual = (uint32_t)dvr_malloc_uncached_specific(
	...
	addr->physical = (uint32_t)dvr_to_phys((void *)addr->virtual);
	if (!addr->physical) {
		pr_err("[%s %d]alloc memory fail\n", __func__, __LINE__);
		goto err;
	}
				  ...
				  struct rtk_vdec_dmabuf_allocation_data *data)
{
	...
	if (!data) {
		pr_err("[rtk_vdec_dmabuf:%d] Error parameter\n", __LINE__);
		goto err;
	}

	privdata = kzalloc(sizeof(*privdata), GFP_KERNEL);
	if (!privdata) {
		pr_err("[rtk_vdec_dmabuf:%d] Allocate privdata failed\n",
		       __LINE__);
		goto err;
	}
	...
	if (fd < 0) {
		ret = -EFAULT;
		pr_err("[rtk_vdec_dmabuf:%d] Get dma_buf_fd error \n",
		       __LINE__);
		goto err;
	}
					 ...
					 struct dma_buf *dmabuf)
{
	...
	privdata = dmabuf->priv;
	if (!privdata) {
		pr_err("[rtk_vdec_dmabuf:%d] Error dmabuf val\n", __LINE__);
		goto err;
	}
	...
	if (ret != 0) {
		pr_err("[rtk_vdec_dmabuf:%d] Allocate ring buffer failed\n",
		       __LINE__);
		up(&DmabufLock);
		goto err;
	}
	...
	struct rtk_vdec_dmabuf_allocation_data *data, bool secured)
{
	...
	if (fd < 0) {
		ret = -EFAULT;
		pr_err("[rtk_vdec_dmabuf:%d] dma_buf_fd error \n", __LINE__);
		goto err;
	}
...
static long rtk_vdec_dmabuf_get_info(struct file *filp, unsigned long arg)
{
	...
	if (IS_ERR_OR_NULL(dmabuf)) {
		pr_err("[rtk_vdec_dmabuf:%d] Get dma_buf by fd failed\n",
		       __LINE__);
		up(&DmabufLock);
		goto err;
	}
	...
	case FLAG_NONE:
		...
		break;
	default:
		pr_err("[rtk_vdec_dmabuf:%d] Error info flags\n", __LINE__);
		up(&DmabufLock);
		goto err;
	}
	up(&DmabufLock);
						      ...
						      unsigned long arg)
{
	...
	if (!(privdata->flags & FLAG_CPB) ||
		...
		pr_err("[rtk_vdec_dmabuf:%d] Get wrong privdata->flags\n",
		       __LINE__);
		up(&DmabufLock);
		goto err;
	}
	...
	if (ret != 0) {
		pr_err("[rtk_vdec_dmabuf:%d] Start secure buffer updates failed",
		       __LINE__);
		up(&DmabufLock);
		goto err;
	}
						       ...
						       unsigned long arg)
{
	...
	if (ret != 0) {
		pr_err("[rtk_vdec_dmabuf:%d] Commit secure buffer updates failed",
		       __LINE__);
		up(&DmabufLock);
		goto err;
	}
