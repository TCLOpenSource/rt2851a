...
typedef struct  
{
	...
	UBYTE  u1_mixMode_rgnEn[_FILM_MOT_NUM -1];
...
typedef struct  
{
	...
	UBYTE  u8_pre_phase_Idx[_FILM_MOT_NUM];
	...
	/////
	UINT32 u32_FilmFrame_cnt[_FILM_MOT_NUM];
	UINT32 u27_FrameMotionPool[60]; // pool length = _FILM_MOT_NUM.

	UBYTE  u1_32322_BadEdit_flag;
	UBYTE  u8_32322_BadEdit_cnt;
	UBYTE  u8_32322_BadEdit_idx;
...
VOID PowerSaving_Proc(UBYTE type);
...
DEBUGMODULE_BEGIN(_PARAM_FilmDetectCtrl,_OUTPUT_FilmDetectCtrl)
	...
	ADD_PARAM_ARRAY_DES(UBYTE , u1_mixMode_rgnEn, _FILM_MOT_NUM-1,   0, "L->M: rgn lft/rht/top/bot/cen")
