...
static inline void show_queued_rpc(RPC_DEV_EXTRA *extra, char *out)
{
	...
	if (out) {
		...
		/*pr_err("%s-rpc(%p),program:%u version:%u procedure:%u taskID:%u sysPID:%u size:%u context:%x %s\n",
...
#ifdef CONFIG_REALTEK_RPC_KCPU
#ifdef CONFIG_ARM64_RPC
extern RPC_INTR_Dev rpc_intr_kcpu_devices[RPC_INTR_DEV_KCPU_TOTAL];
extern RPC_KERN_Dev rpc_kern_kcpu_devices[RPC_NR_KERN_KCPU_DEVS];
#else
