            ...
            uint32_t bytemode, uint32_t movemode, uint32_t val)
{
    ...
    do {
        ...
        rtd_outl(MD_REG_CTRL_reg, rtd_inl(MD_REG_CTRL_reg) | 0x1);
        rtd_outl(MD_REG_MODE_reg, rtd_inl(MD_REG_MODE_reg) | 0x1);
        ...
        while(rtd_inl(MD_REG_MODE_reg) & 0x1) {
            ...
            if(timeout > 100) {
                ...
                rtd_outl(MD_REG_MODE_reg, rtd_inl(MD_REG_MODE_reg) | 0);
                return -1;
            }
        }
        len -= len_tmp;
