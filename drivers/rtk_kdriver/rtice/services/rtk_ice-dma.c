			      ...
			      unsigned int response_buff_size )
{
    ...
    break;
    case RTICE_IOMEM_CMD_START_ADDR_TOGGLE:
        RTICE_DEBUG("[%s:%d]start_address_toggle.\r\n", __func__,__LINE__);
        ret = start_address_toggle(cmd_data, cmd_len);
        if(ret != 0){
            RTICE_DEBUG("[%s:%d]start_address_toggle.\r\n", __func__,__LINE__);
            ret = stop_address_toggle();
        }
        break;
    case RTICE_IOMEM_CMD_GET_ADDR_TOGGLE:
        RTICE_DEBUG("[%s:%d]RTICE_IOMEM_CMD_GET_ADDR_TOGGLE.\r\n", __func__,__LINE__);
        ret = get_address_toggle(response_data, response_buff_size);
        break;
    case RTICE_IOMEM_CMD_EXIT_ADDR_TOGGLE:
        ret = stop_address_toggle();
        break;
    default:
        break;
