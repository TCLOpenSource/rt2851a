...
static int check_result(unsigned int*pRet,unsigned int len,int fdcu2)
{
	...
	*ptotal_idle2 = 0;
...
#ifdef SUPPORT_DCU2    
	reg_eff_meas_ctrl.regValue = rtd_inl(dcu_addr(DC1_MC_EFF_MEAS_CTRL_reg,TRUE));
	reg_tmctrl3.regValue = rtd_inl(dcu_addr(DC1_MC_TMCTRL3_reg,TRUE));
	reg_read_cmd.regValue = rtd_inl(dcu_addr(DC1_MC_READ_CMD_reg,TRUE));
	reg_write_cmd.regValue = rtd_inl(dcu_addr(DC1_MC_WRITE_CMD_reg,TRUE));
	reg_read_chop_cmd.regValue = rtd_inl(dcu_addr(DC1_MC_READ_CHOP_CMD_reg,TRUE));
	reg_write_chop_cmd.regValue = rtd_inl(dcu_addr(DC1_MC_WRITE_CHOP_CMD_reg,TRUE));
	dividend = (u64)40000 * reg_read_cmd.rd_cnt - (u64)20000 * reg_read_chop_cmd.rd_chop_cnt;
	divisor = (u64)16 * (reg_tmctrl3.refcycle + 1) * reg_eff_meas_ctrl.meas_period * 4;
	...
	/*efficient extension info(conflict ,w2r,r2w count)*/
	eff2[3] = DC1_MC_BG_UNBALANCE_CNT_get_bg_unbalance_cnt(rtd_inl(dcu_addr(DC1_MC_BG_UNBALANCE_CNT_reg,TRUE)));//bg_unbalance_cnt
	/*small range ratio*/
