...
extern void creat_led_device_node(struct device *dev);
extern void remove_led_device_node(struct device *dev);
...
#ifdef CONFIG_PM
static int rtk_led_suspend(struct device *dev)
{
    RTK_LED_INFO("suspend\n");
    return 0;
}

static int rtk_led_resume(struct device *dev)
{
    RTK_LED_INFO("resume\n");
    return 0;
}

static const struct dev_pm_ops rtk_led_pm_ops = {
    .suspend = rtk_led_suspend,
    .resume = rtk_led_resume,
};
#endif
