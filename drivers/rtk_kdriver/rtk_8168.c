        ...
        char *buf)
{
    ...
    {
        printk(KERN_EMERG"mac addr[0x%02x]=0x%08x\n",i,RTL_R8(i));
    }
        ...
        char *buf)
{
    ...
    printk(KERN_EMERG"cur_tx=%u\n",pp);
    pp=tp->cur_rx%NUM_RX_DESC;
    printk(KERN_EMERG"cur_rx=%u\n",pp);
    for(i=0;i<NUM_TX_DESC;i++)
    {
        txd = tp->TxDescArray+i;
        if(txd)
        printk(KERN_EMERG"tx desc[%d] addr=0x%llx opt1=0x%x opt2=0x%x\n"
                ,i
                ,txd->addr
    ...
    for(i=0; i<NUM_RX_DESC; i++)
    {
        ...
        if(rxd)
        printk(KERN_EMERG"rx desc[%d] addr=0x%llx opt1=0x%x opt2=0x%x\n"
                ,i
                ,rxd->addr
