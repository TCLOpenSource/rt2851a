...
#define EMCU_WIFI_POWER_NAME    "PIN_WOL_PWR_EN"
    ...
    while ((opt = strsep(&buftmp, ",")) != NULL) {
        ...
        if(i && (type == 0xff)){    // parse PCB parameter
            if(pcb_mgr_get_enum_info_byname(opt, &tmp_long) == 0){
                ...
                else if (!strncmp(opt, EMCU_WIFI_POWER_NAME, strlen(EMCU_WIFI_POWER_NAME))){
                    EMCU_DEBUG("wifi power pin hit\n");
                    param[i] |= IO_TPYE_WIFI_POWER;
                }
    ...
    const char *buf, size_t count)
{
    ...
    if(in_val > 3){
        EMCU_DEBUG("\nparameter big then 7. Ignore setting.\n");
        ret = -EINVAL;
        goto FINISH_OUT;
    }else{
 ...
 ************************************************************************/
char* power_pin_tlb[3] = {
    EMCU_POWER_PIN_NAME,
    EMCU_WIFI_POWER_NAME,   //for PIN_WLAN_RESET
    EMCU_DDR_POWER_NAME
};
...
int powerMgr_power_en_parser(const char *buf, size_t count,unsigned int ctl)
{
    ...
    if(ctl == PARSE_USER){
        ...
        cnt_tmp = 0;
        for(i=0;i<PARAM_LEN_PWEN;i++){
            /* You can change the table power_pin_tlb[] to select which pin you want to check */
            if(pcb_mgr_get_enum_info_byname(power_pin_tlb[i], &rtk_tmp_gpio)==0){
                if (!strncmp(power_pin_tlb[i], EMCU_DDR_POWER_NAME, sizeof(EMCU_DDR_POWER_NAME))) {
                    rtk_tmp_gpio |= IO_TPYE_DDR_POWER;
                }else if (!strncmp(power_pin_tlb[i], EMCU_WIFI_POWER_NAME, sizeof(EMCU_WIFI_POWER_NAME))) {
                    rtk_tmp_gpio |= IO_TPYE_WIFI_POWER;
                }
                cnt_tmp++;
        ...
        if(cnt_tmp == 0){
            EMCU_DEBUG("No power pin defined\n");
            ret = -EINVAL;
            goto FINISH_OUT;
        }
 ...
 ************************************************************************/
#ifdef  CONFIG_CUSTOMER_TV030
void emcu_pwm_output(int index,int duty){
    iso_pwm_duty_set_RBUS pwm_duty_set_reg;
    I_PWM_DUTY_SET_REG_R(index,&pwm_duty_set_reg);
    pwm_duty_set_reg.mis_pwm0_dut = duty;
    I_PWM_DUTY_SET_REG_W(index,&pwm_duty_set_reg);
}
#endif
...
int __init rtk_emcu_module_init(void)
{
    ...
    if (retval)
        goto FINISH_OUT;
#ifdef  CONFIG_CUSTOMER_TV030
    EMCU_INFO("emcu_kdv emcu_51 h_h\n");
    rtd_outl(REG_MAGIC_51, 0);
    rtd_outl(STB_ST_SRST1_reg, _BIT9);           // rst off
    rtd_outl(STB_ST_CLKEN1_reg, _BIT9 );

    if (pcb_mgr_get_enum_info_byname("PIN_LED_ON_OFF", &value) == 0) {
        index = GET_PIN_INDEX(value);
        emcu_pwm_output(index,0);
    }
#endif
