...
extern unsigned char newbase_hdmi_get_is_hdr10_type(unsigned char port);
...
unsigned char pre_hdmi_hdr10_mode[4]={0};
extern int flag_dolby_vsif_change;
unsigned char dolby_mode_change_triger_mute = FALSE;
unsigned char hdr10_mode_change_triger_mute = FALSE;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	{	// _BIT24
		...
		hdr10_mode_change_triger_mute= FALSE;
		if(vfe_hdmi_drv_get_infoframe_thread_stop()==_TRUE)
		{
				if(VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
				{
					...
					if(pre_hdmi_hdr10_mode[port]!=newbase_hdmi_get_is_hdr10_type(port))
					{
						hdr10_mode_change_triger_mute=TRUE;
						pr_info("[%s:%d][HDR10] change: %d -> %d, set forecebg enable !!\n", __func__, __LINE__,pre_hdmi_hdr10_mode[port],newbase_hdmi_get_is_hdr10_type(port));
					}
					pre_hdmi_hdr10_mode[port]=newbase_hdmi_get_is_hdr10_type(port);
				}
		}
	}
#endif
...
//<<<End 20171228, pinyen modify for Dolby HDMI to avoid pink gabage when the begining from SDR to DolbyHDR
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)&& (hdr10_mode_change_triger_mute) && handle_ch1_end_flag)//interrupt timing
	{
		rtd_maskl(PPOVERLAY_Main_Display_Control_RSV_reg, ~(_BIT1), _BIT1);
		ppoverlay_double_buffer_ctrl3_reg.regValue = rtd_inl(PPOVERLAY_Double_Buffer_CTRL3_reg);
		//ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_en = 0;
	...
	if(I2R_PVR_MAIN_flag){
		...
		if ( Scaler_DispGetInfoPtr() != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE ){
                        ...
                        if(   drv_memory_get_vdec_direct_low_latency_mode()  ){
                                printk("[KK] Run on Low Latency Mode");
                                Enter_Hist = FALSE;
                        }else{
                                printk("[KK] Run on Normal Mode");
                                Enter_Hist = TRUE;
                        }
                        */
