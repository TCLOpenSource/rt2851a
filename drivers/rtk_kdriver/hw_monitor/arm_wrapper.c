...
static int arm_wrapper_init_flag;
#ifdef CONFIG_ARM64
void arm_wrapper_dft_set_linux_text (int on)
{
    unsigned long addr_start = __pa_symbol(_text);               // linux text start address
    unsigned long addr_end =   __pa_symbol(__start_rodata);      // linux text end address,cannot use __end_rodata

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
    addr_start = __pa_symbol(_stext);
#endif

    if(!arm_wrapper_init_flag)
    {
        return;
    }
...
void arm_wrapper_dft_set_linux_text (int on)
{
    ...
    if (1==on)
    {
        ...
        clear_arm_wrapper_arange(WRAPPER_DFT_ENTRY_SCPU_TEXT);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0))
        clear_arm_wrapper_arange(WRAPPER_DFT_ENTRY_02);
#endif
        dmac_flush_range(_stext,__start_rodata);
#endif
    }
