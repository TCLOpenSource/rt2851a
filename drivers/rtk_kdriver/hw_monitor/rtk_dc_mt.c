...
//for use DCMT_DEBUG_INFO_DECLARE in kernel module
...
EXPORT_SYMBOL(dump_dcmt_last_set);
...
{
#if 0 //marked because mac7p no dc2h module 
	DC_MT_DESC mt_desc;
...
{
    DC_MT_DESC mt_desc;
    unsigned int mem_size, mem_addr;
    unsigned long total_ddr_size = 0x60000000;
    total_ddr_size = get_memory_size(GFP_DCU1) + get_memory_size(GFP_DCU2);
    
    if(total_ddr_size != 0x60000000){
        pr_err("[%s] DDR total size(0x%lx) != 1.5G,only mac7p 1.5G platform need to set boundaries\n", __FUNCTION__, total_ddr_size);
        return;
    }
    
    mem_size=0;
    mem_addr=0;
    mem_size=carvedout_buf_query(CARVEDOUT_DDR_BOUNDARY,(void *)&mem_addr);
    
    if (mem_size==0 ||mem_addr==0)
    {
        pr_err("[%s] skip CARVEDOUT_DDR_BOUNDARY setting ......\n",__FUNCTION__);
        return;
    }

    memset(&mt_desc, 0, sizeof(mt_desc));
    mt_desc.type = MEMORY_TYPE_SERIAL_BLOCK;
    mt_desc.start = mem_addr+PAGE_SIZE; //add a gap for hw IPs
    mt_desc.end = mem_addr+mem_size-1;
    mt_desc.mode = 1;
    mt_desc.ip_entry[0].module = MODULE_ANY;
    mt_desc.ip_entry[0].prot = PROT_MODE_RW;
    dcmt_set_monitor(DCMT_DFT_MEM_DDR_BOUNDARY,&mt_desc);
