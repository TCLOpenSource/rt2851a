...
{
	SPI_INFO("Not support FCIC interface!\r\n");
	return -1;
}
...
static int sfc_check_pin_mux_cs0(void)
{
	...
	regval1 = (rtd_inl(0xB8000894) & 0xff0f0f0f) | 0x00181010;
...
static int sfc_set_clk(struct spinor_chip_t *spinor_chip, unsigned int clk)
{
	...
	unsigned int /*i = 0, */valid_loop = 0, valid_num = 0, delay_sel_std = 0, save_id[DELAY_SEL_STEP_TOTAL_CNT] = {0};
	...
	if(clk_div <= support_max_div && clk_div >= support_min_div) {
		...
		for(delay_sel=0; delay_sel< DELAY_SEL_STEP_TOTAL_CNT; delay_sel++)
		{
			...
			SPI_INFO("[SPINOR-INFO] ID: 0x%x, clk_div:0x%x\n", save_id[delay_sel], clk_div); 
		}
...
retry_sacn:
	...
	if (i == DEV_SIZE_S) {
		...
		SPI_INFO("[SPINOR-INFO]ID not in the ID table, use default value\n");
	}
