...
BOOL rtk_ai_mc_init(void){
	...
	aiControl=(AI_CONTROL *)dvr_malloc(sizeof(AI_CONTROL));
	AI_INFO("dvr_malloc aiControl=0x%x\n",(unsigned int)aiControl);
...
BOOL rtk_ai_mc_getShareMem(unsigned int *phy_addr){
	...
	*phy_addr=(unsigned int)virt_to_phys((void *)aiControl);
