...
#define I2C_MP8864_DEV_ADDR 0x68

#define voltage_size 4
//static unsigned char rt6203e_fix_voltage[3]={0xaa, 0x2d, 0xa3};// sepc code 42/45/30 => 1.07V/ 1.10V/1.00V
static unsigned char rt6203e_fix_voltage[voltage_size]={0xaa, 0x2d, 0xa3,0x1e};// sepc code 42/45/30 => 1.07V/ 1.10V/ 1.00V/0.95V
static unsigned char rt6203e_value = 0x0;
//static unsigned char mp8864_hiddq[2]={0x2f,0x2f};
static unsigned char mp8864_hiddq[voltage_size]={0x2f, 0x32, 0x28,0x23};// sepc code 42/45/30 => 1.07V/ 1.07V/ 1.00V/0.95V
static unsigned char mp8864_buf[3];
...
void rtk_tv030_init_voltage(void)
{
	if((tv030_voltage_init == DVFS_RT6203E) || (rt6203e_value != 0x0) )
	{
		...
		{
			if((rtk_get_overfreq_otp() & 0x3) == 0x0)
		...
		if(i2c_master_send_ex_flag(TV030_I2C_ID,I2C_RT6203E_DEV_ADDR,&buf,1,I2C_M_FAST_SPEED)<0)
		{
			pr_emerg(":Y:err. send rt6203e i2c vout:%x\n", buf);
			tv030_voltage_init = DVFS_MP8864;
		}

		pr_err("Init rt6203e with 0x:%x\n", buf);
	}

	if(tv030_voltage_init == DVFS_MP8864){

		mp8864_buf[0] = 0x0;
		mp8864_buf[2] = 0xe3;
		if((rtk_get_overfreq_otp() & 0x3) == 0x0){
			mp8864_buf[1]= mp8864_hiddq[1];
		}else{
			mp8864_buf[1]= mp8864_hiddq[0];
		}			
		pr_emerg(":Y: DVFS_MP8864<1> mp8864 i2c:mp8864_buf[0]=%x mp8864_buf[1]=%x,mp8864_buf[2]=%x,(int)&mp8864_buf=%x,(int)*mp8864_buf=%x. \n",mp8864_buf[0],mp8864_buf[1],mp8864_buf[2],(int)&mp8864_buf,(int)*mp8864_buf);	
			
		if(i2c_master_send_ex_flag(TV030_I2C_ID,I2C_MP8864_DEV_ADDR,&mp8864_buf[0],3,I2C_M_FAST_SPEED)<0){
			pr_emerg(":Y: DVFS_MP8864<2> send err mp8864 i2c:mp8864_buf[0]=%x mp8864_buf[1]=%x,mp8864_buf[2]=%x,(int)&mp8864_buf=%x,(int)*mp8864_buf=%x \n",mp8864_buf[0],mp8864_buf[1],mp8864_buf[2],(int)&mp8864_buf,(int)*mp8864_buf);						
		...
		pr_emerg(":Y:img 4/8k: DVFS_MP8864<3> mp8864 is set. i2c:mp8864_buf[0]=%x mp8864_buf[1]=%x,mp8864_buf[2]=%x,(int)&mp8864_buf=%x,(int)*mp8864_buf=%x \n",mp8864_buf[0],mp8864_buf[1],mp8864_buf[2],(int)&mp8864_buf,(int)*mp8864_buf);	
						
		if(i2c_master_send_ex_flag(TV030_I2C_ID,I2C_MP8864_DEV_ADDR,&mp8864_buf[0],2,I2C_M_FAST_SPEED)<0){
				pr_emerg(":Y: DVFS_MP8864<4> send err mp8864 i2c:mp8864_buf[0]=%x mp8864_buf[1]=%x,mp8864_buf[2]=%x,(int)&mp8864_buf=%x,(int)*mp8864_buf=%x \n",mp8864_buf[0],mp8864_buf[1],mp8864_buf[2],(int)&mp8864_buf,(int)*mp8864_buf);						
		}
	}
}

void rtk_tv030_set_voltage(int level)
{
	unsigned char buf;
	if(level >= voltage_size)
	{
		pr_err("send rt6203e i2c vout level error:%d\n", level);
		return;
	}

    if(tv030_voltage_init == DVFS_MP8864)
	{
		pr_err("DVFS_MP8864 voltage\n");
		mp8864_buf[0] = 0x0;
		mp8864_buf[2] = 0xe3;
		mp8864_buf[1] = mp8864_hiddq[level];
		if(i2c_master_send_ex_flag(TV030_I2C_ID,I2C_MP8864_DEV_ADDR,&mp8864_buf[0],3,I2C_M_FAST_SPEED)<0){
			pr_emerg(":Y: DVFS_MP8864<2> send err mp8864 i2c:mp8864_buf[0]=%x mp8864_buf[1]=%x,mp8864_buf[2]=%x,(int)&mp8864_buf=%x,(int)*mp8864_buf=%x \n",mp8864_buf[0],mp8864_buf[1],mp8864_buf[2],(int)&mp8864_buf,(int)*mp8864_buf);						
	...
	{
		pr_err("rt6203e voltage\n");
		buf = rt6203e_fix_voltage[level];
		if(i2c_master_send_ex_flag(TV030_I2C_ID,I2C_RT6203E_DEV_ADDR,&buf,1,I2C_M_FAST_SPEED)<0)
		{
			pr_err("send rt6203e i2c vout fail:%x\n", buf);
		}
	}
}
...
int __rtk_init_cpufreq_table(struct cpufreq_frequency_table **table)
{
	...
	if (!np)
		pr_err("%s: failed to find cpufreq table on DT node\n",__func__);
	...
	if(pCpufreq)
	{
		...
		for(i=0;i<5;i++)
		{
			if(pCpufreq[i].content[0]==1)
			{
	                        ...
	                        for (j=1; j<6 ;j++)
	                        {
		                     if (pCpufreq[i].content[j] > 1300)
		                         pCpufreq[i].content[j] = 1300;
                                }
#endif
...
#ifndef CONFIG_TV001_PLATFORM
	{
		...
		if((otp == 5) || (otp == 7))	//GRADE_A
			...
			freqTbl4[3] = 1200000;
		pr_info("Set CPU freq table(%d)(org:%d), boot_freq(%d), otp(%d)\n", freqTbl4[3], org, boot_freq, otp);
...
#ifdef CONFIG_TV030_PLATFORM
			rtk_tv030_set_voltage(current_level+1);
#endif
			rtk_cpu_dvfs_disable = 1;
		}
		else if(current_level == 1)
...
#ifdef CONFIG_TV030_PLATFORM
				if((rtk_get_overfreq_otp() & 0x3) == 0x0)
					rtk_tv030_set_voltage(1);	//1.10V
				else
					rtk_tv030_set_voltage(0);	//1.07V
				udelay(50);
#endif
