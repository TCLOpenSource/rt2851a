...
VOID FRC_decPhT_goldenGen_Proc(const Ph_decGen_Param *pParam, Ph_decGen_Out *pOutput)
{
...
#if 0
		pr_notice("[%s][%d][%d][%d,%d][%d][,%d,%d,%d,%d,%d,%d,%d,%d,%d,]", __FUNCTION__, __LINE__, u16_k,
	...
	{
		phTable_DecInfoGen[0].me1_i_offset   = 2; // 3;
		phTable_DecInfoGen[0].me1_p_offset   = 3; // 4;

		phTable_DecInfoGen[0].me2_i_offset   = 1;// 2;
		phTable_DecInfoGen[0].me2_p_offset   = 2;// 3;
#if 1  // k4lp
...
VOID FRC_phTable_LowDelay_Gen_Video(const Ph_decGen_Param *pParam, Ph_decGen_Out *pOutput, UINT16 u16_k , PQL_INPUT_FRAME_RATE in_fmRate, PQL_OUTPUT_FRAME_RATE out_fmRate, UBYTE u1_lowdelaystate)
{
	...
	printk(KERN_EMERG"[%s][%d][FrmRate=,%d,%d,][,%d,%d,]\n\r", __FUNCTION__, __LINE__, in_fmRate, out_fmRate, u16_MEMCMode, u1_adaptivestream_flag);
	...
	if(u1_lowdelaystate == 1 && (VR360_en == 0)){
		if(((in_fmRate == _PQL_IN_60HZ) && (out_fmRate == _PQL_OUT_120HZ))){
			if(u1_adaptivestream_flag == 1){
				for (u16_k = (UINT16)(pOutput->u16_phT_stIdx); u16_k < pOutput->u16_phT_endIdx_p1; u16_k ++)
				{
					UBYTE mc_i_offset[5] = 	{2,2};
					UBYTE mc_p_offset[5] = 	{2,2};
				...
				for (u16_k = (UINT16)(pOutput->u16_phT_stIdx); u16_k < pOutput->u16_phT_endIdx_p1; u16_k ++)
				{
					...
					UBYTE mc_i_offset[5] = 	{3,3};
					UBYTE mc_p_offset[5] = 	{3,3}; // jerry_wang, for the case "low delay + real cinema on" (K-12267) //{3,4} will cause transient noise from game mode to HDR-effect mode (K-12340)
				...
				for (u16_k = (UINT16)(pOutput->u16_phT_stIdx); u16_k < pOutput->u16_phT_endIdx_p1; u16_k ++)
				{
					...
					UBYTE mc_i_offset[5] = 	{4,4};
					UBYTE mc_p_offset[5] = 	{4,4}; // can't be "4". It's because "4" is the write buffer.
					UBYTE ppfv_offset[2] = 	{1,1};
...
#if 0
	pr_notice("[%s][%d][n2m,%d,%d,][,%d,%d,][,%d,%d,%d,]", __FUNCTION__, __LINE__, 
		pParam->u4_sys_N, pParam->u6_sys_M, fracPh_nume, fracPh_denomi, pOutput->u16_phT_stIdx, pOutput->u16_phT_length, pOutput->u16_phT_endIdx_p1);
#endif
