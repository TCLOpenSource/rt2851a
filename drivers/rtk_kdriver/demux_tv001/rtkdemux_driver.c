...
long DEMUX_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DEMUX_IOC_INIT || cmd == DEMUX_IOC_INIT_COMPAT) {
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_OPEN failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_CLOSE failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TP_SOURCE_CONFIG_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_SET_TP_INPUT_CONFIG failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_REQUEST_BBFRAME failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_CANCEL_BBFRAME failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_GET_BBFRAME_BUFFER failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_RETURN_BBFRAME_BUFFER failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_GET_BBFRAME_BUFF_INFO failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_GET_CURRENT_TIME_INFO failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		if(copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_TIME_INFO_T)))
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_SET_CURRENT_TIME_INFO failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_SET_SYSTEM_TIME_INFO failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_SET_CLOCK_RECOVERY failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_SET_MODE failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		if(copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_REQUEST_DATA failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		if(copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_DELIVERY_CHANNEL_T)))
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_CANCEL_DATA failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else
		...
		{
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_DELIVERY_GET_DATA failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		} else {
