...
void Demux_Enter_ECP(demux_channel *pCh)
{
	...
	if (pCh->tpBuffer.phyAddr){
		...
		unmap_kernel_line_addr(pBuf->phyAddr, pBuf->allocSize);
...
void Demux_Exit_ECP(demux_channel *pCh)
{
	...
	if (pCh->tpBuffer.phyAddr){
		...
		remap_kernel_line_addr(pBuf->phyAddr, pBuf->allocSize);
		if(pBuf->isCache){
	...
	for (pin=0;pin< MAX_VDEC_NUM;pin++){
		if(IS_VIDEO_PIN(pin) && pCh->bsDecBuf[pin].phyAddr && pCh->bsDecBuf[pin].virAddr == NULL)
		{
			...
			remap_kernel_line_addr(pBufBS->phyAddr, pBufBS->allocSize);
			if(pBufBS->isCache){
		   ...
		   dmx_notice("[%s] pBuf->cachedaddr=%lx pBuf->virAddr=%lx pBuf->nonCachedaddr=%lx pBuf->phyAddr=%lx\n",__func__, (unsigned long)(uintptr_t) pBufBS->cachedaddr,(unsigned long)(uintptr_t) pBufBS->virAddr, (unsigned long)(uintptr_t)pBufBS->nonCachedaddr,(unsigned long)(uintptr_t) pBufBS->phyAddr);
		}
	}
...
inline int DeliverInBandCommand(DEMUX_BUF_PIN_T *pPinInfo, unsigned char *Data, long size)
{
	...
	if (writableSize < size) {
		dmx_err("inband buffer full :w:%lx, r:%lx, b:%p, l:%p (%ld/)%ld\n",(unsigned long)(uintptr_t) phyWr,(unsigned long)(uintptr_t) phyRd, pPinInfo->pBufferLower, pPinInfo->pBufferUpper, writableSize, size);
		return -1;
	}
		...
		if (writableSize < totalPayloadSize[i]) {
			...
			if (++g_debug_ring_buffer_full_count > 0 && abs64(debug_time-ring_buffer_debug_time) > 3*DEMUX_CLOCKS_PER_SEC) {
				dmx_alert(DGB_COLOR_RED_WHITE" RTKDemux: (CH%d)(%s) ring buffer full (%d times), Drop = %d, WP = %lxh, RP = %lxh, size = %ld, req-size = %d\n"DGB_COLOR_NONE,
						...
						IsDropping(pCh, i),
						(unsigned long)(uintptr_t)phyWr,
						(unsigned long)(uintptr_t)phyRd,
...
int DEMUX_ReadData(demux_channel *pCh, NAVBUF *pBuffer, int isProhibited)
{
	...
	if (isProhibited) {
		...
		if (pCh->tp_src != MTP || pCh->events == 0) //for TCL mantis 0347050;JIRA TCL2851M-4712
			return 0 ;
	}
...
int DEMUX_ThreadProc(demux_channel *pCh)
{
	...
	while (!kthread_should_stop()) {
		...
		if(RHAL_Delivery_GetMode() == TP_DELIVERY_MODE_TS && pCh->tunerHandle == 0){
