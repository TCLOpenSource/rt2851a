...
#ifndef CONFIG_RTK_KDRV_JP4K
			/*[TCL2851M-3864]no audio pts loog time(almost 17s),then cause mpts << apts
			* use DTV_DELAY_TIME to calc mpts, if apts less than mpts, then enlarge pcr offset
			*/
			else if(closerInfo.demuxAPTS < (pcrinfo.pcr - pcrinfo.stc - DTV_DELAY_TIME + pll)){
				pPcrProcesser->pcrOffset = 4*DTV_DELAY_TIME;
			}
#endif
			else {
				/*
...
void PCRSYNC_CheckAVPTSStable(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	...
	if(!pPcrProcesser->isUnderFreeRunModePTS){
		...
		{
			if(AFreerunThresholdValue == MIN_FREE_RUN_THRESHOLD && (false == pPcrProcesser->isUnderFreeRunMode_audio)){
				REFCLK_SetAudioFreeRunThreshold(pPcrProcesser->pRefClock, DEFAULT_FREE_RUN_THRESHOLD, DEFAULT_FREE_RUN_THRESHOLD);
