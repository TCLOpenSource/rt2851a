...
{
	dmx_notice("[%s] pBuf->cachedaddr=%lx pBuf->virAddr=%lx pBuf->nonCachedaddr=%lx pBuf->phyAddr=%lx\n",__func__, (unsigned long)(uintptr_t) pBuf->cachedaddr,(unsigned long)(uintptr_t) pBuf->virAddr, (unsigned long)(uintptr_t)pBuf->nonCachedaddr,(unsigned long)(uintptr_t) pBuf->phyAddr);
...
int DEMUX_BUF_AllocTPBuffer(demux_channel * pCh)
{
	...
	unsigned int demux_tp_buffer_size;
	...
	if(pCh->tunerHandle == 0)
		demux_tp_buffer_size = DEMUX_TP0_BUFF_SIZE;
	else
		demux_tp_buffer_size = DEMUX_TP1_BUFF_SIZE;
...
#ifdef USE_STATIC_RESERVED_BUF
	{
		...
		dmx_info("startPhyAddr:0x%x chNum:%d totalSize:%d size:%d\n",startPhyAddr,pCh->tunerHandle,totalSize,demux_tp_buffer_size);
		if (totalSize > 0) {
			phyAddr = startPhyAddr + pCh->tunerHandle * DEMUX_TP0_BUFF_SIZE;
			if ((phyAddr+demux_tp_buffer_size) <= (startPhyAddr + totalSize)) {
				dmx_info("Using Reserved Buffer phyAddr: 0x%x\n",phyAddr);
				pBuf->phyAddr = phyAddr;
				pBuf->allocSize = demux_tp_buffer_size < 0x1000 ? 0x1000 : demux_tp_buffer_size ;
				pBuf->cachedaddr = dvr_remap_cached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
				pBuf->nonCachedaddr = dvr_remap_uncached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
				...
				pBuf->size = demux_tp_buffer_size ;
				pBuf->fromPoll = 0 ;
				pBuf->fromVBM = 0;
...
#ifdef TP_BUFFER_FROM_VBM
	{
		...
		if( DEMUX_BUF_AllocBuffer(&rpcBuf, sizeof(VIDEO_RPC_VBM_BORROWMEMCHUNK), 0, 1) == 0 && rpcBuf.virAddr) {
			...
			cmd->memChunkAddr = 0;
			cmd->memChunkSize = 0;
			cmd->reqSize = DEMUX_reverseInteger(demux_tp_buffer_size);
...
#endif
	{
		if (DEMUX_BUF_AllocBuffer_Ext(pBuf, demux_tp_buffer_size, 0, 0) < 0)
		{
			dmx_err("%s_%d: ch:%d DEMUX_BUF_AllocBuffer\n!!!!!", __func__, __LINE__,pCh->tunerHandle );
			goto ALLOCATE_FAIL;
		}
...
int  DEMUX_BUF_MTPBuffer_Create(demux_channel *pCh)
{
	...
	//reduce tp size
	if( pCh->tunerHandle == 0 )
		pTpBuf->size = DEMUX_TP0_BUFF_SIZE - MTP_BUFFER_SIZE;
	else
		pTpBuf->size = DEMUX_TP1_BUFF_SIZE - MTP_BUFFER_SIZE;
	// config mtp buffer
...
int DEMUX_BUF_MTPBuffer_Destroy(demux_channel *pCh)
{
	...
	if( pCh->tunerHandle == 0 )
		pTpBuf->size = DEMUX_TP0_BUFF_SIZE;
	else
		pTpBuf->size = DEMUX_TP1_BUFF_SIZE;
