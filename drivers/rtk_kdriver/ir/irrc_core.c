    ...
    */
    if(srValue & 0x1) {
        ...
        *regValue = rtd_inl(rpReg); /* read IRRP */
        ...
        else if(PANASONIC == ir_protocol && (((*regValue &0xFFFF0000)==PANASONIC_SNOFF)||((*regValue &0xFFFF0000)==PANASONIC_SNON))){
            *regValue = (*irrp2Value ) |(*regValue >>16);
           IR_DBG("[%s]qjREG_IR_RP = [%08X] REG_IR_RP2 = [%08X]\n", __func__, *regValue,*irrp2Value);
            }
...
int ir_proto_irda_init(char *options)
{
    ...
    if(strcmp(remoter, "rca") == 0)
        snprintf(ir_boot_parameter_settings[0].keylist_remoter, 32, "mtkinp");
    else
