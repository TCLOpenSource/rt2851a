...
void rtk_amp_ops_set(int amp_num, int sel)
{
    switch(sel){
        ...
        case RTK_ALC1310:
            ...
            rtk_amp[amp_num].error_handler = NULL;
        ...
        case TI_TAS5707:
            ...
            rtk_amp[amp_num].error_handler = check_amp_error_handler;
...
static int rtk_amp_init(void) {
        ...
        AMP_ERR("amp: AMP init for Realtek AMP(2020/12/24)\n");
        ...
        for (amp_num=0; amp_num<MULTI_AMP_CNT; amp_num++)
        {
            if (rtk_amp[amp_num].error_handler != NULL) {
                rtk_amp[amp_num].error_handler(rtk_amp[amp_num].amp_i2c_id, rtk_amp[amp_num].slave_addr);
            }
        }
        return 0;
