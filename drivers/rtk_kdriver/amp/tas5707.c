...
unsigned char get_error_status_data = 0x0;
...
int tas5707_dump_all(const char *buf, int amp_i2c_id, unsigned short slave_addr)
{
    ...
    for (index = 0; index < (sizeof(TAS5707_Dump)/sizeof(TAS5707_DUMP)) ;++index){
        ...
        if(strcmp(TAS5707_Dump[index].reg_name, "ERROR_STATUS") == 0) {
            get_error_status_data = data[0];
            AMP_WARN("get_error_status_data = %x, data[0] = %x\n", get_error_status_data, data[0]);
        }
...
int check_amp_error_handler(int amp_i2c_id, unsigned short slave_addr)
{
    unsigned char send_data[2] ={0};
    char *buf_info = NULL;
    ...
    send_data[0] = TAS5707_ERROR_STATUS;
    send_data[1] = 0;
    tas5707_dump_all(buf_info, amp_i2c_id, slave_addr);
    AMP_WARN("%s get_error_status_data:%x (amp_i2c_id=%x, slave_addr=%x)", __func__, get_error_status_data, amp_i2c_id, slave_addr);

    if(get_error_status_data != 0) {
        ...
        tas5707_param_set(send_data, amp_i2c_id, slave_addr);
        rtk_gpio_output(AMP_RESET_pin, 0);
        mdelay(50);
        rtk_gpio_output(AMP_RESET_pin, 1);
        if (rtk_gpio_output_get(AMP_RESET_pin) == 1) {
            mdelay(50);
            // resend init data.
            tas5707_func(amp_i2c_id, slave_addr);
        }
    }
    return 0;
}
