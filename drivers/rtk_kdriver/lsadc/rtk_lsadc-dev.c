...
int __init rtk_lsadc_dev_init(void)
{
    ...
    if (request_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_CEC), rtk_lsadc_interrupt_handler,IRQF_SHARED, "RTK_LSADC", (void *)~MAX_LSADC_COUNT) < 0)
    {
        LSADC_ERROR("rtk_gpio_init failed, request misc_iso irq %d failed\n",IRQ_CEC);
        ret = -EIO;
        goto FAILD_REQUEST_IRQ_FAILED;
    }

    LSADC_INFO( "[%s] LSADC device init ok !\n", __func__);

    return 0;
FAILD_REQUEST_IRQ_FAILED:
...
static void __exit rtk_lsadc_dev_uninit(void)
{
    ...
    class_destroy(_gstlsadc_class);
    platform_driver_unregister(&rtk_lsadc_driver);
    free_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_CEC),rtk_lsadc_interrupt_handler);
    LSADC_ClockEnable(0);
}
