...
void drvif_color_DCC_Curve_Blending(unsigned char channel,unsigned char bin,unsigned int *Adavance_Control_Curve_Blending_info,unsigned int *Adavance_Control_info)
{
	...
	static unsigned char SC_Times = 0;
	...
	if((Adavance_Control_Curve_Blending_info[DCC_mean_diff]>=3/*5*/) && (Adavance_Control_Curve_Blending_info[DCC_Dev_diff]>=1 /*4*/)){
...
//=======DCC adjust by mean value =========
 		...
 		if (display == SLR_MAIN_DISPLAY)  //main channel init
		{
			...
			if(MainDCC_SC)				
				printk("SceneChange=%d,dcc SC=%d,sc by diff=%d \n",(RPC_SmartPic_clue->SceneChange),(system_info_structure_table->DCC_Status_Change),(SmartPic_clue->SceneChange_by_diff_mean_Dev==1));
