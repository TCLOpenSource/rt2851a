	...
	/*if dumplocation=scaler output and video path has do overscan,
		...
		if((get_VT_Pixel_Format() == VT_CAP_NV12) || (get_VT_Pixel_Format() == VT_CAP_NV16))
		{
			...
			|| (srctype == VSC_INPUTSRC_ADC) || (srctype == VSC_INPUTSRC_AVD))
			{
				...
				{
					rtd_printk(KERN_NOTICE, TAG_NAME, "drvif_i3ddma_NN_SE_proc result fail\n");
					return FALSE;
				}
			...
			else if((srctype == VSC_INPUTSRC_VDEC) || (srctype == VSC_INPUTSRC_JPEG))
			{
				...
				{
					rtd_printk(KERN_NOTICE, TAG_NAME, "drvif_vdec_NN_SE_proc result fail\n");
					return FALSE;
				}
...
void set_vfod_freezed_for_dc2h(unsigned char boffon)
{
	...
	if(get_vt_VtBufferNum() == 1) 
	{
		...
		if(boffon == TRUE) //vidioc_dqbuf  
		{
			...
			if(timeoutcount == 0)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME, "fun:%s=%d, freeze action timeout\n", __FUNCTION__, __LINE__);
			}		
		}
		else
		{
			...
			{
				rtd_printk(KERN_NOTICE, TAG_NAME, "fun:%s=%d, unfreeze index error\n", __FUNCTION__, __LINE__);
				return;
			}
			...
			if(timeoutcount == 0)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME, "fun:%s=%d, unfreeze action timeout\n", __FUNCTION__, __LINE__);
			}					
		}
...
void sub_mdomain_seq_VtCap_freeze(unsigned char enable)
{
	...
	if(TRUE == enable)
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME,"MDOMAIN_CAP_DDR_In2Status_reg = %x\n", IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg));
		...
		rtd_printk(KERN_NOTICE, TAG_NAME,"MDOMAIN_CAP_DDR_In2Addr_reg = %x\n", IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg));
		rtd_printk(KERN_NOTICE, TAG_NAME,"MDOMAIN_CAP_DDR_In2_2ndAddr_reg = %x\n", IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg));
		rtd_printk(KERN_NOTICE, TAG_NAME,"MDOMAIN_CAP_DDR_In2_3rdAddr_reg = %x\n", IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg));	
	}
	else
		VtFreezeRegAddr = 0;	
}
...
void set_vfod_freezed_for_i3ddma_sub_capture(unsigned char boffon)
{
	rtd_printk(KERN_NOTICE, TAG_NAME,"func:%s,%d,%d\n",__FUNCTION__,get_VT_Pixel_Format(), boffon);
...
unsigned char Capture_BufferMemInit_VT(unsigned int buffernum) 
{
	...
	for (i = 0; i < buffernum; i++)
	{
		if(CaptureCtrl_VT.cap_buffer[i].phyaddr == 0)
		{
			...
			{				
				rtd_printk(KERN_EMERG, TAG_NAME,"[error]VT retry pli malloc fail(%lx)\n", mallocphysicaladdr);
				return FALSE;
			}
			else
	...
	//seperate allocate memory 
	for (i = 0; i < buffernum; i++)
	{
		if(CaptureCtrl_VT.cap_buffer[i].cache == NULL){
			malloccacheaddr = (unsigned long)dvr_malloc_specific(size, GFP_DCU2_FIRST);
			if((unsigned long)NULL == malloccacheaddr)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME,"error:alloc VT memory failed[non-dc2h path]\n");
				return FALSE;
			}
			
			CaptureCtrl_VT.cap_buffer[i].cache = (void*)(malloccacheaddr);
...
void Capture_BufferMemDeInit_VT(unsigned int buffernum) 
{
	...
	if(VtBlockCapFlag == TRUE)
	{
		if (CaptureCtrl_VT.cap_buffer[0].cache)
		{
			...
			rtd_printk(KERN_NOTICE, TAG_NAME,"Block capture free memory done!\n");
...
unsigned int memory_get_line_size_subvt(unsigned int LineSize, unsigned char InputFormat, unsigned char BitNum)
{
	...
	pr_notice("[VT]memory_get_line_size LineSize=%d\n",LineSize);
	return LineSize;
}
		...
		//if (SCALING_LOG) {
			//for(i=0; i<8; i++) {
				//if((*(coef_pt+i)+*(coef_pt+i+16)+*(coef_pt+31-i)+*(coef_pt+15-i)) != 1024)
					//rtd_printk(KERN_NOTICE, TAG_NAME,"[Warnning] [%s-%s-%d] DC Not Balance!, i=%d\n", __FILE__, __func__, __LINE__, i);
			//}
		//}
...
/*o============V scale-down=============o*/
	if (ptInSize->nLength > ptOutSize->nLength) {
		...
		rtd_printk(KERN_NOTICE, TAG_NAME,"filter number =%d\n", SDFilter);
...
unsigned char force_enable_vtdatapath_clock(void)
{
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME, "VT[warning] NO DI source, Capture will fail!;%s=%d \n", __FUNCTION__, __LINE__);
		return FALSE;
	}	
...
void drvif_vt_check_subvgip_data_dclk(unsigned char enable)
{
	...
	if(enable == TRUE)
	{
		...
		mdelay(2);
		rtd_printk(KERN_NOTICE, TAG_NAME, "VT Enable Sub VGIP Data.Dclk!;%s=%d \n", __FUNCTION__, __LINE__);
	} else {
		vgip_chn2_ctrl_reg.ch2_ivrun = 0;
		vgip_chn2_ctrl_reg.ch2_in_clk_en= 0;
		IoReg_Write32(SUB_VGIP_VGIP_CHN2_CTRL_reg, vgip_chn2_ctrl_reg.regValue);
		mdelay(2);
		rtd_printk(KERN_NOTICE, TAG_NAME, "VT Disable Sub VGIP Data.Dclk!;%s=%d \n", __FUNCTION__, __LINE__);
...
void config_sub_vgip_for_Vt_capture(void)
{
	...
	{	
		...
		IoReg_Write32(VGIP_Data_Path_Select_reg, data_path_select_reg.regValue);
		IoReg_Write32(SUB_VGIP_VGIP_CHN2_CTRL_reg, vgip_chn2_ctrl_reg.regValue);
		rtd_printk(KERN_NOTICE, TAG_NAME,"go rtnr/di:SUB_VGIP_VGIP_CHN2_CTRL_reg = %x\n", IoReg_Read32(SUB_VGIP_VGIP_CHN2_CTRL_reg));	
	}	
	else if(data_path_select_reg.uzd1_in_sel == 0)
	{
		...
		IoReg_Write32(SUB_VGIP_VGIP_CHN2_CTRL_reg, vgip_chn2_ctrl_reg.regValue);
		mdelay(2);
		rtd_printk(KERN_NOTICE, TAG_NAME,"not go rtnr/di:SUB_VGIP_VGIP_CHN2_CTRL_reg = %x\n", IoReg_Read32(SUB_VGIP_VGIP_CHN2_CTRL_reg));
	}
...
//	unsigned int buffer_size = 0;
	...
	IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, CaptureCtrl_VT.cap_buffer[0].phyaddr & 0x7ffffff0);
	pr_notice("[VT]###################sub buffer TotalSize=%d\n", TotalSize);
...
//	h3ddma_i3ddma_enable_RBUS i3ddma_enable_reg;
	...
	rtd_printk(KERN_NOTICE, TAG_NAME,"H3DDMA_CAP0_Cap_CTL0_reg = %x\n", IoReg_Read32(H3DDMA_CAP0_Cap_CTL0_reg));
...
void i3ddma_cap0_seq_VtCap_freeze(unsigned char enable)
{
	if(TRUE == enable)
	{
		if(get_vt_VtBufferNum() == 3)
		{
			...
			rtd_printk(KERN_NOTICE, TAG_NAME,"cap0 freeze done!\n");
			...
			rtd_printk(KERN_NOTICE, TAG_NAME,"cap0 freezed buffer Idx = %d\n", IndexOfFreezedVideoFrameBuffer);
			rtd_printk(KERN_NOTICE, TAG_NAME,"H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_0_reg = %x\n", IoReg_Read32(H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_0_reg));
			rtd_printk(KERN_NOTICE, TAG_NAME,"H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_1_reg = %x\n", IoReg_Read32(H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_1_reg));
			rtd_printk(KERN_NOTICE, TAG_NAME,"H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_2_reg = %x\n", IoReg_Read32(H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_2_reg));
		}
		else if(get_vt_VtBufferNum() == 1)
		{
			h3ddma_cap0_cap_status_RBUS h3ddma_cap0_cap_status_Reg;
			...
			if(timeoutcount == 0)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME, "wait cap0_cap_last_wr_flag timeout\n");
			}
		}
	}
	else
...
void i3ddma_cap1cap2_vi_VtCap_freeze(unsigned char enable)
{
	rtd_printk(KERN_NOTICE, TAG_NAME,"func:%s,%d\n",__FUNCTION__,enable);
	if(TRUE == enable)
	{ //calculate last capture done buffer,which send to ap
		if(get_vt_VtBufferNum() == 3)
		{
...
//disable dma
	...
	if(CaptureCtrl_VT.cap_buffer[0].phyaddr != 0)
	{
		pr_notice("[VT] free memory when nv12 capture\n");
	...
	rtd_printk(KERN_NOTICE, TAG_NAME,"nv12 capture output size(%d,%d),datafmt:%d\n",nWidth, nLength, data_format);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME,"nv12 index=0,uv phy addr=0x%x,%x,%x\n", ucBufAddr, uvIdx1, uvIdx2);
...
void I3DDMA_color_colorspacergb2yuvtransfer_VtCapture(VT_DISPD_I3DDMACAP_SRC idmaDispD_in_sel)
{
	...
	rtd_printk(KERN_NOTICE, TAG_NAME,"[VT] en_rgb2yuv=%d\n", i3ddma_rgb2yuv_ctrl_reg.en_rgb2yuv);
...
unsigned char is_yuv_cap_in_non_full_screen(VT_DISPD_I3DDMACAP_SRC position)
{
	...
	if((get_VT_Pixel_Format() == VT_CAP_NV12) || (get_VT_Pixel_Format() == VT_CAP_NV16))
	{
		...
		{
			rtd_printk(KERN_NOTICE, TAG_NAME,"is_yuv_cap_in_non_full_screen\n");
			return TRUE;
		}
	}
	return FALSE;
...
unsigned char drvif_I3DDMA_dispD_VtCaptureConfig(VT_CUR_CAPTURE_INFO capInfo, unsigned char waitbufferfresh)
{
	...
	if(capInfo.enable == TRUE) {
		...
		rtd_printk(KERN_NOTICE, TAG_NAME, "drvif_I3DDMA_dispD_VtCaptureConfig dispD_sel:%d;%s=%d \n",dispD_sel, __FUNCTION__, __LINE__);	
		ppoverlay_dh_den_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_DH_DEN_Start_End_reg);
		ppoverlay_dv_den_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_DV_DEN_Start_End_reg);
		main_active_h_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Active_H_Start_End_reg);
		main_active_v_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Active_V_Start_End_reg);
		...
		rtd_printk(KERN_NOTICE, TAG_NAME, "drvif_I3DDMA_dispD_VtCaptureConfig outSize.nWidth:%d; outSize.nLength = %d;inSize.nWidth=%d;inSize.nLength=%d \n",outSize.nWidth,outSize.nLength,inSize.nWidth,inSize.nLength);
		...
		rtd_printk(KERN_NOTICE, TAG_NAME,"result:PPOVERLAY_Display_Timing_CTRL2_reg = %x\n", IoReg_Read32(PPOVERLAY_Display_Timing_CTRL2_reg));
		disable_i3_comp_en();
...
#include <include/rtk_kdriver/tvscalercontrol/scalerdrv/scaler_hdr_ctrl.h>
...
unsigned char get_i3ddma_idlestatus(void)
{
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME, "VT[info] I3DDMA is used by I3ddma video source,please use sub capture!;%s=%d \n", __FUNCTION__, __LINE__);
		return FALSE;
	}
#endif
	rtd_printk(KERN_NOTICE, TAG_NAME, "VT[info] I3DDMA is idle use i3ddma capture!;%s=%d \n", __FUNCTION__, __LINE__);
	return TRUE;
}
...
unsigned char HAL_VT_InitEx(unsigned int buffernum)
{
	...
	{
		rtd_printk(KERN_EMERG, TAG_NAME, "max support 5 buffer mode\n");
		up(&VT_Semaphore);
		return FALSE;
	}
	...
	{
		rtd_printk(KERN_EMERG, TAG_NAME, "[Warning]D domain clock is off status,could not do vt capture!\n");
		up(&VT_Semaphore);
		return FALSE;	
	}
...
unsigned char HAL_VT_GetVideoFrameBufferIndex(KADP_VT_VIDEO_WINDOW_TYPE_T videoWindowID, unsigned int *pIndexOfCurrentVideoFrameBuffer)
{
	...
	if(get_vt_function() == TRUE) 
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME,"VFODState.bFreezed = %d\n", VFODState.bFreezed);
		if(VFODState.bFreezed == TRUE) 
		{
			*pIndexOfCurrentVideoFrameBuffer = IndexOfFreezedVideoFrameBuffer;
			rtd_printk(KERN_NOTICE, TAG_NAME,"VFOD is freezed IndexOfFreezedVideoFrameBuffer = %d\n", IndexOfFreezedVideoFrameBuffer);
		}
...
unsigned char HAL_VT_SetVideoFrameOutputDeviceOutputRegion(KADP_VT_VIDEO_WINDOW_TYPE_T videoWindowID, KADP_VT_DUMP_LOCATION_TYPE_T dumpLocation, KADP_VT_RECT_T *pOutputRegion)
{
	...
	if(get_vt_function() == TRUE) {
		...
		else
		{
			if(get_VT_Pixel_Format() != VT_CAP_NV12)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME, "not nv12,sub only support nv12,return false,%s\n", __FUNCTION__);
				up(&VT_Semaphore);
				return FALSE;
			}
			drvif_sub_source_VtCaptureConfig(curCapInfo,TRUE);
...
unsigned char HAL_VT_set_Pixel_Format(VT_CAP_FMT value)
{
	...
	if((value >= VT_CAP_RGB888)&&(value <= VT_CAP_NV16)) {
		if (value != get_VT_Pixel_Format()) {
			...
			rtd_printk(KERN_NOTICE, TAG_NAME, "%s=%d,fmt=%d\n", __FUNCTION__, __LINE__, value);
...
static int vivt_suspend (struct device *p_dev)
{
	...
	if(HAL_VT_Finalize()==TRUE)
		rtd_printk(KERN_EMERG, TAG_NAME, "%s=%d  (HAL_VT_Finalize()==TRUE)\n", __FUNCTION__, __LINE__);
	else
		rtd_printk(KERN_EMERG, TAG_NAME, "%s=%d  (HAL_VT_Finalize()==FALSE)\n", __FUNCTION__, __LINE__);
#if 0
