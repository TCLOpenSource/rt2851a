...
#include "tvscalercontrol/util/rtk_util_isr_wait.h"
...
extern int tconless_set_gamma_bin(KADP_VBE_DISP_TCON_GAMMA_BIN_DATA_T data);
...
int vbe_disp_resume_is_done = 0;
int tconless_cs602_cs603_reset_is_done = 0;
...
*/
	...
	pr_notice("[%s] mode=%d. (goldenv: %x, uzu_ctrl1:%x)\n", __FUNCTION__, mode,
    ...
    , // code blk if loop count timed out
        {
            ...
            rtd_printk(KERN_ERR, TAG_NAME_VBE, "dispd_smooth_toggle1 loopCnt timeout !!!");
...
#if 0  // Discussed w/ Frank and Ben, we don't know the history but it seems no need anymore. Remove to save unmute time but keep the codes here. I put the measurement info here.
    ...
    if(0 == modestate_set_fractional_framesyncByDisp(_channel, false)) {
                ...
                <= (PPOVERLAY_MAIN_DEN_V_Start_End_get_mv_den_end(IoReg_Read32(PPOVERLAY_MAIN_DEN_V_Start_End_reg)) - 300)))                 // and there is enough time to set franctional
        {
            ...
            if(eRUTIL_ISR_WAIT_ISR_NOT_FINISH_CANCELED ==
                ...
                rtd_printk(KERN_EMERG, TAG_NAME_VBE, "input vfreq has changed when setting franctional, return!!!\n");
...
#if 0
    rtd_printk(KERN_INFO, TAG_NAME_VBE, "Get UZU vfreq: %d * %d / %d   * %d / %d = %d",
...
*/
	...
	//		vbe_dtg_memec_bypass_switch_set(0);
			...
			if(drvif_Hdmi_GetVRREnable() ||vbe_disp_get_freesync_mode_flag()){
				//disable MEMC clock
				Scaler_MEMC_MEMC_CLK_OnOff(0, 0, 1);
				Scaler_MEMC_Bypass_On(1);
	...
	//		vbe_dtg_memec_bypass_switch_set(1);
		...
		}else if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdmi_4k_hfr_mode() != HDMI_NON_4K120))
		{
			...
			else if(drvif_Hdmi_GetVRREnable() ||vbe_disp_get_freesync_mode_flag()){	//Freesync need go low delay mode, bypass memc
				//disable MEMC clock
				Scaler_MEMC_MEMC_CLK_OnOff(0, 0, 1);
				Scaler_MEMC_Bypass_On(1);
...
unsigned int scaler_disp_smooth_get_memc_protect_vtotal_By_100(void)
{
	...
	{
		// unsupport input frame rate, do error handle.
		vertical_total = Get_DISP_VERTICAL_TOTAL()*100;
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s][ERR] invalid vfreq(%d), protect vtotal=%d\n", __FUNCTION__, vFreq, vertical_total);
	}	
...
void vbe_disp_panel_resume(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_CMPI))
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] EPI and tcon resume finished\n",__FUNCTION__);
	}
	else if(Get_DISPLAY_PANEL_TYPE() == P_ISP)
	{
...
void vbe_disp_suspend(void)
{
	...
	vbe_disp_resume_is_done = 0;
	tconless_cs602_cs603_reset_is_done = 0;
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] vbe_disp_resume_is_done = %d tconless_cs602_cs603_reset_is_done = %d \n",__FUNCTION__,vbe_disp_resume_is_done,tconless_cs602_cs603_reset_is_done);
	Panel_TurnOff();
}
...
{
    rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "####[%s] thread start...####\n",__FUNCTION__);
    while (!kthread_should_stop() && vbe_panel_tsk_running_flag == TRUE)
    {
        if(down_interruptible(&panel_on_flow_sema))
            rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "interrupted, no semaphore held\n");
        //vbe_panel_tsk_running_flag = FALSE;
        Panel_resume_TurnOn();
        //msleep(10); // sleep 10 ms
    }
    rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "####[%s] thread end...####\n",__FUNCTION__);
    vbe_panel_tsk_running_flag = FALSE;
    do_exit(0);
    return 0;
}
...
{
    int err;
    rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "####vbe_panel_tsk_running_flag=%d...####\n",vbe_panel_tsk_running_flag);
    if (vbe_panel_tsk_running_flag == FALSE) {
        ...
        else
        {
            ...
            rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "Unable to start vbe_resume_panel_tsk (err_id = %d)./n", err);
        }
    }
    else
...
//	UINT32 cnt=0;
	...
	if(vbe_disp_resume_is_done == 0)
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] vbe_disp_resume_is_done = %d \n",__FUNCTION__,vbe_disp_resume_is_done);
		vbe_disp_resume_is_done = 1;
	}
	else
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] vbe_disp_resume_is_done = %d \n",__FUNCTION__,vbe_disp_resume_is_done);
...
void HAL_VBE_DISP_Set_Panel_Ti_Mode(UINT8 table)
{
        ...
        if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K || Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)
        {
                ...
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"do not set Ti Mode!\n");
        up(&VBE_Semaphore);
}
...
void HAL_VBE_DISP_Set_Panel_Bit_Mode(UINT8 bitMode)
{
        ...
        if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K || Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)
        {
                ...
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"do not set Bit Mode!\n");
        up(&VBE_Semaphore);
}
...
void HAL_VBE_DISP_Set_Panel_Swap_Mode(UINT8 bSwap)
{
        ...
        if(bSwap > 1){
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "set Swapmode = %d error,only apply 0;1\n",bSwap);
                return;
        }
        ...
        if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K || Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)
        {
                ...
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"do not swap AB!\n");
        up(&VBE_Semaphore);
...
void HAL_VBE_DISP_Set_Panel_SFG_Dual_Mode(UINT8 dualMode)
{
        ...
        if(Get_DISPLAY_PANEL_TYPE() != P_VBY1_4K2K){
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "only run at vby1 panel\n");
                return;
        }
        if(dualMode > 1){
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "set dualMode = %d error,only apply 0;1\n",dualMode);
                return;
        }
...
void HAL_VBE_DISP_Get_Panel_SFG_Dual_Mode(UINT8* dualMode)
{
        ...
        *dualMode = sfg_ctrl_0_reg.seg_num;
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "Get dualMode %d\n",*dualMode);
        up(&VBE_Semaphore);
}
...
void HAL_VBE_DISP_Get_Panel_Ti_Mode(UINT8* table)
{
        ...
        *table = lvds_ctrl3_reg.lvds_map;
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "Get tiMode %d\n",*table);
        up(&VBE_Semaphore);
}
...
void HAL_VBE_DISP_Get_Panel_Bit_Mode(UINT8* bitMode)
{
        ...
        *bitMode = sfg_zz_shift_dbg_ctrl_reg.dout_mode;
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "Get bitMode %d\n",*bitMode);
        up(&VBE_Semaphore);
}
...
void vbe_resume_Backlight_TurnOn(struct device *p_dev)
{
	...
	if(Panel_Get_STR_BackLight_Ctrl_Mode() == _BACKLIGHT_STR_CTRL_MODE_BY_KERNEL){
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] STR BACKLIGHT CTRL MODE_BY_AP, no need turn on back light\n",__FUNCTION__);
	}
	else{
		Panel_SetBackLightMode(1);
...
{
	return drvif_Hdmi_GetVRREnable();
}
	...
	switch (cmd)
	{
				...
				*/
				if(panel_set.ctrl_set == 0)
				{
					if (panel_set.power_enable == false)
					{
						...
						{	
							rtd_printk(KERN_NOTICE, TAG_NAME_VBE, " Trun_off stv pin J5 \n");
							rtd_maskl(0xb80008a8,0xffffff0f,0x000000f0); // J5 for SCLK(STV), use GPIO,
						}
                    }
                    else
                    {
							...
							((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
						{
							...
							rtd_printk(KERN_NOTICE, TAG_NAME_VBE, " Trun_on stv pin of Listen_alone \n");
						}
        ...
        case VBE_IOC_SET_SGF_DUAL_MODE:
        {
                ...
                if(copy_from_user((void *)&dualMode, (const void __user *)arg, sizeof(UINT8))) {
                        retval = -EFAULT;
                        DBG_PRINT("scaler vbe ioctl code=VBE_IOC_SET_SGF_DUAL_MODE failed!!!!!!!!!!!!!!!\n");
                } else {
        ...
        case VBE_IOC_GET_PANEL_TI_MODE:
        {
                ...
                {
                        retval = -EFAULT;
                        DBG_PRINT("scaler vbe ioctl code=VBE_IOC_GET_PANEL_TI_MODE failed!!!!!!!!!!!!!!!\n");
                }
                break;
        }
        case VBE_IOC_GET_PANEL_BIT_MODE:
        {
                ...
                {
                        retval = -EFAULT;
                        DBG_PRINT("scaler vbe ioctl code=VBE_IOC_GET_PANEL_BIT_MODE failed!!!!!!!!!!!!!!!\n");
                }
                break;
        }
        case VBE_IOC_GET_SGF_DUAL_MODE:
        {
                ...
                {
                        retval = -EFAULT;
                        DBG_PRINT("scaler vbe ioctl code=VBE_IOC_GET_SGF_DUAL_MODE failed!!!!!!!!!!!!!!!\n");
                }
                break;
        }
		case VBE_IOC_PANEL_INTERNET_WAKEUP:
		{
			unsigned int value;
			if(copy_from_user((void *)&value, (const void __user *)arg, sizeof(unsigned int))) {
				retval = -EFAULT;
				DBG_PRINT("scaler vbe ioctl VBE_IOC_PANEL_INTERNET_WAKEUP failed!!!!!!!!!!!!!!!\n");
			} else {
			    rtd_printk(KERN_DEBUG, TAG_NAME_VBE,"Ap set panel on, value=%d\n", value);
				Panel_TurnOn_By_KernelAp(value);
			}
			break;
		}
        case VBE_IOC_DISP_TCON_SET_GAMMA_BIN:
        {
            KADP_VBE_DISP_TCON_GAMMA_BIN_DATA_T data_param;
            unsigned char* tmp_p_gamma_data = NULL;
            unsigned char* tmp_pmu_data = NULL;
            //unsigned char bin_data_cs603[48];
            //int i =0;
            if(copy_from_user((void *)&data_param, (const void __user *)arg, sizeof(KADP_VBE_DISP_TCON_GAMMA_BIN_DATA_T))) {
                retval = -EFAULT;
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] VBE_IOC_DISP_TCON_SET_GAMMA_BIN failed\n",__FUNCTION__);
            }else {
                rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "iCONFIG_PANEL_CUSTOM_INDEX=%d \n",data_param.iCONFIG_PANEL_CUSTOM_INDEX);
                if(data_param.iCONFIG_PANEL_CUSTOM_INDEX > 255){
                    rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "iCONFIG_PANEL_CUSTOM_INDEX is not right! \n");
                    retval = -EFAULT;
                }else{
                    rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] enter VBE_IOC_DISP_TCON_SET_GAMMA_BIN \n",__FUNCTION__);
                    tmp_p_gamma_data = memdup_user(data_param.tcon_p_gamma_data ,data_param.tcon_p_gamma_data_len);
                    tmp_pmu_data = memdup_user(data_param.tcon_pmu_data ,data_param.tcon_pmu_data_len);
                    //for debug
                    /*bin_data_cs603[0] = 0x00;
                    for( i=0 ;i<43 ;i++)
                        bin_data_cs603[i+1] = *tmp_pmu_data++;
                    for( i=0; i<43; i++)
                        rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "LX pmu_bin [%x]\n",bin_data_cs603[i]);*/
                    data_param.tcon_pmu_data = tmp_pmu_data;
                    data_param.tcon_p_gamma_data = tmp_p_gamma_data;
                    rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "tcon_pmu_data_len=%d ,tcon_p_gamma_data_len=%d  \n",data_param.tcon_pmu_data_len,data_param.tcon_p_gamma_data_len);
                    if(tconless_set_gamma_bin(data_param) != 0){
                        rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "tconless_set_gamma_bin fail! \n");
                        retval = -EFAULT;
                    }
                    if(tmp_p_gamma_data){
                        kfree(tmp_p_gamma_data);
                        tmp_p_gamma_data = NULL;
                    }
                    if(tmp_pmu_data){
                        kfree(tmp_pmu_data);
                        tmp_pmu_data = NULL;
...
RUTIL_ISR_WAIT_EXPORT(ISR_WAIT_MV_DEN_STA);
RUTIL_ISR_WAIT_EXPORT(ISR_WAIT_MV_DEN_END);
RUTIL_ISR_WAIT_EXPORT(ISR_WAIT_MEMCDTG_DEN_STA);
RUTIL_ISR_WAIT_EXPORT(ISR_WAIT_VGIP_VACT_STA);
...
if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE==14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX==229)){
	tconless_CS602_cs603_reserve();
}
