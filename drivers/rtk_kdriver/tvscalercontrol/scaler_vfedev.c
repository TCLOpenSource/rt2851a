			...
			if (AVD_Global_Status == SRC_CONNECT_DONE) {
				...
				if (0 == ret){
					if (copy_to_user((int __user *)arg, (const void * )ptTimingInfo, sizeof(KADP_VFE_AVD_TIMING_INFO_T)) ){
						rtd_printk(KERN_ERR, TAG_NAME, "#####[%s(%d)] VFE_IOC_AVD_GET_TIMINGINFO fail\n",__func__,__LINE__);
						ret = -1;
					}else{
		...
		case VFE_IOC_GET_AV_OFFLINE_SIGTATUS:
			{
				...
				{
					rtd_printk(KERN_DEBUG, TAG_NAME, "scaler vbe ioctl code=VFE_IOC_GET_AV_OFFLINE_SIGTATUS fail!!!!!!!!!\n");
					ret = -1;
				}else
					ret = 0;
				break;
