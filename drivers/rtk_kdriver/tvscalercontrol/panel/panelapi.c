...
#include <linux/i2c.h>
#include <common/include/ioctrl/scaler/vbe_cmd_id.h>
...
unsigned char CS602_Data_BankA_75D03_2[49] =
{
	0x00, 0x48, 0x1A, 0xA4, 0x00, 0x2B, 0x23, 0xFC, 0x66, 0x7B, 0x33, 0x31, 0x00, 0x14, 0x14, 0x0B, 0x0B,
	0x3E, 0x63, 0xE2, 0x35, 0x52, 0xE4, 0x2A, 0x52, 0x58, 0x21, 0x81, 0xC1, 0x18, 0x91, 0x4F, 0x10,
	0x30, 0x9E, 0x01, 0x70, 0x0E, 0x1A, 0x52, 0x0A, 0x5E, 0xCA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

#define CS603_DEV_ADDR         0x36	//  ic slave address
#define CS603_DATA_NUM          42
unsigned char cs603_reg[49];
	
int cs602_i2c_port = 1;
extern int tconless_cs602_cs603_reset_is_done; 
	
unsigned char CS603_Data_BankA_55inch_D12_4[49] =
{
	0x00, 0x48, 0x09, 0xA4, 0x00, 0x23, 0xAA, 0xF8, 0x66, 0xF3, 0x0B, 0x88, 0x05, 0x10, 0x07, 0x01, 0xA1,
	0x3E, 0x63, 0xCA, 0x32, 0x42, 0xCC, 0x29, 0xE2, 0x34, 0x21, 0x81, 0xBD, 0x1A, 0x11, 0x33, 0x0F,
	0xF0, 0xA3, 0x01, 0xC0, 0x0D, 0x1A, 0x11, 0x9C, 0x88, 0xB7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

unsigned char CS603_Data_BankA_65inch_D06_3[49] =
{
	0x00, 0x48, 0x09, 0x92, 0x00, 0x23, 0xAA, 0xfc, 0x76, 0xF3, 0x14, 0x92, 0x0f, 0x14, 0x0B, 0x02, 0x31,
	0x3F, 0x83, 0xF2, 0x36, 0x42, 0xf1, 0x2B, 0x02, 0x66, 0x22, 0x41, 0xcB, 0x19, 0xB1, 0x58, 0x10,
	0xF0, 0xA6, 0x01, 0xD0, 0x0F, 0x1A, 0x31, 0xA3, 0xB0, 0xF9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

//RT6966
#define RT6966_PMIC_ADDR        		0x36	//  7bit slave addr is 0x36
#define RT6966_LS_ADDR        		 	0x26	//  7bit slave addr is 0x26
#define RT6966_BACKA_CTRL_ADDR        	0x00	// control register address
#define RT6966_PMIC_DATA_NUM          	40	
#define RT6966_LS_DATA_NUM          	14
unsigned char RT6966_PMIC_reg[41];
unsigned char RT6966_LS_reg[15];

unsigned char RT6966_Data_PMIC_50inch_U2[41] =
{
   0x00, 0x00, 0x00, 0x00, 0x00, 0x27, 0xab, 0xfc, 0x34, 0xf7, 0x0e, 0x0a, 0x03, 0x14, 0x0b, 0x02, 0x2c,
   0x3e, 0x13, 0xd6, 0x34, 0xe3, 0x01, 0x2d, 0xd2, 0x76, 0x25, 0xf1, 0xf8, 0x1e, 0x11, 0x77, 0x14,
   0x50, 0xe6, 0x03, 0x20, 0x21, 0x1c, 0x71, 0xc7, 
};
unsigned char RT6966_Data_LevelShift_50inch_U2[15] =
{
   0x00, 0x00, 0x40, 0x5e, 0x40, 0x5e, 0x40, 0x5e, 0x08, 0x03, 0x00, 0x00, 0x00, 0x27, 0x01,	
};

//SM4810
#define SM4810_PMIC_ADDR        		0x18	//  7bit slave addr is 0x36
#define SM4810_LS_ADDR        		 	0x1A	//  7bit slave addr is 0x26
#define SM4810_BACKA_CTRL_ADDR        	0x00	// control register address
#define SM4810_PMIC_DATA_NUM          	39	
#define SM4810_LS_DATA_NUM          	14
unsigned char SM4810_PMIC_reg[41];
unsigned char SM4810_LS_reg[16];

unsigned char SM4810_data_LS_43inch_DJ1[15] =
{
	0x84, 0x40, 0x51, 0xcc, 0x7e, 0xeb, 0x7f, 0x55, 0xfC, 0x35, 0x4C, 0xc8, 0x03, 0x57, 0x01,
};
unsigned char SM4810_data_PMIC_43inch_DJ1[40] =
{
	0x92, 0x96, 0x71, 0x64, 0xDA, 0xa7, 0xC9, 0x3F, 0xf4, 0xc6, 0xaf, 0x8C, 0x77, 0x52, 0x38, 0x05,
	0x37, 0x5a, 0x7c, 0x3e, 0x25, 0x00, 0xb3, 0x00, 0x25, 0xb0, 0x53, 0xE3, 0x01, 0x22, 0x63, 0x03,
	0x8F, 0xE3, 0x71, 0xbb, 0xF9, 0x7F, 0xCC, 0xF3,
};

// GMT IC G2532
#define G2532_DEV_ADDR				0x20	//  ic slave address
#define G2532_BACKA_CTRL_ADDR		0x00	// control register address
#define G2532_BACKB_CTRL_ADDR		0x30	// control register address
#define G2532_DATA_NUM				66	

//---PM ic is GMT G2510S------------
#define G2510S_DEV_ADDR         0x70	// pmu ic slave address
#define G2510S_CTRL_ADDR        0x00	// control register address
#define G2510S_DATA_ADDR        0x10	// pum data start
#define G2510S_DATA_NUM         25		// total data number


//---gamma ic is GMT G1621---------
#define G1621_DEV_ADDR              0x74	// gamma ic slave address
#define G1621_CTRL_ADDR			    0x00	// control sub address
#define G1621_VCOM_ADDR		        0x01	// vcom voltage

#define G1621_DATA_NUM          24		// total data number(1 vcom + 24 other)
...
{
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]\n",__FUNCTION__);
...
void cs602_i2c_port_check(void)
{
	...
	for(i=0;i<10;i++)
	{
		...
		if(ret < 0)
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] cs602_i2c_port is wrong  i2c-port= %d\n",__FUNCTION__,cs602_i2c_port);
				if(cs602_i2c_port == 1)
					cs602_i2c_port = 2;
				else
					cs602_i2c_port = 1;
				...
				if(ret < 0)
				{
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] cs 602_i2c_port is wrong after change the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
				}
				else
				{
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]cs602_i2c_port is right after change the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
					msleep(5);
					return; 
				}
			}
			...
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]cs 602_i2c_port is right i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
				msleep(5);
				return; 
			}
			...
			if(cs602_i2c_port == 1)
				...
				cs602_i2c_port = 1;
			msleep(10);
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]cs602_i2c_port_check %d time\n",__FUNCTION__,i+1);
			if(i== 2)
			{	
				cs602_i2c_port = 1;
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]cs602_i2c_port is wrong after check 3 times the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
				return; 
			}
...
{
	int ret,i,sum = 0;
	unsigned char temp_cs603[48];
	unsigned char temp_cs602[48];
	unsigned char temp_RT6966_PMIC[48];
	unsigned char temp_RT6966_LS[48];
	unsigned char temp_SM4810_PMIC[48];
	unsigned char temp_SM4810_LS[48];

	int sum_pmic = 0;
	int sum_ls = 0;	
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"kernel type = %d , index = %d\n",pPanelCOnfigParameter->iCONFIG_PANEL_TYPE,pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX);
	
	if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))||
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 232)))
	{
		temp_cs603[0] = 0x00;
		for(i=0 ;i<43 ;i++)
		{
			temp_cs603[i+1] = cs603_reg[i];
			sum = sum + cs603_reg[i];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_cs603 sum reg is %d \n",__FUNCTION__,sum);
		
		for(i=0; i<43; i++) //for debug
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless [%x]\n",__FUNCTION__,temp_cs603[i]);
		
		if((sum == 401)||(sum == 0))
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_cs603 reg is all 401 zero\n",__FUNCTION__);
			return -1;
		}
		
		ret = i2c_master_send_ex(1,CS603_DEV_ADDR,&temp_cs603[0],43);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 write fail\n",__FUNCTION__);
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 write ok\n",__FUNCTION__);
			return 1;
		}
	}
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229)))
	{
		temp_cs602[0] = 0x00;
		for(i=0 ;i<43 ;i++)
		{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_cs602 sum reg is %d \n",__FUNCTION__,sum);
		cs602_i2c_port_check();
		if(sum == 0)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_cs602 reg is all zero\n",__FUNCTION__);
			return -1;
		}
		ret = i2c_master_send_ex(cs602_i2c_port,CS602_DEV_ADDR,&temp_cs602[0],43);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 write fail\n",__FUNCTION__);
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 write ok\n",__FUNCTION__);
			return 1;
		}
		for(i=0; i<43; i++) //for debug
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless [%x]\n",__FUNCTION__,temp_cs602[i]);
	}
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
	{
		temp_RT6966_PMIC[0] = 0x00;
		temp_RT6966_LS[0] = 0x00;
		for(i=0 ;i<40 ;i++)
		{
			temp_RT6966_PMIC[i+1] = RT6966_PMIC_reg[i];
			sum_pmic = sum_pmic + RT6966_PMIC_reg[i];
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless pimc 0X[%x]\n",__FUNCTION__, RT6966_PMIC_reg[i]);
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_RT6966 sum reg is %d \n",__FUNCTION__,sum_pmic);

		for(i=0 ;i<14 ;i++)
		{
			temp_RT6966_LS[i+1] = RT6966_LS_reg[i];
			sum_ls = sum_ls + RT6966_LS_reg[i];
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless ls 0X[%x]\n",__FUNCTION__, RT6966_LS_reg[i]);
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_RT6966 sum reg is %d \n",__FUNCTION__,sum_ls);

		if((sum_pmic == 430)||(sum_ls == 51)||(sum_pmic == 0)||(sum_ls == 0))
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_RT6966 reg is all zero\n",__FUNCTION__);
			return -1;
		}

		ret = i2c_master_send_ex(1,RT6966_PMIC_ADDR,&temp_RT6966_PMIC[0],41);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_pmic write fail\n",__FUNCTION__);
			return -1;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_pmic write ok\n",__FUNCTION__);
		}

		msleep(10);
		ret = i2c_master_send_ex(1,RT6966_LS_ADDR,&temp_RT6966_LS[0],15);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_ls write fail\n",__FUNCTION__);
			return -1;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_ls write ok\n",__FUNCTION__);
			return 1;
		}
	}
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 16) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 242)))
	{
		temp_SM4810_PMIC[0] = 0x00;
		temp_SM4810_LS[0] = 0x00;
		for(i=0 ;i<40 ;i++)
		{
			temp_SM4810_PMIC[i+1] = SM4810_PMIC_reg[i];
			sum_pmic = sum_pmic + SM4810_PMIC_reg[i];
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless pimc 0X%x\n",__FUNCTION__, SM4810_PMIC_reg[i]);
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_SM4810 sum reg is %d \n",__FUNCTION__,sum_pmic);

		for(i=0 ;i<15 ;i++)
		{
			temp_SM4810_LS[i+1] = SM4810_LS_reg[i];
			sum_ls = sum_ls + SM4810_LS_reg[i];
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless ls 0X[%x]\n",__FUNCTION__, SM4810_LS_reg[i]);
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_SM4810 sum reg is %d \n",__FUNCTION__,sum_ls);

		if((sum_pmic == 0)||(sum_ls == 0))
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_SM4810 reg is all zero\n",__FUNCTION__);
			return -1;
		}

		ret = i2c_master_send_ex(1,SM4810_PMIC_ADDR,&temp_SM4810_PMIC[0],41);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_pmic write fail\n",__FUNCTION__);
			return -1;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_pmic write ok\n",__FUNCTION__);
		}
		...
		for(i=0 ; i<16; i++)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS reg 0x%x\n",__FUNCTION__,temp_SM4810_LS[i]);
		}
#endif
		ret = i2c_master_send_ex(1,SM4810_LS_ADDR,&temp_SM4810_LS[0],16);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_ls write fail\n",__FUNCTION__);
			return -1;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_ls write ok\n",__FUNCTION__);
			msleep(10);
			rtd_maskl(0xb8000824,0xff0fffff,0x00f00000);
			rtd_outl(0xb801bd00,0x00004001);
			rtd_outl(0xb801bd18,0x00004001);
			return 1;
		}
	}
...
void tconless_cs602_cs603_reset(void)
{
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_reset_flag = %d \n",__FUNCTION__,tconless_cs602_cs603_reset_is_done);
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] write cs602 data from reserve\n",__FUNCTION__);
		return;
	}
	else
	{
		for(i = 0; i < 3; i++)
       {
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] write PMIC data from code\n",__FUNCTION__);
			if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))
			{
				ret = i2c_master_send_ex(1,CS603_DEV_ADDR,&CS603_Data_BankA_55inch_D12_4[0],CS603_DATA_NUM+1);
			}
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229))
			{
				cs602_i2c_port_check();
				ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&CS602_Data_BankA_75D03_2[0],CS602_DATA_NUM+1);
			}
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231))
			{
				ret = i2c_master_send_ex(1,RT6966_PMIC_ADDR,&RT6966_Data_PMIC_50inch_U2[0],RT6966_PMIC_DATA_NUM+1);
				if(ret < 0)
				{		
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_pmic write fail\n",__FUNCTION__);
				}
				...
				msleep(10);
				ret = i2c_master_send_ex(1,RT6966_LS_ADDR,&RT6966_Data_LevelShift_50inch_U2[0],RT6966_LS_DATA_NUM+1);
				if(ret < 0)
				{		
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_ls write fail\n",__FUNCTION__);
				}
			...
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 232))
			{
				ret = i2c_master_send_ex(1,CS603_DEV_ADDR,&CS603_Data_BankA_65inch_D06_3[0],CS603_DATA_NUM+1);
			}
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 16)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 242))
			{
				ret = i2c_master_send_ex(1,SM4810_PMIC_ADDR,&SM4810_data_PMIC_43inch_DJ1[0],SM4810_PMIC_DATA_NUM+1);
				if(ret < 0)
				{		
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_pmic write fail\n",__FUNCTION__);
				}
				...
				msleep(10);
				ret = i2c_master_send_ex(1,SM4810_LS_ADDR,&SM4810_data_LS_43inch_DJ1[0],SM4810_LS_DATA_NUM+1);
				if(ret < 0)
				{		
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_ls write fail\n",__FUNCTION__);
				}
			...
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] this panel is not tconless panel with cs602_cs603\n",__FUNCTION__);
                return;
			}
		
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c cs602_cs603 write fail\n",__FUNCTION__);
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c cs602_cs603 write ok\n",__FUNCTION__);
                return;
			}
        }  
	}
...
void tconless_CS602_cs603_reserve(void)
{
	...
	unsigned char RT6966_reg_pmic_temp1[41] ={0xff};
	unsigned char RT6966_reg_pmic_temp2[41] ={0x00};
	unsigned char RT6966_reg_ls_temp1[15] ={0xff};
	unsigned char RT6966_reg_ls_temp2[15] ={0x00};
	unsigned char SM4810_reg_pmic_temp1[41] ={0xff};
	unsigned char SM4810_reg_pmic_temp2[41] ={0x00};
	unsigned char SM4810_reg_ls_temp1[16] ={0xff};
	unsigned char SM4810_reg_ls_temp2[16] ={0x00};
	   ...
	   ((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 232)))
	{
		for(i = 0;i < 3; i++)
		{
			ret = i2c_master_recv_ex(1, CS603_DEV_ADDR, &sub_addr, 1, &cs602_cs603_reg_temp1[0], 42);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 read fail \n",__FUNCTION__);
		
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 read ok\n",__FUNCTION__);
			}
			msleep(20);
			ret = i2c_master_recv_ex(1, CS603_DEV_ADDR, &sub_addr, 1, &cs602_cs603_reg_temp2[0], 42);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 read fail \n",__FUNCTION__);
			...
			for(i=0 ; i< 42; i++)
			{
				...
				{
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 reg match fail\n",__FUNCTION__);
					match_flag = 0;
					break;
				}
				else
			...
			if(match_flag == 1)
			{
				for(i=0 ; i< 42; i++)
				{
					cs603_reg[i] = cs602_cs603_reg_temp2[i];
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 reg 0x%x\n",__FUNCTION__,cs603_reg[i]);
				}
	...
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229)))
	{
		for(i = 0;i < 3; i++)
		{
			...
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 read fail \n",__FUNCTION__);
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 read ok\n",__FUNCTION__);
			}
			msleep(20);
			...
			for(i=0 ; i< 42; i++)
			{
				if(cs602_cs603_reg_temp1[i] != cs602_cs603_reg_temp2[i])
				{
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 reg match fail\n",__FUNCTION__);
					match_flag = 0;
					break;
				}
				else
			...
			if(match_flag == 1)
			{
				for(i=0 ; i< 42; i++)
				{
					...
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 reg 0x%x\n",__FUNCTION__,cs602_reg[i]);
				}
	...
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
	{
		for(i = 0;i < 3; i++)
		{
			ret = i2c_master_recv_ex(1, RT6966_PMIC_ADDR, &sub_addr, 1, &RT6966_reg_pmic_temp1[0], RT6966_PMIC_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC read fail \n",__FUNCTION__);
		
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC read ok\n",__FUNCTION__);
			}
			msleep(20);
			ret = i2c_master_recv_ex(1, RT6966_PMIC_ADDR, &sub_addr, 1, &RT6966_reg_pmic_temp2[0], RT6966_PMIC_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC read fail \n",__FUNCTION__);
			...
			ret = i2c_master_recv_ex(1, RT6966_LS_ADDR, &sub_addr, 1, &RT6966_reg_ls_temp1[0], RT6966_LS_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_LS read fail \n",__FUNCTION__);
			
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_LS read ok\n",__FUNCTION__);
			}
			msleep(20);
			ret = i2c_master_recv_ex(1,RT6966_LS_ADDR, &sub_addr, 1, &RT6966_reg_ls_temp2[0], RT6966_LS_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_LS read fail \n",__FUNCTION__);
			...
			if((!strcmp(RT6966_reg_pmic_temp1,RT6966_reg_pmic_temp2))&&(!strcmp(RT6966_reg_ls_temp1,RT6966_reg_ls_temp2)))
			{
				for(i=0 ; i< RT6966_PMIC_DATA_NUM; i++)
				{
					RT6966_PMIC_reg[i] = RT6966_reg_pmic_temp2[i];
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC reg 0x%x\n",__FUNCTION__,RT6966_PMIC_reg[i]);
				}

				for(i=0 ; i< RT6966_LS_DATA_NUM; i++)
				{
					RT6966_LS_reg[i] = RT6966_reg_ls_temp2[i];
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_LS reg 0x%x\n",__FUNCTION__,RT6966_LS_reg[i]);
				}
				return;
			}
			else
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC %d, LS %d no match \n",__FUNCTION__,!strcmp(RT6966_reg_pmic_temp1,RT6966_reg_pmic_temp2),!strcmp(RT6966_reg_ls_temp1,RT6966_reg_ls_temp2));
	...
	else if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 16) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 242)))
	{
		for(i = 0;i < 3; i++)
		{
			ret = i2c_master_recv_ex(1, SM4810_PMIC_ADDR, &sub_addr, 1, &SM4810_reg_pmic_temp1[0], SM4810_PMIC_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC read fail \n",__FUNCTION__);
		
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC read ok\n",__FUNCTION__);
			}
			msleep(20);
			ret = i2c_master_recv_ex(1, SM4810_PMIC_ADDR, &sub_addr, 1, &SM4810_reg_pmic_temp2[0], SM4810_PMIC_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC read fail \n",__FUNCTION__);
			...
			ret = i2c_master_recv_ex(1, SM4810_LS_ADDR, &sub_addr, 1, &SM4810_reg_ls_temp1[0], SM4810_LS_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS read fail \n",__FUNCTION__);
			
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS read ok\n",__FUNCTION__);
			}
			msleep(20);
			ret = i2c_master_recv_ex(1,SM4810_LS_ADDR, &sub_addr, 1, &SM4810_reg_ls_temp2[0], SM4810_LS_DATA_NUM);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS read fail \n",__FUNCTION__);
			...
			if((!strcmp(SM4810_reg_pmic_temp1,SM4810_reg_pmic_temp2))&&(!strcmp(SM4810_reg_ls_temp1,SM4810_reg_ls_temp2)))
			{
				for(i=0 ; i< SM4810_PMIC_DATA_NUM; i++)
				{
					SM4810_PMIC_reg[i] = SM4810_reg_pmic_temp2[i];
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC reg 0x%x\n",__FUNCTION__,SM4810_PMIC_reg[i]);
				}

				for(i=0 ; i< SM4810_LS_DATA_NUM; i++)
				{
					SM4810_LS_reg[i] = SM4810_reg_ls_temp2[i];
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS reg 0x%x\n",__FUNCTION__,SM4810_LS_reg[i]);
				}
				return;
			}
			else
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC %d, LS %d no match \n",__FUNCTION__,!strcmp(SM4810_reg_pmic_temp1,SM4810_reg_pmic_temp2),!strcmp(SM4810_reg_ls_temp1,SM4810_reg_ls_temp2));
	...
	if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))
	{
		for(i=0 ; i< 42; i++)
		{
			cs603_reg[i] =	CS603_Data_BankA_55inch_D12_4[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 reg ok from code\n",__FUNCTION__);
	...
	else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229))
	{
		for(i=0; i<42; i++)
		{
			cs602_reg[i] = CS602_Data_BankA_75D03_2[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 reg ok from code\n",__FUNCTION__);
	}
	else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231))
	{
		for(i=0 ; i< RT6966_PMIC_DATA_NUM; i++)
		{
			RT6966_PMIC_reg[i] = RT6966_Data_PMIC_50inch_U2[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_PMIC reg ok from code\n",__FUNCTION__);

		for(i=0 ; i< RT6966_LS_DATA_NUM; i++)
		{
			RT6966_LS_reg[i] = RT6966_Data_LevelShift_50inch_U2[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_RT6966_LS reg ok from code\n",__FUNCTION__);
	}
	else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 232))
	{
		for(i=0 ; i< 42; i++)
		{
			cs603_reg[i] =	CS603_Data_BankA_65inch_D06_3[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 reg ok from code\n",__FUNCTION__);
	}
	else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 16)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 242))
	{
		for(i=0 ; i< SM4810_PMIC_DATA_NUM; i++)
		{
			SM4810_PMIC_reg[i] =	SM4810_data_PMIC_43inch_DJ1[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_PMIC reg ok from code\n",__FUNCTION__);

		for(i=0 ; i< SM4810_LS_DATA_NUM; i++)
		{
			SM4810_LS_reg[i] =	SM4810_data_LS_43inch_DJ1[i+1];
		}
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_LS reg ok from code\n",__FUNCTION__);
	}	
}

 int tconless_set_gamma_bin(KADP_VBE_DISP_TCON_GAMMA_BIN_DATA_T data)
{
	int ret,ret2,i;
	unsigned char bin_data_cs603[48];
	unsigned char bin_data_cs602[48];
	unsigned char bin_data_G2532[70];
	unsigned char bin_data_RT6966_PMIC[48];
	//unsigned char bin_data_RT6966_LS[48];
	unsigned char bin_data_SM4810_PMIC[48];
	//unsigned char bin_data_SM4810_LS[48];
	unsigned char bin_data_G2510S_PMU[48];
	unsigned char bin_data_G1621_GAMMA[48];
	
	KADP_VBE_DISP_TCON_GAMMA_BIN_DATA_T data_param=data;

	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] write PMU_BIN_DATA from code\n",__FUNCTION__);
	if((data_param.iCONFIG_PANEL_CUSTOM_INDEX == 228)||
		(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 232))
	{
		bin_data_cs603[0] = 0x00;
		if(data_param.tcon_pmu_data_len == 90)
		{
			for(i=0 ;i<43 ;i++)
				bin_data_cs603[i+1] = *data_param.tcon_pmu_data++;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] read file size does not match\n",__FUNCTION__);
			return -1;
		}
		for(i=0; i<43; i++) //for debug
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]pmu_bin 0X[%x]\n",__FUNCTION__,bin_data_cs603[i]);
		
		ret = i2c_master_send_ex(1,CS603_DEV_ADDR,&bin_data_cs603[0],CS603_DATA_NUM+1);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs603 write fail\n",__FUNCTION__);
			return -1;
	...
	else if(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 229)
	{
		bin_data_cs602[0] = 0x00;
		if(data_param.tcon_pmu_data_len == 90)
		{
			for(i=0 ;i<43 ;i++)
				bin_data_cs602[i+1] = *data_param.tcon_pmu_data++;
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] read file size does not match\n",__FUNCTION__);
		...
		for(i=0; i<43; i++) //for debug
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]pmu_bin 0X[%x]\n",__FUNCTION__,bin_data_cs602[i]);
		
		cs602_i2c_port_check();
		ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&bin_data_cs602[0],CS602_DATA_NUM+1);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_cs602 write fail\n",__FUNCTION__);
			return -1;
	...
	else if(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 231)
	{
		bin_data_RT6966_PMIC[0] = 0x00;
		if(data_param.tcon_pmu_data_len == 40)
		{
			for(i=0 ;i<41 ;i++)
			{
				bin_data_RT6966_PMIC[i+1] = *data_param.tcon_pmu_data++;
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] pmu_bin 0X[%x]\n",__FUNCTION__, bin_data_RT6966_PMIC[i]);
			}
		}
		else
		{
	...
	else if((data_param.iCONFIG_PANEL_CUSTOM_INDEX == 242)||
			(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 242))
	{
		bin_data_SM4810_PMIC[0] = 0x00;
		if(data_param.tcon_pmu_data_len == 40)
		{
			for(i=0 ;i<41 ;i++)
			{
				bin_data_SM4810_PMIC[i+1] = *data_param.tcon_pmu_data++;
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] pmu_bin 0X[%x]\n",__FUNCTION__, bin_data_SM4810_PMIC[i]);
			}
		}
		else
		{
		...
		rtd_outl(0xb801bd18,0x00000001);

		ret = i2c_master_send_ex(1,SM4810_PMIC_ADDR,&bin_data_SM4810_PMIC[0],SM4810_PMIC_DATA_NUM+1);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_SM4810_pmic write fail\n",__FUNCTION__);
			return -1;
	...
	else if((data_param.iCONFIG_PANEL_CUSTOM_INDEX == 233)||
			(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 235)||
			(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 236)||
			(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 237))
	{
			bin_data_G2532[0] = 0x00;
			if(data_param.tcon_pmu_data_len == 66)
			{
				for(i=0 ;i<67 ;i++)
				{
					bin_data_G2532[i+1] = *data_param.tcon_pmu_data++;
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] pmu_bin 0X[%x]\n",__FUNCTION__, bin_data_G2532[i]);
				}
			}
			else
			{
			...
			ret = i2c_master_send_ex(1,G2532_DEV_ADDR,&bin_data_G2532[0],G2532_DATA_NUM+1);
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G2532_pmic write fail\n",__FUNCTION__);
				return -1;
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G2532_pmic write ok\n",__FUNCTION__);
			}


	}
	else if((data_param.iCONFIG_PANEL_CUSTOM_INDEX == 230)||
			(data_param.iCONFIG_PANEL_CUSTOM_INDEX == 234))
	{
		bin_data_G2510S_PMU[0] = G2510S_DATA_ADDR;
		bin_data_G1621_GAMMA[0] = G1621_VCOM_ADDR;
		if(data_param.tcon_pmu_data_len == 48)
		{
			for(i=0;i<25 ;i++)
				{
					bin_data_G2510S_PMU[i+1] = *(data_param.tcon_pmu_data+G2510S_DATA_ADDR+i);
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless bin_data_G2510S_PMU 0X[%x]\n",__FUNCTION__, bin_data_G2510S_PMU[i]);
				}
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] read bin_data_G2510S_PMU size does not match\n",__FUNCTION__);
			ret =-1;
		}

		if(data_param.tcon_p_gamma_data_len == 47)
		{
			for(i=0 ;i<25 ;i++)
			{
				bin_data_G1621_GAMMA[i+1] = *(data_param.tcon_p_gamma_data+G1621_VCOM_ADDR+i);
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]tconless bin_data_G1621_GAMMA 0X[%x]\n",__FUNCTION__, bin_data_G1621_GAMMA[i]);
			}
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] read bin_data_G1621_GAMMA size does not match\n",__FUNCTION__);
			ret2 =-1;
		}

		
		ret = i2c_master_send_ex(1,G2510S_DEV_ADDR,&bin_data_G2510S_PMU[0],G2510S_DATA_NUM+1);
		if(ret < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G2510S_pmu write fail\n",__FUNCTION__);
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G2510S_pmu write ok\n",__FUNCTION__);
		}

		msleep(10);
		ret2 = i2c_master_send_ex(1,G1621_DEV_ADDR,&bin_data_G1621_GAMMA[0],G2510S_DATA_NUM+1);
		if(ret2 < 0)
		{		
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G1621_gamma write fail\n",__FUNCTION__);
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c tconless_G1621_gamma write ok\n",__FUNCTION__);
		}
		if(ret<0 && ret2<0)
			return -1;
	}
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] this panel is not tconless panel with PMIC\n",__FUNCTION__);
        return -1;
	}
	return 0;
}
...
void Panel_resume_TurnOn(void)
{
	...
	{
	    rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] Normal wakeup, ctrl panel on now\n",__FUNCTION__);
	    Panel_TurnOn_By_KernelAp(0);
	}
	else
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] RTC/WOW wakeup, ctrl panel on later\n",__FUNCTION__);
}
//extern HAL_VBE_DISP_SetMute(BOOLEAN bOnOff);
void Panel_TurnOn_By_KernelAp(unsigned int value)
{
	unsigned int regValue;
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] STR wakeup, panel on by kernel(0) or ap(1)? %d\n",__FUNCTION__, value);
	...
	if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))||
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
	{
		msleep(259);
		rtd_maskl(0xb80008a8,0xffffff0f,0x00000070);
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "Trun_on stv pin of STR \n");
	}
	regValue = rtd_inl(WKSOR_REG);
	if((regValue & 0xff) == WKSOR_WUT){
//            HAL_VBE_DISP_SetMute(1);
	    rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] WUT wakeup, open backlight later\n",__FUNCTION__);
        }
	else
	{
	    rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"Backlight_TurnOn BL_delay=%d(ms) \n",Get_LVDS_TO_LIGHT_ON_ms());
