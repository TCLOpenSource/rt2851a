...
{//disable capture 0 for 4k120 borrow i3ddma memory
	...
	pr_notice("### %s i3ddma capture disable status (%d), timeoutcnt=%d db=%x", __FUNCTION__, (H3DDMA_CAP0_CTI_DMA_WR_Ctrl_get_cap0_dma_enable(IoReg_Read32(H3DDMA_CAP0_CTI_DMA_WR_Ctrl_reg))),
...
//	int i;
	...
	{
		size_video = MDOMAIN_MAIN_BUFFER_SIZE_16BITS_HDMI_4K ;//m domain pc mode，vbm borrow 64M 
	...
	if((uhd_flag == 0) && (get_platform() == PLATFORM_KXLP))//2k kxlp need 24M for dexc
	{
	...
	*/
	...
	if(bAlloc)
    {
		...
		if(getSize < _4K_2K_10BIT_SIZE)
		    pr_emerg("[DolbyVision][Minchay][%s] Error: vbm mem size(getSize:%ld) is not 64M \n", __FUNCTION__ ,getSize);
#endif
...
//	vodma_vodma_pvs0_gen_RBUS vodma_vodma_pvs0_gen_Reg;
	...
	if (enable == _ENABLE){
		...
		if (get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 1) {//fix 4k max size
...
#define _2K_2K_8BIT_SIZE  ((_2K_WID * 8 + 256)  * _2K_LEN / 8)
	...
	if(g_4K120_SE_Buffer[0].phyaddr_unalign == 0)
	{
		...
		if(g_4K120_SE_Buffer[0].phyaddr_unalign == 0)
		{
			pr_emerg("i3ddma se alloc memory fail!\n");
			return false;
		}
	}
	...
	pr_notice("[4k120]SE_buf[0]=%lx,SE_buf[1]=%lx,SE_buf[2]=%lx,c_size=%x",g_4K120_SE_Buffer[0].phyaddr,g_4K120_SE_Buffer[1].phyaddr,g_4K120_SE_Buffer[2].phyaddr,g_4K120_SE_Buffer[0].size);
...
#define _2K_WID 1920	
	...
	pr_notice("[4k120]cap_y_buf[0]=%lx,cap_y_buf[1]=%lx,cap_y_buf[2]=%lx,y_size=%x",g_4K120_Cap_Y_Buffer[0].phyaddr,g_4K120_Cap_Y_Buffer[1].phyaddr,g_4K120_Cap_Y_Buffer[2].phyaddr,g_4K120_Cap_Y_Buffer[0].size);
