...
extern unsigned char scdc_tmds_config_update[HDMI_PORT_TOTAL_NUM];
...
unsigned char hdmi_source_change_disable_delay[HDMI_RX_PORT_MAX_NUM];
...
int vfe_hdmi_drv_connect(unsigned char ch)
{
	...
	hdmi_source_change_disable_delay[hdmi_port] = TRUE;
...
{    
    ...
    if (lib_hdmi_get_fw_debug_bit(DEBUG_12_FORCE_NO_TIMING))
        {
        HDMI_EMG("this is debug test ! vfe_hdmi_drv_get_port_timing_info force no timing for test scaler rerun!");
        return HDMI_DRV_NO_ERR;
        }

    if((tm.h_act_len == 640)&&(tm.v_act_len == 480)&&(HDMI_ABS(tm.v_freq, 720) <10))// tcl 22293 timingID 005
    	{
      	tm.v_freq=720;
    	}
	...
	switch(tm.mode) {
	case MODE_DVI:    timing_info->mode = KADP_VFE_HDMI_DVI;    break;
	case MODE_HDMI:   timing_info->mode = KADP_VFE_HDMI_HDMI;   break;
	case MODE_HDMI21: timing_info->mode = KADP_VFE_HDMI_HDMI21; break;
	default:          timing_info->mode = KADP_VFE_HDMI_DVI;    break;
	}
	...
	)
{
	...
	memcpy(edid, p_edid, sizeof(edid));                    // copy edid for modification
	...
	{
		if(newbase_hdmi_write_edid(port, edid, sizeof(edid))==0)
		{
...
int vfe_hdmi_drv_get_SCDC_Status(unsigned char ch, unsigned int *val)
{
	...
	*val = lib_hdmi_scdc_read(port,  SCDC_TMDS_CONFIGURATION) |
	(scdc_tmds_config_update[port]<<8) |
