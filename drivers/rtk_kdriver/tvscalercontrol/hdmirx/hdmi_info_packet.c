...
unsigned char newbase_hdmi_update_vsi(unsigned char port)
{
    ...
    if (lib_hdmi_read_vsi_info_packet(port, vsi)==0) {
...
unsigned char newbase_hdmi_get_is_hdr10_type(unsigned char port)
{
    if(hdmi_rx[port].drm_t.eEOTFtype == 0)
        return 0;
    else if(hdmi_rx[port].drm_t.eEOTFtype == 2)
        return 1;
    else
        return 0;
}
