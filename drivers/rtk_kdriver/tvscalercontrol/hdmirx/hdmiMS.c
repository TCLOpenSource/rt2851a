...
unsigned char newbase_hdmims_measure_video(unsigned char port, HDMI_TIMING_T *tm, unsigned char frl_mode)
{
	...
	HDMI_OFMS_WARN("[newbase_hdmims_measure_video port:%d] HVSync Measure Mode, vrr_mode=%d\n",
...
static unsigned int m_onms_watchdog_en[3];
static unsigned int m_onms_int_en[3];

void lib_hdmims_onms1_watchdog_int_en(unsigned char enable)
{
	if(enable)
	{
		hdmi_out(ONMS_onms1_watchdog_en_reg, m_onms_watchdog_en[0]); // recover to original
		hdmi_out(ONMS_onms1_interrupt_en_reg, m_onms_int_en[0]); // recover to original
	}
	else
	{
		m_onms_watchdog_en[0] = hdmi_in(ONMS_onms1_watchdog_en_reg);
		hdmi_out(ONMS_onms1_watchdog_en_reg, 0);    //all set 0
		m_onms_int_en[0] = hdmi_in(ONMS_onms1_interrupt_en_reg);
		hdmi_out(ONMS_onms1_interrupt_en_reg, 0);    //all set 0

	}
}


void lib_hdmims_onms3_watchdog_int_en(unsigned char enable)
{
	if(enable)
	{
		hdmi_out(ONMS_onms3_watchdog_en_reg, m_onms_watchdog_en[2]); // recover to original
		hdmi_out(ONMS_onms3_interrupt_en_reg, m_onms_int_en[2]); // recover to original
	}
	else
	{
		m_onms_watchdog_en[2] = hdmi_in(ONMS_onms3_watchdog_en_reg);
		hdmi_out(ONMS_onms3_watchdog_en_reg, 0);    //all set 0
		m_onms_int_en[2] = hdmi_in(ONMS_onms3_interrupt_en_reg);
		hdmi_out(ONMS_onms3_interrupt_en_reg, 0);    //all set 0
