...
extern unsigned char lib_hdmi_audio_get_thread_run(unsigned char nport);
...
extern unsigned char hdmi_source_change_disable_delay[HDMI_RX_PORT_MAX_NUM];
...
unsigned char hdmi_check_hdcp_delay_flag[HDMI_RX_PORT_MAX_NUM];
...
unsigned char newbase_hdmi_detect_mode(unsigned char port)
{
	...
	switch (GET_H_VIDEO_FSM(port)) {
		case MAIN_FSM_HDMI_WAIT_SYNC:
			...
			hdmi_check_hdcp_delay_flag[port]=0;
		...
		case MAIN_FSM_HDMI_DISPLAY_ON:
			if (newbase_hdmi_display_on(port, p_phy_st->frl_mode)) {
				...
				hdmi_check_hdcp_delay_flag[port]=1;
				if(!hdmi_good_timing_ready)
...
unsigned char newbase_hdmi_check_mode(void)
{
	...
	if (TRUE == newbase_hdmi_get_is_interlace_change(port, p_phy_st->frl_mode)) {
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    else if (lib_hdmi_ps_measure_complete(port))
    {
        ...
        else
        {
           ...
           // if (hdmi_rx[port].video_t.vs_valid < 0xFF)
            ...
            {
                ERP_WARN("HDMI[p%d] VSync Freq Changed = %d -> %d (valid_cnt=%d, last_vs_locked=%d, last_stable_cnt=%d))\n",
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        ERP_INFO("HDMI[p%d] RxStatus(%02x, %02x), hdcp_enc=%d, bch 2bit error (detect,duration,cnt)=(%d, %d, %d), 1bit=%d\n", 
...
void newbase_hdmi_check_connection_state(unsigned char port)
{
	...
	if (conn_st != hdmi_rx[port].cable_conn)
	{
		...
		if (conn_st) {
			...
			hdmi_source_change_disable_delay[port] = FALSE;
...
//			return _FALSE;
	...
	if(hdmi_check_hdcp_delay_flag[port]==1 || hdmi_source_change_disable_delay[port] != TRUE)
	{
		if((((hdmi_rx[port].timing_t.h_act_len==1920)&&((hdmi_rx[port].timing_t.v_act_len==540)||(hdmi_rx[port].timing_t.v_act_len==1080)))||
		((hdmi_rx[port].timing_t.h_act_len==1280)&&((hdmi_rx[port].timing_t.v_act_len==360)||(hdmi_rx[port].timing_t.v_act_len==720))))&&
		((ABS(hdmi_rx[port].timing_t.v_freq,500)<10)||(ABS(hdmi_rx[port].timing_t.v_freq,600)<10)))
		{
			for(j=0;j<16;j++ )
			{
				if (newbase_hdcp_get_auth_mode(port)== NO_HDCP)
					msleep(70);
				else
					break;
			}
	...
	if(newbase_hdmi_get_online_measure_error_flag(port) && !lib_hdmi_audio_get_thread_run(port))
	{
		HDMI_EMG("AUDIO FSM == %d, Onlinemeasure_error_flag == 1, detect again!!!",GET_H_AUDIO_FSM(port));
		goto check_condition_fail;
	}
	hdmi_check_hdcp_delay_flag[port]=0;
