    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "meas")==0)
    {
        ...
        if(lib_hdmims_offline_measure_native(&tm, frl_mode))
        {
            ...
            ptr+=n; count-=n;
            memset(&tm, 0, sizeof(HDMI_TIMING_T));
            lib_hdmims_onms1_watchdog_int_en(0);
            measure_result &= lib_hdmims_online1_measure(&tm);

            memset(&tm2, 0, sizeof(HDMI_TIMING_T));
            lib_hdmims_onms3_watchdog_int_en(0);
        ...
        ptr+=n; count-=n;
        n = scnprintf(ptr, count, "ONMSWDG_EN=x; x=0~1, 1: enable, 0: disable\n");
        ptr+=n; count-=n;
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "meas")==0)
    {
        if (sscanf(buf, "PIXEL_CLOCK=%d",&val)==1)
        {
            unsigned char port = newbase_hdmi_get_current_display_port();
            lib_hdmi_ofms_clock_sel(port, (val) ?1:0);
        }
        else if (sscanf(buf, "ONMSWDG_EN=%d",&val)==1)
        {
            lib_hdmims_onms1_watchdog_int_en((val) ?1:0);
            lib_hdmims_onms3_watchdog_int_en((val) ?1:0);
        }
        else if (sscanf(buf, "ONMS1_SRCSEL=%d",&val)==1)
        {// 0: VGIP input, 1: VGIP output
            lib_hdmims_online1_set_ms_src_sel((val) ?1:0);
        }
        else if (sscanf(buf, "ONMS3_SRCSEL=%d",&val)==1)
        {// 0: VGIP input, 1: VGIP output
