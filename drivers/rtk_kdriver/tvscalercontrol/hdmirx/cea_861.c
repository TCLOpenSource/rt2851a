...
unsigned char _svd_to_vic(unsigned char svd)
{
    ...
    if(svd>=1 && svd<=64)
        return (svd & (~0x80)); // 7-bit VIC is defined (7-LSB¡¦s) and NOT a native code
    else if ((svd >= 65 && svd <= 127) ||
        (svd >= 193 && svd <= 255))
        return svd;
    else if(svd>=129 && svd<=192)
        return ((svd | 0x7F) & 0x80); // 7-bit VIC is defined (7-LSB¡¦s) and IS a native code
    else
        return 0;
}
 ...
 * Retn : svd : svd number 
 *-------------------------------------------*/
unsigned char _vic_to_svd(unsigned char vic)
{
    // CEA-861-F section 7-5.1 - Video Data block
     if(vic>=1 && vic<=64)
        return (vic | 0x80);
    else if ((vic >= 65 && vic <= 127) ||
        (vic >= 193 && vic <= 255))
        return vic;
    else if(vic >= 129 && vic <= 192)
        return (vic | 0x80);
    else
        return 0; //VIC=0,128,254,255 RESERVED
}
