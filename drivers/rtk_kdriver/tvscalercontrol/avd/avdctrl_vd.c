...
//			drvif_module_vdc_Set_lose_chromalock_count(0x0b);	/* set to default */
			...
			if((avd_good_timing_ready == false) && (vdc_TV_reset == true)){
				rtd_printk(KERN_WARNING, TAG_NAME_AVD, "#####[%s(%d)] 0xb8019008=0x%08x,0xb801900c=0x%08x,0xb8019010=0x%08x\n", __func__, __LINE__, vdcf_inl(0xb8019008),vdcf_inl(0xb801900c),vdcf_inl(0xb8019010));
				return ucRet;
			}
			...
			else
			{
				...
				if (vd_power_check == 0x1) { //vd_cpeak_enable
				    if (!module_vdc_Cpeak_read()){
				        rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[VDC] cpeak_control on !!!! ");
				        cpeak_check_flag =1;
				    } 
