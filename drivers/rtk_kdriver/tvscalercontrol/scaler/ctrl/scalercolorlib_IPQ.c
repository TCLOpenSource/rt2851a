...
unsigned short auto_gamma_R[1025]={0};
unsigned short auto_gamma_G[1025]={0};
unsigned short auto_gamma_B[1025]={0};
...
unsigned int LinearOutputGamma[Bin_Num_Output_InvOutput_Gamma/2]={
    0x00001010, 0x00201010, 0x00401010, 0x00601010, 0x00801010, 0x00a01010, 0x00c01010, 0x00e01010, //B 0 ~ 7
    0x01001010, 0x01201010, 0x01401010, 0x01601010, 0x01801010, 0x01a01010, 0x01c01010, 0x01e01010, //B 8 ~ 15
    0x02001010, 0x02201010, 0x02401010, 0x02601010, 0x02801010, 0x02a01010, 0x02c01010, 0x02e01010, //B 16 ~ 23
    0x03001010, 0x03201010, 0x03401010, 0x03601010, 0x03801010, 0x03a01010, 0x03c01010, 0x03e01010, //B 24 ~ 31
    0x04001010, 0x04201010, 0x04401010, 0x04601010, 0x04801010, 0x04a01010, 0x04c01010, 0x04e01010, //B 32 ~ 39
    0x05001010, 0x05201010, 0x05401010, 0x05601010, 0x05801010, 0x05a01010, 0x05c01010, 0x05e01010, //B 40 ~ 47
    0x06001010, 0x06201010, 0x06401010, 0x06601010, 0x06801010, 0x06a01010, 0x06c01010, 0x06e01010, //B 48 ~ 55
    0x07001010, 0x07201010, 0x07401010, 0x07601010, 0x07801010, 0x07a01010, 0x07c01010, 0x07e01010, //B 56 ~ 63
    0x08001010, 0x08201010, 0x08401010, 0x08601010, 0x08801010, 0x08a01010, 0x08c01010, 0x08e01010, //B 64 ~ 71
    0x09001010, 0x09201010, 0x09401010, 0x09601010, 0x09801010, 0x09a01010, 0x09c01010, 0x09e01010, //B 72 ~ 79
    0x0a001010, 0x0a201010, 0x0a401010, 0x0a601010, 0x0a801010, 0x0aa01010, 0x0ac01010, 0x0ae01010, //B 80 ~ 87
    0x0b001010, 0x0b201010, 0x0b401010, 0x0b601010, 0x0b801010, 0x0ba01010, 0x0bc01010, 0x0be01010, //B 88 ~ 95
    0x0c001010, 0x0c201010, 0x0c401010, 0x0c601010, 0x0c801010, 0x0ca01010, 0x0cc01010, 0x0ce01010, //B 96 ~ 103
    0x0d001010, 0x0d201010, 0x0d401010, 0x0d601010, 0x0d801010, 0x0da01010, 0x0dc01010, 0x0de01010, //B 104 ~ 111
    0x0e001010, 0x0e201010, 0x0e401010, 0x0e601010, 0x0e801010, 0x0ea01010, 0x0ec01010, 0x0ee01010, //B 112 ~ 119
    0x0f001010, 0x0f201010, 0x0f401010, 0x0f601010, 0x0f801010, 0x0fa01010, 0x0fc01010, 0x0fe0100f, //B 120 ~ 127
};
...
void fwif_color_gamma_encode_TableSize_2(RTK_TableSize_Gamma *pData)
{
	...
	static unsigned short cal_outGamma_R[256],cal_outGamma_G[256],cal_outGamma_B[256];
	...
	//printk("#p,1Ori=%d,%d,%d,%d,%d\n",pData->pu16Gamma_r[53],pData->pu16Gamma_r[54],pData->pu16Gamma_r[55],pData->pu16Gamma_r[56],pData->pu16Gamma_r[57]);
	for(i=0;i<256;i++){
		cal_outGamma_R[i]=pData->pu16Gamma_r[i]<<2;//(4096/1024)=4
		cal_outGamma_G[i]=pData->pu16Gamma_g[i]<<2;
		cal_outGamma_B[i]=pData->pu16Gamma_b[i]<<2;
	}
	//printk("#p,2Ori=%d,%d,%d,%d,%d\n",cal_Gamma_R[53],cal_Gamma_R[54],cal_Gamma_R[55],cal_Gamma_R[56],cal_Gamma_R[57]);
	
	maxvalueR=cal_outGamma_R[255];
	maxvalueG=cal_outGamma_G[255];
	maxvalueB=cal_outGamma_B[255];
	
	for(i=255;i>1;i--)
	{
		cal_outGamma_R[i]=cal_outGamma_R[i-1];
		cal_outGamma_G[i]=cal_outGamma_G[i-1];
		cal_outGamma_B[i]=cal_outGamma_B[i-1];
	}

	cal_outGamma_R[1]=(cal_outGamma_R[1])*2/3;
	cal_outGamma_G[1]=(cal_outGamma_G[1])*2/3;
	cal_outGamma_B[1]=(cal_outGamma_B[1])*2/3;


	//R---
		for(i=0;i<256;i++){
		if(i<255){
			bit_diff_r = ((cal_outGamma_R[(i+1)]-cal_outGamma_R[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_R[4*i+j]= cal_outGamma_R[i]+bit_diff_r*j;
		}else{
			bit_diff_r = ((maxvalueR-cal_outGamma_R[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_R[4*i+j]= cal_outGamma_R[i]+bit_diff_r*j;
		}
	}
	outGamma_R[Bin_Num_Gamma]=(outGamma_R[Bin_Num_Gamma-1]+1)>4095?outGamma_R[Bin_Num_Gamma-1]:(outGamma_R[Bin_Num_Gamma-1]+1);

	//G
		for(i=0;i<256;i++){
		if(i<255){
			bit_diff_g = ((cal_outGamma_G[(i+1)]-cal_outGamma_G[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_G[4*i+j]= cal_outGamma_G[i]+bit_diff_g*j;
		}else{
			bit_diff_g = ((maxvalueG-cal_outGamma_G[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_G[4*i+j]= cal_outGamma_G[i]+bit_diff_g*j;
		}
	}
	outGamma_G[Bin_Num_Gamma]=(outGamma_G[Bin_Num_Gamma-1]+1)>4095?outGamma_G[Bin_Num_Gamma-1]:(outGamma_G[Bin_Num_Gamma-1]+1);

	//B
	for(i=0;i<256;i++){
		if(i<255){
			bit_diff_b = ((cal_outGamma_B[(i+1)]-cal_outGamma_B[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_B[4*i+j]= cal_outGamma_B[i]+bit_diff_b*j;
		}else{
			bit_diff_b = ((maxvalueB-cal_outGamma_B[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outGamma_B[4*i+j]= cal_outGamma_B[i]+bit_diff_b*j;
		}
	}
	outGamma_B[Bin_Num_Gamma]=(outGamma_B[Bin_Num_Gamma-1]+1)>4095?outGamma_B[Bin_Num_Gamma-1]:(outGamma_B[Bin_Num_Gamma-1]+1);
	
	memcpy(auto_gamma_R, outGamma_R, sizeof(unsigned short)*(Bin_Num_Gamma+1));
	memcpy(auto_gamma_G, outGamma_G, sizeof(unsigned short)*(Bin_Num_Gamma+1));
	memcpy(auto_gamma_B, outGamma_B, sizeof(unsigned short)*(Bin_Num_Gamma+1));
	fwif_color_gamma_curve_data_protect(auto_gamma_R, auto_gamma_G, auto_gamma_B);		/*DO NOT mark this, must check*/

	outTBL_index=2;
	
	fwif_color_gamma_encode(GammaTable_R,GammaTable_G,GammaTable_B,outGamma_R,outGamma_G,outGamma_B);
...
void fwif_color_outgamma_extend_TableSize(unsigned short*outR,unsigned short*outG,unsigned short*outB,unsigned int*inR,unsigned int*inG,unsigned int*inB)
{
	unsigned short i=0;
	unsigned short bit_diff_r=0,bit_diff_g=0,bit_diff_b=0;
	unsigned char j=0;
	unsigned short maxvalueR=4095,maxvalueG=4095,maxvalueB=4095;
	//unsigned short outTBL_index=0;
	//static unsigned short cal_outGamma_R[256],cal_outGamma_G[256],cal_outGamma_B[256];
	...
	for(i=0;i<256;i++){
		if(i<255){
			bit_diff_r = ((inR[(i+1)]-inR[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outR[4*i+j]= inR[i]+bit_diff_r*j;
		}else{
			bit_diff_r = ((maxvalueR-inR[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outR[4*i+j]= inR[i]+bit_diff_r*j;
		}
	}
	...
	//G
		for(i=0;i<256;i++){
		if(i<255){
			bit_diff_g = ((inG[(i+1)]-inG[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outG[4*i+j]= inG[i]+bit_diff_g*j;
		}else{
			bit_diff_g = ((maxvalueG-inG[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outG[4*i+j]= inG[i]+bit_diff_g*j;
		}
	}
	...
	for(i=0;i<256;i++){
		if(i<255){
			bit_diff_b = ((inB[(i+1)]-inB[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outB[4*i+j]= inB[i]+bit_diff_b*j;
		}else{
			bit_diff_b = ((maxvalueB-inB[i])>>2);
				for(j = 0 ; j < 4 ; j++)
					outB[4*i+j]= inB[i]+bit_diff_b*j;
		}
	}
	...
	/*data protect*/
	outR[Bin_Num_Gamma] = outR[Bin_Num_Gamma - 1];
	outG[Bin_Num_Gamma] = outG[Bin_Num_Gamma - 1];
	outB[Bin_Num_Gamma] = outB[Bin_Num_Gamma - 1];
...
void fwif_set_gamma_multiply_remap_Tcon(UINT16 *final_R, UINT16 *final_G, UINT16 *final_B, UINT16 *Out_R, UINT16 *Out_G, UINT16 *Out_B)
{
	...
	#if 0
	{
		...
		for(ii=0;ii< 20;ii++)
		{
			printk("remap:fr[%d]=%d,ar[%d]=%d\n",ii,final_R[ii],ii,auto_gamma_R[ii]);
		}
		
		for(ii=1025-10;ii<1025;ii++)
		{
			printk("remap:fr[%d]=%d,ar[%d]=%d \n",ii,final_R[ii],ii,auto_gamma_R[ii]);
		}
	}
...
{
	unsigned char ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
	unsigned char isHD;
	...
	printk("Get_DisplayMode_Src(SLR_MAIN_DISPLAY) = %d \n",Get_DisplayMode_Src(SLR_MAIN_DISPLAY));
	...
	if (srcType == _SRC_HDMI) {
		...
		printk("hdmi,hdr yuv2rgb,colorFMT_info=%d,EC=%d,C=%d \n",colorFMT_info,pAviInfo.EC,pAviInfo.C);
		...
		data_range = HDMI_CSC_DATA_RANGE_MODE_LIMITED;
		printk("HDR Y2R,VO colorFMT_info = %d \n",colorFMT_info);
	...
	printk("HDR Y2R,CSC_SMR,disp=%d,src=%d,fmt=%d,ret=%d,mode=%d,CSC1_range=%d,\n",
...
VIP_YUV2RGB_LEVEL_SELECT m_uv_offset_level = VIP_YUV2RGB_LEVEL_OFF;
...
void fwif_SetSubChennelRBswap(unsigned char swap)
{	
	if (swap)
	{
		drvif_color_set_sub_RGB_swap(0,1,1);	
	}
	else
	{
		drvif_color_set_sub_RGB_swap(0,0,0);	
	}
