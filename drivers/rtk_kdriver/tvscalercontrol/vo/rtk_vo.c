...
static unsigned int vdec_start_shift_value = 0;//v start value
...
unsigned char vdec_seamless_change_flag = OFF;//i3ddma format need to change
  ...
  // -------------------------------
void vdec_set_vstart_shift( unsigned int shift)
{//set vdec_start_shift_info
	vdec_start_shift_value = shift;
}

unsigned char vdec_get_vstart_shift(unsigned int *shift)
{//retunr TRUE: need shift  return false: no need to shift
	if(shift)
	{
		*shift = vdec_start_shift_value;
		return (vdec_start_shift_value) ? TRUE : FALSE;  
	}
	return FALSE;  
}
...
unsigned int rpcVoReady(unsigned long para1, unsigned long para2)
{
	...
	StructRect bbd_vo_info = {0};//for first timie BBD info
	VO_DATA *vo = &vodma_data[0];
	SLR_VOINFO VOInfo;
	...
	vdec_set_vstart_shift(pVOInfo->startPointShiftOffset);
	if ((ch == 0) || (ch == 1)) {
	...
	//for KTASKWBS-11225
	bbd_vo_info.width = pVOInfo->h_width;
	bbd_vo_info.height = pVOInfo->v_length;
	#if 0
	if(ch == 0)
	{//main
		vsc_v4l2_active_win_event_wakeup(SLR_MAIN_DISPLAY, bbd_vo_info, bbd_vo_info);
	}
	else
	{//sub
		vsc_v4l2_active_win_event_wakeup(SLR_SUB_DISPLAY, bbd_vo_info, bbd_vo_info);
	}
	#endif
...
//#ifdef CONFIG_HDR_SDR_SEAMLESS
		...
		#endif
#else
		if ((memcmp(&vo->videoPlane[ch].VOInfo, pVOInfo, ((unsigned int)&(pVOInfo->source)-(unsigned int)&(pVOInfo->plane))) == 0))
