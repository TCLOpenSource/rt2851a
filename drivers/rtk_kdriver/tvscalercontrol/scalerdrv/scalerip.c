...
unsigned char set_di_doublebuf_write_TV006(unsigned char diflag,unsigned char _2d_flag)
{
	...
	unsigned int db_apply_st, db_apply_end;
	int timeout_1;
	...
	if (DI_IP_En_Lv2 == 0) {
		...
		if (diflag == 0) {
			...
			if (timeout_1 < 0) {
				pr_emerg("DI_Enable time out, IP_disable timeout=%d, db_st=%d, db_end=%d, diflag=%d, 2dflag=%d,\n", 
					timeout_1, db_apply_st, db_apply_end, diflag, _2d_flag);
			}
			pr_info("DI_Enable un-f2D done IP_disable,t_0=%d, set_di_sta_90k=%d, db_st=%d, db_end=%d, diflag=%d, 2dflag=%d,\n", 
				timeout_1, set_di_sta_90k, db_apply_st, db_apply_end, diflag, _2d_flag);	
		...
		if (diflag == 0) {
			...
			db_apply_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
			if (timeout_1 < 0) {
				pr_emerg("DI_Enable time out, IP_en timeout=%d, db_st=%d, db_end=%d, diflag=%d, 2dflag=%d,\n", 
					timeout_1, db_apply_st, db_apply_end, diflag, _2d_flag);
			}
			pr_info("DI_Enable un-f2D done IP_en,t_0=%d, set_di_sta_90k=%d, db_st=%d, db_end=%d, diflag=%d, 2dflag=%d,\n", 
				timeout_1, set_di_sta_90k, db_apply_st, db_apply_end, diflag, _2d_flag);	
...
//					if(Scaler_DispGetInputInfo(SLR_INPUT_CAP_WID) >= 960)
			...
			{   //sync from pacific
				...
				IoReg_Write32(DI_IM_DI_CONTROL_reg, im_di_control_reg.regValue);
				Scaler_DispSetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_MA3A,TRUE);
				printk("timli di Memory Main 3A %s,%d \n",__FUNCTION__,__LINE__);
			}
				else //motion adaptive 5A or 3A
				{	// 5A
				drv_memory_set_ip_fifo(1);
				...
				IoReg_Write32(DI_IM_DI_CONTROL_reg, im_di_control_reg.regValue);
				Scaler_DispSetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_MA3A,FALSE);
				printk("timli di Memory Main 5A %s,%d \n",__FUNCTION__,__LINE__);
			}
#endif //#ifdef CONFIG_ENABLE_VD_27MHZ_MODE
		}
		else {
			...
			IoReg_Write32(DI_IM_DI_CONTROL_reg, im_di_control_reg.regValue);
			//printk("\nSet RTNR Memory\n");
			printk("timli di RTNR Memory %s,%d \n",__FUNCTION__,__LINE__);
		}
			...
			if(vgip_chn1_ctrl_reg.ch1_digital_mode) //digital mode need to set porch
			{
				...
				else//hdmi case
				{
					...
					if(Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) > (Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) + VGIP_H_PORCH_MARGIN)){
						...
						{
							vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE);
							pr_notice("The H porch number is smaller than act start!!!!\n");
						}
					}
					else{
						vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE);
