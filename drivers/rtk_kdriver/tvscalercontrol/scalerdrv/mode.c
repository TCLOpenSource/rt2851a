	...
	if(_ENABLE == wdgenable)
	{
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_YPBPR  || src == _SRC_VGA){//for K3LG-963 rzhen@2016-09-09
				...
				if(src == _SRC_VGA){
					onms_onms1_watchdog_en_reg.on1_wd_hs_per_over_range = _ENABLE;//bit14
					onms_onms1_watchdog_en_reg.on1_wd_hs_per_overflow = _ENABLE;//bit10
				}
			}
			...
			if (src == _SRC_HDMI) {
				...
				if(drvif_Hdmi_GetVRREnable() || vbe_disp_get_freesync_mode_flag())//VRR case input is hdmi 2.1 need HDMI provide the api
				{
					onms_onms1_watchdog_en_reg.on1_wd_vs_per_over_range = _DISABLE; //VSYNC_OVER_RANGE Watch-Dog Enable
	...
	if(_ENABLE == interruptenable)
	{
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_YPBPR || src == _SRC_VGA){//for K3LG-963 rzhen@2016-09-09
				...
				if(src == _SRC_VGA){
					onms_onms1_interrupt_en_reg.on1_ie_hs_per_over_range = _ENABLE;//bit14
					onms_onms1_interrupt_en_reg.on1_ie_hs_per_overflow = _ENABLE;//bit10
				}
			}
...
#ifdef HDMI_VRR_ONLINEMEASURE_SUPPORT
				if(drvif_Hdmi_GetVRREnable() || vbe_disp_get_freesync_mode_flag())//vrr caseinput is hdmi 2.1 need HDMI provide the api
				{
...
//						onms_onms3_hsync_meas_ctrl_reg.on3_htotal_max_delta_new = 1;
	...
	if(_ENABLE == wdgenable)
	{
		...
		if (src == VSC_INPUTSRC_ADC) {//for K3LG-963 rzhen@2016-09-09
			...
			if(get_ADC_Input_Source() == _SRC_VGA){
				onms_onms3_watchdog_en_reg.on3_wd_hs_per_over_range = _ENABLE;//bit14
				onms_onms3_watchdog_en_reg.on3_wd_hs_per_overflow = _ENABLE;//bit10
			}
		}
	...
	if(_ENABLE == interruptenable)
	{
			...
			if (src == VSC_INPUTSRC_ADC) {//for K3LG-963 rzhen@2016-09-09
				...
				if(get_ADC_Input_Source() == _SRC_VGA){
					onms_onms3_interrupt_en_reg.on3_ie_hs_per_over_range = _ENABLE;//bit14
					onms_onms3_interrupt_en_reg.on3_ie_hs_per_overflow = _ENABLE;//bit10
				}
			}
...
//			Scaler_MEMC_Bypass_CLK_OnOff(1);
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
	 ...
	 if(get_platform_model() == PLATFORM_MODEL_2K) {
	 	extern unsigned int mdomain_2k_memory_addr; 
		if(mdomain_2k_memory_addr) {
			pli_free(mdomain_2k_memory_addr);
			mdomain_2k_memory_addr = 0;
		}
	 }
	// Disable Sample Clock in VGIP
...
{//this api is for wait online measure ready to pop frame rate result. frame_rate is *10
	...
	{
		pr_err("##func:%s err frame rate zero###\r\n", __FUNCTION__);
		return;
	}

	pr_notice("###func:%s display:%d ori:0x%x needwait time:0x%x##\r\n", __FUNCTION__, display, enable_time, need_time);
	...
	if(!timeout)
		pr_err("###func:%s [display:%d] err wait timeout ###\r\n", __FUNCTION__, display);
}
