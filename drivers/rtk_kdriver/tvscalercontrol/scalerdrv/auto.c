...
static void drvif_get_h_boundary(unsigned char display)
{
	...
	if(get_ADC_Input_Source() == _SRC_VGA){
		...
		rtd_printk(KERN_EMERG, TAG_NAME_ADC,"#####[%s(%d)] IHSyncPulseCount = %d\n", __FUNCTION__, __LINE__, IHSyncPulseCount);
...
MEASURE_V_POS:
	...
	*v_end = smartfit_auto_result_vsta_end_reg.vx_end;
	...
	if((*v_end== 0x0000)||(*v_end<=*v_start))
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xb8022004=0x%08x, 0xb8022008=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb8022004),IoReg_Read32(0xb8022008));
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xb802200c=0x%08x, 0xb8022010=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb802200c),IoReg_Read32(0xb8022010));
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xb8022014=0x%08x, 0xb8022018=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb8022014),IoReg_Read32(0xb8022018));
		if(vcnt++ > 5){//timeout
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]ABORT due to vcnt=%d\n", __func__, __LINE__, vcnt);
			return  ERROR_ABORT;
		}
...
MEASURE_H_POS:
	...
	*h_end = smartfit_auto_result_hsta_end_reg.hx_end;
	...
	if((*h_end == 0x0000)||(*h_end <= *h_start))
	{
		...
		if(hcnt++ > 5){//timeout
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]ABORT due to hcnt=%d\n", __func__, __LINE__, hcnt);
			return  ERROR_ABORT;
		}
...
unsigned int get_sod_software_auto_phase(unsigned short phase)
{
	...
	do{
		...
		if(smartfit_auto_adj_reg.now_af == 0){
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)] finished, timeout=%d!\n",__func__,__LINE__,timeout);
			break;
		}
	...
	if(timeout == 0){
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)] timeout error, 0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
	}else{
...
//	auto_soy_soych0_a4_RBUS autosoy_soych0_a4_reg;
	...
	if(HW_SW_sel == Hardware_Auto_Phase){
		...
		IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg, smartfit_auto_phase_ctrl0_reg.regValue);
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]Hardware_Auto_Phase, 0xb8020424=0x%08x",__func__,__LINE__,smartfit_auto_phase_ctrl0_reg.regValue);

	}else if(HW_SW_sel == Software_Auto_Phase){
		smartfit_auto_phase_ctrl0_reg.autophase_en = HW_SW_sel;
		IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg, smartfit_auto_phase_ctrl0_reg.regValue);
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]Software_Auto_Phase, 0xb8020424=0x%08x",__func__,__LINE__,smartfit_auto_phase_ctrl0_reg.regValue);
	...
	smartfit_auto_adj_reg.now_af = 1;
	IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg, smartfit_auto_adj_reg.regValue);
	rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]Sel_Mode=%d,0xb8020410=0x%08x",__func__,__LINE__,Sel_Mode,smartfit_auto_adj_reg.regValue);
...
static unsigned char ypp_auto_phase_waitfinish(unsigned char display)
{
	...
	do {
		smartfit_auto_adj_reg.regValue = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg);
		if(smartfit_auto_adj_reg.now_af == 0){
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]auto phase finished, Wait_Time_Cnt=%d!\n",__func__,__LINE__,Wait_Time_Cnt);
			break;
		}

		if(get_ADC_Input_Source() == _SRC_VGA){
			if(vga_clock_check_onlinemeasure_status(display) == FALSE){
				rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]VGA ERROR_ABORT##########\n",__func__,__LINE__);
				return ERROR_ABORT;
			}
		}else{
			if(judge_scaler_break_case(display)){
				rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]ERROR_ABORT##########\n",__func__,__LINE__);
				return ERROR_ABORT;
			}
		}
	...
	if(Wait_Time_Cnt == 0){
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]auto phase timeout, 0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
    }
...
unsigned char auto_phasesearch_WaitFinish(unsigned char *a_pucPhaseResult)
{
	...
	if (ERROR_SUCCEED != Result){
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xb8020424=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg));
		return ERROR_ABORT;
	}
...
unsigned char drvif_ypbpr_auto_phasedo_WaitFinish(unsigned char display)
{
	...
	if (ERROR_SUCCEED == Result)
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)] auto_phasesearch_WaitFinish success\n",__func__,__LINE__);
	else
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)] auto_phasesearch_WaitFinish  fail\n",__func__,__LINE__);
...
void OTP_Initial_Setting(void)
{
    ...
    //rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xB802000C=0x%08x\n", __func__,__LINE__,IoReg_Read32(ADC_ADC_VBIAS_reg));
    ...
    //rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xB8020024=0x%08x\n", __func__,__LINE__,IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg));
	...
	//rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]0xB8020104=0x%08x\n", __func__,__LINE__,IoReg_Read32(DDS_PLL_PLL_CTRL_reg));
}
