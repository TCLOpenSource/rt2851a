...
#include "tvscalercontrol/util/rtk_util_tm.h"
...
extern void fwif_color_check_maxrgb_histirq(void);
extern void scaler_set_force_BG_Status(unsigned char enable);
...
/**********************************************************************************
    As you can see it's done after UZU vsync, we'll wait @ UZU DEN STA

    use busy loop to check the cond true line cnt:

    VO                  VGIP       DTG  UZU                 MEMC
   928(22,1102,4095),  923(   0),   65, 2248( 0,2160,2249), 2248(45,2205,2249)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    10(22,1102,4095),    5(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2252( 0,2160,2251), 2251(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2252( 0,2160,2251), 2251(45,2205,2251)
    10(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2251( 0,2160,2251), 2250(45,2205,2251)
    11(22,1102,4095),    6(   0),    0, 2252( 0,2160,2251), 2251(45,2205,2251)
...
RUTIL_ISR_WAIT_ADV_ADD_COND(ISR_WAIT_MV_DEN_STA, ISR_WAIT_UNMUTE_DONE, NULL
    , // condition for ISR to check if finish
        return (0 == PPOVERLAY_Double_Buffer_CTRL3_get_dmainreg_other_dbuf_set(IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg)))

    , // code blk to execute if condition is true, the pUsrParam was passed as a parameter
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: ISR true");

    , // code blk to execute if condition is false, the pUsrParam was passed as a parameter
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: ISR 0   ");
);

RUTIL_ISR_WAIT_ADV_WAIT_FUNCTION(ISR_WAIT_MV_DEN_STA, ISR_WAIT_UNMUTE_DONE
    ...
    , // code blk to pre check, you may add codes to decide to return immediately or enter the while loop
            ...
            RUTIL_ISR_WAIT_CS_WAIT_ISR_COND_TRUE(eRUtilTM_VSC_Unmute, ISR_WAIT_MV_DEN_STA, "ISR_WAIT_UNMUTE_DONE: done");
            goto ISR_Wait_Return;
        #endif

    , // code blk if ISR finished (condition check is true)
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: Finished");
    ...
    , // code blk if ISR returned but not finished (condition check is false), jump to label ISR_Wait_Cancel_Then_Return if you want to cancel the process
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: Not yet ");

     , // code blk if ISR timed out
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: isr gg  ");

    , // code blk if loop count timed out
        //RUTIL_TM_DUMP_LINE_INFO(RUTIL_ISR_WAIT_ISR_CNT(ISR_WAIT_MV_DEN_STA), "ISR_WAIT_UNMUTE_DONE: while gg");
...
void drvif_scalerdisplay_wait_d0_apply(unsigned char display)
{
	...
	if(SLR_MAIN_DISPLAY == display){
        ...
        // 1. usually this should be done within one frame, here we wait 10 * 1 uzu frames
        // 2. each ISR should return at around one memc_v_period_in_us, we set the timeout of each wait to be 3 * Scaler_get_uzu_v_period_in_us
        RUTIL_ISR_WAIT_ADV_WAIT(ISR_WAIT_MV_DEN_STA, ISR_WAIT_UNMUTE_DONE, 10 * 1, 3 * Scaler_get_uzu_v_period_in_us(SLR_MAIN_DISPLAY));
	}else{//for sub
...
//#endif
  ...
  else
  {
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		/*
		if(Scaler_InputSrcGetMainChType()==_SRC_VO){
			printk("[%s][%d]disable FBG!!\n", __FUNCTION__, __LINE__);
			fwif_color_check_maxrgb_histirq();
		}
		*/
		//apply double buffer
		drvif_scalerdisplay_wait_d0_apply(channel);
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"####RTK Disable Main ForceBG (90K=%d)!!!####\r\n", IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
...
//#endif
	...
	scaler_set_force_BG_Status(enable);
}
