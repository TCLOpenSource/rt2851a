...
//	unsigned int  TotalSizeFrame=0;
	...
	if(((vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY))||(m_dispwin.srcy <= 100)) && (!drv_memory_get_vdec_direct_low_latency_mode())) {
 ...
 *
 */
 unsigned int mdomain_2k_memory_addr = 0;
...
//	unsigned char lineRepeatEnable = FALSE;
    ...
    //mdomain_cap_ddr_in1linestep_RBUS mdomain_cap_ddr_in1linestep_reg;
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...
#endif
		{
			...
			if(dvrif_memory_compression_get_enable(display)==_ENABLE) {
				...
				else if((display == SLR_MAIN_DISPLAY) && Get_rotate_function(SLR_MAIN_DISPLAY))
				{
			...
			if(0==is_2k_memory_buffer_source(SLR_MAIN_DISPLAY))
			{
                ...
                {
                    pr_err("[%s %d]ERROR memory is not enough\n", __FUNCTION__, __LINE__);
                }
			}
			...
			if(get_platform_model() == PLATFORM_MODEL_2K) {
				if(!query_addr)
				{
					...
					{
						pr_emerg("mdomain 2k pli alloc memory fail!\n");
						return;
					}
			...
			{//4k
				...
				{
					printk(KERN_ERR "##[ERR] 4k no memory cap##\r\n");
					return; 
				}
			}
			else{
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
...
#endif
		{
			...
			{//4k
				if(i3ddmaCtrl.cap_buffer[0].phyaddr)
				{
					MemTag[MEMIDX_MAIN_SEC].StartAddr = drvif_memory_get_data_align(i3ddmaCtrl.cap_buffer[0].phyaddr, DMA_SPEEDUP_ALIGN_VALUE);
					MemTag[MEMIDX_MAIN].StartAddr = drvif_memory_get_data_align(i3ddmaCtrl.cap_buffer[0].phyaddr + i3ddmaCtrl.cap_buffer[0].size, DMA_SPEEDUP_ALIGN_VALUE);
				}
				else
				{
					printk(KERN_ERR "##[ERR] 4k no memory size##\r\n");
					return; 
				}
			}
			else{
...
void drvif_memory_compression_rpc(unsigned char display){
	...
	mdomain_compression->RGB444Mode =rtk_hal_vsc_GetRGB444Mode();
...
unsigned char drv_memory_get_di_PQC_timing(unsigned char src_idx, unsigned char access, unsigned int DI_Width, unsigned int DI_Height, unsigned char index)
{
	...
	if (access == SLR_DI_3A_mode)
		...
		rtd_printk(KERN_INFO, TAG_VPQ_MEM, "get_di_PQC_timing, input error, access=%d, DI_Width=%d, DI_Height=%d \n", access, DI_Width, DI_Height);		
	
	rtd_printk(KERN_INFO, TAG_VPQ_MEM, "get_di_PQC_timing,timing=%d, access=%d, DI_Width=%d, DI_Height=%d \n", timing, access, DI_Width, DI_Height);
	return timing;
}
...
unsigned char drv_memory_di_Decide_PQC_mode(unsigned char src_idx, unsigned char access, unsigned int DI_Width, unsigned int DI_Height, unsigned char DI_DMA_Mode, unsigned char index)
{
	...
	rtd_printk(KERN_INFO, TAG_VPQ_MEM, "di_Decide_PQC_mode,PQC_Mode=%d,\n", PQC_Mode);
	return PQC_Mode;
}
...
unsigned int drv_GameMode_get_snr_vstart(void)
{
	...
	if(!snr_result1)
	{
		printk(KERN_ERR "\r### func:%s error snr_result1no result ### \r\n",__FUNCTION__);
...
//	ppoverlay_fs_iv_dv_fine_tuning5_RBUS fs_iv_dv_fine_tuning5_reg;
	...
	if(!get_force_i3ddma_enable(SLR_MAIN_DISPLAY))
	{
		...
		{
			printk(KERN_EMERG"[IV2DV]Invalid HDMI Info!!!!\n");
			return 0;
		}
	}
	else if((pVOInfo == NULL) || (pVOInfo->h_total == 0)){
		printk(KERN_EMERG"[IV2DV]Invalid VOInfo!!!!\n");
		return 0;
	}
...
void drv_memory_capture_multibuffer_update_addr(unsigned int buffer_size, unsigned int start_addr){
	...
	if((drv_memory_Get_multibuffer_number() > 3) && MemTag[MEMIDX_MAIN_FOURTH].StartAddr){
		...
		#ifdef CONFIG_ARM64
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FOURTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_FOURTH].StartAddr);
		#else
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FOURTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FOURTH].StartAddr);
		#endif
	}
	...
	if((drv_memory_Get_multibuffer_number() > 4) && MemTag[MEMIDX_MAIN_FIFTH].StartAddr){
		...
		#ifdef CONFIG_ARM64
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FIFTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
		#else
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FIFTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
		#endif
	}
	...
	if((drv_memory_Get_multibuffer_number() > 5) && MemTag[MEMIDX_MAIN_SIXTH].StartAddr){
		...
		#ifdef CONFIG_ARM64
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SIXTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
		#else
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SIXTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
		#endif
	}
	...
	if(drv_memory_Get_multibuffer_number() > 6 && MemTag[MEMIDX_MAIN_SEVENTH].StartAddr){
		...
		#ifdef CONFIG_ARM64
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_SEVENTH].StartAddr);
		#else
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SEVENTH].StartAddr);
		#endif
	}
	...
	if((drv_memory_Get_multibuffer_number() > 7) && MemTag[MEMIDX_MAIN_EIGHTH].StartAddr){
		...
		#ifdef CONFIG_ARM64
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_EIGHTH].StartAddr);
		#else
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_EIGHTH].StartAddr);
		#endif
	}
}
...
void drv_memory_display_multibuffer_switch_buffer_number(void)
{
	...
	printk("disp Captblock=%d,Dispblock=%d, buf_num=%d\n",mdomain_cap_ddr_in1status_reg.in1_block_select,mdomain_disp_ddr_mainstatus_reg.main_block_select,buf_num);
}
...
{
	unsigned int buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_MULT4K_12BITS;
	unsigned int nStartAddr = 0;
	...
	{
		// buffer[0]
		nStartAddr = (unsigned int)dvr_malloc_uncached_specific(buffer_size, GFP_DCU2_FIRST, (void **)&uncache);//(unsigned int)dvr_malloc(size_metadata*2);//for double buffer use
		if(nStartAddr == (unsigned int)NULL)
		{
			pr_emerg("[MultiBuf]video delay MEMIDX_MAIN_FOURTH memory failed[%x]\n", nStartAddr);
			return 1; //malloc memory fail
		}
		MemTag[MEMIDX_MAIN_FOURTH].Allocate_VirAddr = (unsigned int)(nStartAddr);
		//16 bytes alignment
		MemTag[MEMIDX_MAIN_FOURTH].StartAddr = (unsigned int)dvr_to_phys((void*)MemTag[MEMIDX_MAIN_FOURTH].Allocate_VirAddr);
		MemTag[MEMIDX_MAIN_FOURTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_FOURTH].StartAddr;
		MemTag[MEMIDX_MAIN_FOURTH].Size = buffer_size;
		MemTag[MEMIDX_MAIN_FOURTH].Status = ALLOCATED_FROM_PLI;
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FOURTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FOURTH].StartAddr);
	...
	{
		// buffer[0]
		nStartAddr = (unsigned int)dvr_malloc_uncached_specific(buffer_size, GFP_DCU1_FIRST, (void **)&uncache);//(unsigned int)dvr_malloc(size_metadata*2);//for double buffer use
		if(nStartAddr == (unsigned int)NULL)
		{
			pr_emerg("[MultiBuf]video delay MEMIDX_MAIN_FIFTH memory failed[%x]\n", nStartAddr);
			return 1; //malloc memory fail
		}
		MemTag[MEMIDX_MAIN_FIFTH].Allocate_VirAddr = (unsigned int)(nStartAddr);
		//16 bytes alignment
		MemTag[MEMIDX_MAIN_FIFTH].StartAddr = (unsigned int)dvr_to_phys((void*)MemTag[MEMIDX_MAIN_FIFTH].Allocate_VirAddr);
		MemTag[MEMIDX_MAIN_FIFTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_FIFTH].StartAddr;
		MemTag[MEMIDX_MAIN_FIFTH].Size = buffer_size;
		MemTag[MEMIDX_MAIN_FIFTH].Status = ALLOCATED_FROM_PLI;
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_FIFTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
	...
	if(MemTag[MEMIDX_MAIN_SIXTH].Allocate_VirAddr == 0)
	{
		...
		if(nStartAddr == (unsigned int)NULL)
		{
			pr_emerg("[MultiBuf]video delay MEMIDX_MAIN_SIXTH memory failed[%x]\n", nStartAddr);
			return 1; //malloc memory fail
		}
		MemTag[MEMIDX_MAIN_SIXTH].Allocate_VirAddr = (unsigned int)(nStartAddr);
		//16 bytes alignment
		MemTag[MEMIDX_MAIN_SIXTH].StartAddr = (unsigned int)dvr_to_phys((void*)MemTag[MEMIDX_MAIN_SIXTH].Allocate_VirAddr);
		MemTag[MEMIDX_MAIN_SIXTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_SIXTH].StartAddr;
		MemTag[MEMIDX_MAIN_SIXTH].Size = buffer_size;
		MemTag[MEMIDX_MAIN_SIXTH].Status = ALLOCATED_FROM_PLI;
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SIXTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
	...
	if(MemTag[MEMIDX_MAIN_SEVENTH].Allocate_VirAddr == 0)
	{
		...
		if(nStartAddr == (unsigned int)NULL)
		{
			pr_emerg("[MultiBuf]video delay MEMIDX_MAIN_SEVENTH memory failed[%x]\n", nStartAddr);
			return 1; //malloc memory fail
		}
		MemTag[MEMIDX_MAIN_SEVENTH].Allocate_VirAddr = (unsigned int)(nStartAddr);
		//16 bytes alignment
		MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = (unsigned int)dvr_to_phys((void*)MemTag[MEMIDX_MAIN_SEVENTH].Allocate_VirAddr);
		MemTag[MEMIDX_MAIN_SEVENTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_SEVENTH].StartAddr;
		MemTag[MEMIDX_MAIN_SEVENTH].Size = buffer_size;
		MemTag[MEMIDX_MAIN_SEVENTH].Status = ALLOCATED_FROM_PLI;
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SEVENTH].StartAddr);
	...
	if(MemTag[MEMIDX_MAIN_EIGHTH].Allocate_VirAddr == 0)
	{
		...
		if(nStartAddr == (unsigned int)NULL)
		{
			pr_emerg("[MultiBuf]video delay MEMIDX_MAIN_EIGHTH memory failed[%x]\n", nStartAddr);
			return 1; //malloc memory fail
		}
		MemTag[MEMIDX_MAIN_EIGHTH].Allocate_VirAddr = (unsigned int)(nStartAddr);
		//16 bytes alignment
		MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = (unsigned int)dvr_to_phys((void*)MemTag[MEMIDX_MAIN_EIGHTH].Allocate_VirAddr);
		MemTag[MEMIDX_MAIN_EIGHTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_EIGHTH].StartAddr;
		MemTag[MEMIDX_MAIN_EIGHTH].Size = buffer_size;
		MemTag[MEMIDX_MAIN_EIGHTH].Status = ALLOCATED_FROM_PLI;
		printk(KERN_EMERG"[MultiBuf]MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_EIGHTH].StartAddr);
...
{	
	...
	for (i = MEMIDX_MAIN_FOURTH; i <= MEMIDX_MAIN_EIGHTH; i++) {
		if (MemTag[i].Allocate_VirAddr)
		{
			...
			MemTag[i].Size = 0;
			MemTag[i].Status = UNDEFINED;
			printk(KERN_EMERG"[MultiBuf]free MemTag[%d]\n",i);
		}
	}
	return 0;
}
...
unsigned int calc_compress_memory_size(SCALER_DISP_CHANNEL display, MemCompression_Bits eCompressBit)
{
    ...
    unsigned int ulCapWid = 0;
    unsigned int ulCapLen = 0;
    ...
    pr_info("[%s %d]ulCapWid:%d, ulCapLen:%d, eCompressBit:%d, ulCompressMemorySize:0x%x\n", __FUNCTION__, __LINE__, ulCapWid, ulCapLen, eCompressBit, ulCompressMemorySize);
