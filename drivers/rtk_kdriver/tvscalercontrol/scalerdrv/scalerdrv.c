...
//	vgip_chn1_3d_right_act_vsta_length_RBUS vgip_chn1_3d_right_act_vsta_length_reg;
	...
	{//vdec mode 
#ifdef CONFIG_VGIP_DIGITAL_MODE
		extern unsigned char vdec_get_vstart_shift(unsigned int *shift);
		unsigned int vstart = 0;
		unsigned char need_shift = false;
		need_shift = vdec_get_vstart_shift(&vstart);
		vgip_chn1_ctrl_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg);
...
RUTIL_ISR_WAIT_ADV_WAIT_FUNCTION(ISR_WAIT_MV_DEN_STA, ISR_WAIT_I2D_DONE
    ...
    , // code blk if ISR timed out
        //rtd_printk(KERN_NOTICE, TAG_NAME_VBE, "ISR_WAIT_I2D_DONE: ISR time out\n");
...
//  rtd_printk(KERN_NOTICE, TAG_NAME_VSC, "PPOVERLAY_I2D_CTRL_0_reg = %x \n", IoReg_Read32(PPOVERLAY_I2D_CTRL_0_reg));
    ...
    RUTIL_ISR_WAIT_ADV_WAIT(ISR_WAIT_MV_DEN_STA, ISR_WAIT_I2D_DONE, 3 * 8, 3 * Scaler_get_uzu_v_period_in_us(display));
...
void drivf_scaler_reset_freerun()
{
	...
	if(VODMA_VODMA_CLKGEN_get_en_fifo_full_gate(IoReg_Read32(VODMA_VODMA_CLKGEN_reg))){
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] disable gatting\n", __FUNCTION__);
...
unsigned char drivf_i3ddma_config(unsigned char display, unsigned char src)
{
	...
	else
	{
        ...
        if(rtk_hal_vsc_GetRGB444Mode() && ((src == VGIP_SRC_TMDS) || (src == VGIP_SRC_ADC))){
...
{//only for vtop mode setting for scaler
	...
	printk("timli,scaler_vtop_dolby_mode_setting() hdrtype=%d \n",hdrtype);
	switch(hdrtype)
	{
...
void scaler_hdr_mode_setting(unsigned char display, unsigned char hdrtype)
{
	...
	printk("timli,scaler_hdr_mode_setting() hdrtype=%d,display=%d \n",hdrtype,display);
...
{//TRUE: use two step UZU not normal uzu   2k 120 case
	if((Get_DISPLAY_REFRESH_RATE() == 120) &&(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) >= 605))
		return TRUE;
	else
		return FALSE;
}
...
void Hdmi_Av_Out_Disable_Vsc_Pre_Action(void)
{
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE))
	{
		...
		{
			Set_Vsc_Force_Timing_Freerun(FALSE);
			printk(KERN_ERR "###func:%s wait timeout total time:0x%x ms####\r\n",__FUNCTION__, cost_time/90);
		}
		else
		{
			printk(KERN_INFO "###func:%s wait total time:0x%x ms####\r\n",__FUNCTION__, cost_time/90);
		}
	}
}
