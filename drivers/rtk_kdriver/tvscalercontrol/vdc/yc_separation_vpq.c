...
void drvif_module_vpq_YcSeparation(unsigned char ucInputSrc, unsigned char uc3DEnable)
{
	...
	if( ucDMATestTimeCnt == 0 )
	{
		...
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] dma check status = %d, FIFO = %x\n", __func__, ucDMATestResult, ucDMATestFIFOStatus);
	}
	...
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] InputSrc = %d, YcStatus = %d, InputFormat = %d\n", __func__, ucCurInputAVTV, ucCurStatus, ucCurFormat);
	...
	if( ucDebugPreStatus != ucCurStatus )
	{
		...
		{
			rtd_printk(KERN_INFO, TAG_NAME, "[VDPQ_INTERFACE] Status change, CurStatus = %d, PreStatus =%d\n", ucCurStatus, ucDebugPreStatus);
		}
	}
	...
	if( ucDebugPreFormat != ucCurFormat)
	{
		...
		if(ucDebugPrint == _ENABLE)
		{
			rtd_printk(KERN_INFO, TAG_NAME, "[VDPQ_INTERFACE] Format change, CurFormat = %d, PreFormat =%d \n", ucCurFormat, ucDebugPreFormat);
		}
	}
...
void module_vpq_dma_check_alive(unsigned char ucWrtClearTimes, unsigned char *ucRetResult, unsigned char *ucRetFIFOStatus )
{
	...
	*ucRetFIFOStatus = 0;
	...
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] Mem/Arb (%d %d %d) is off, cannot check\n",
...
void drvif_module_vpq_DmaAccessCtrl(unsigned char on_off)
{
	if (on_off == ON)
	{
		...
		vdcf_outl(VDPQ_DMA_CTRL5_reg, vdpq_dma_ctrl5_reg.regValue);
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] set ON\n", __func__);
	}
	else if (on_off == OFF)
	{
		vdpq_dma_ctrl5_RBUS vdpq_dma_ctrl5_reg; /* 1_9850 */
		...
		vdcf_outl(VDPQ_DMA_CTRL5_reg, vdpq_dma_ctrl5_reg.regValue);
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] set OFF\n", __func__);
	}
}
...
void drvif_module_vpq_3D_mem_Initial(unsigned int mem_addr)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] Initial set addr = %x, _SDRAM_3D_ONE_FRAME_USE = %x\n", __func__, nRegAddr, _SDRAM_3D_ONE_FRAME_USE);
