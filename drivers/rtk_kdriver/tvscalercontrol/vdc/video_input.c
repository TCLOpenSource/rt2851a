...
#include <tvscalercontrol/scaler_vfedev.h> //ryanliao 2018/09/27
...
unsigned char drvif_AV_Detect_Signal_PlugInOut(unsigned char source)
{
	...
	sys_clksel_reg.atvin_clk_sel = 1;
	...
	vd_adc_clamp_ctl_1_reg.regValue = vdcf_inl(ADC_VD_ADC_CLAMP_CTL_1_reg);
	vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_10_9 = 1;
	vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_13 = 1;
	vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_15 = 0;
	rtdf_outl(ADC_VD_ADC_CLAMP_CTL_1_reg,vd_adc_clamp_ctl_1_reg.regValue);

	vd_adc_clamp_ctl_2_reg.regValue = vdcf_inl(ADC_VD_ADC_CLAMP_CTL_2_reg);
	vd_adc_clamp_ctl_2_reg.vdadc_clamp_ctrl2_9 = 0x01;
	vd_adc_clamp_ctl_2_reg.vdadc_clamp_ctrl2_6 = 0x0;
	rtdf_outl(ADC_VD_ADC_CLAMP_CTL_2_reg,vd_adc_clamp_ctl_2_reg.regValue);

	vd_adc_updn_ctl_reg.regValue = vdcf_inl(ADC_VD_ADC_UPDN_CTL_reg);
	vd_adc_updn_ctl_reg.vdadc_updn_ctrl0_0 = 1;
	vd_adc_updn_ctl_reg.vdadc_updn_ctrl0_2 = 0;
	vd_adc_updn_ctl_reg.vdadc_updn_ctrl0_6_3 = 0x2;
	rtdf_outl(ADC_VD_ADC_UPDN_CTL_reg,vd_adc_updn_ctl_reg.regValue);

	vdadc_pow_ctl_reg.regValue = vdcf_inl(ADC_VDADC_POW_CTL_reg);
...
void module_vdc_PathSelect(unsigned char ucSourceIndex, void* stPath)
{
	...
	if (ucSourceIndex != _SRC_TV) {
		...
		sys_clksel_reg.regValue = rtdf_inl(SYS_REG_SYS_CLKSEL_reg);
