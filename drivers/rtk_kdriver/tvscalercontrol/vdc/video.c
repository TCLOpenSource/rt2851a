...
void drvif_module_vdc_Cpeak_set(unsigned char on_off)
{
	...
	//rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[VDC] drvif_module_vdc_Cpeak_set %d   %d  ", on_off, chroma_agc_peak_ctrl_reg.chroma_peak_en);
...
//					}
			...
			if (module_vdc_read_burst_win_error_flag() == 1) {
				...
				if (vdc_NoiseStatus > 0x70 && drvif_video_status_reg(VDC_Lines_625_detected) == 0) { //fix fluke<30db mode dectet and no color, RL
					...
					rtd_printk(KERN_EMERG,TAG_NAME_VDC,"weak_signal case6 improve\n");
				}
#endif
			}
#endif
	...
	if(ucInputSrc == _SRC_TV){
		...
		if ((vdcf_inl(VDTOP_PEAK_VSAG_reg) & 0x00001FFF) > 0x100 && drvif_video_status_reg(VDC_Lines_625_detected) == 0) {
			...
			rtd_printk(KERN_EMERG,TAG_NAME_VDC,"weak_signal case10 Vsync issue, and 625flag old V change to new V detection\n");
		}else{
			drvif_module_vdc_vline_625_src_set(_DISABLE);
		}
...
unsigned char module_video_check_Hsync_power_2(unsigned char mode)
{
       ...
       if (mode == VDC_MODE_NTSC) {
               ...
               VDC_CpeakSet = vd_cpeak_disable;
               //rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[cpeak_2] [mode == VDC_MODE_NTSC] NTSC 443_p=%x, 358_p=%x, c(%d) n(%d) ", uc443power, uc358power, color_count, normal_count);
       } else if (mode == VDC_MODE_PALI) {
               ...
               for (i = 0; i < 3; i++) {
                        ...
                        if ((uc358power < 0x50) && (uc443power < 0xf0) && (drvif_module_vdc_read_Cgain_data() >0x3ef0)){
                               color_count++;
                               //Rt_Delay(1);
                               //rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[cpeak_1] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());

                       }else if ((uc358power < 0x50) && (uc443power < 0xf0)){
                               color_count++;
                               //rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[cpeak_2] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());
                               //VDC_CpeakSet = vd_cpeak_enable;
                               break;
               ...
               //rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[cpeak_3] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());
       }

               if (color_count > 2) {
                       VDC_CpeakSet = vd_cpeak_enable;
                       color_count = 0;
                       //rtd_printk(KERN_EMERG,TAG_NAME_VDC, "[cpeak] VDC_CpeakSet :%x  %x", VDC_CpeakSet, vdc_LoopCount);
               }
