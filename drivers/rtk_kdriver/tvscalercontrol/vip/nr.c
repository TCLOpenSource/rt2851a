...
//TBD
	...
	if((Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP) == FALSE) && (Get_DisplayMode_Src(SLR_SUB_DISPLAY) == VSC_INPUTSRC_HDMI))// spnr cbypass mode
	{
...
void drvif_IESM_frontporch_set(unsigned char game_mode_check_flag)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "front_porch_num=vlen-actlen-actsta: %d=%d-%d-%d, size=%d*%d,lineMode=%d,fmt_422to444=%d\n",
		front_porch_num, Scaler_DispGetInputInfo(SLR_INPUT_V_LEN), Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN_PRE), Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_STA_PRE),
