...
unsigned char drvif_color_icm_Write3DTable_By_DMA(COLORELEM ICM_TAB_ACCESS[ITNSEGMAX][SATSEGMAX][HUESEGMAX], unsigned int addr, unsigned int* vir_addr)
{
	//color_d_vc_global_ctrl_RBUS	d_vc_global_ctrl_REG;
	...
	if (buf == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "ICM: buf PQ warning, drvif_color_icm_Write3DTable_By_DMA memory allocate fail!\n");
		return false;
	}
