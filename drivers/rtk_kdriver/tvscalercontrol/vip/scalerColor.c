...
extern unsigned int is_InPmode;
...
extern unsigned short auto_gamma_R[1025];
extern unsigned short auto_gamma_G[1025];
extern unsigned short auto_gamma_B[1025];
...
UINT16 F_Out_R[Bin_Num_Output_InvOutput_Gamma + 1], F_Out_G[Bin_Num_Output_InvOutput_Gamma + 1], F_Out_B[Bin_Num_Output_InvOutput_Gamma + 1];
...
extern unsigned char dlut_UI_en;
...
unsigned int Data_Range_HDR_YUV2RGB_TV030;
...
void fwif_color_setCon_Bri_Color_Tint_inYUV2RGB_from_OSD_Gain(unsigned char src_idx, unsigned char display,  _system_setting_info *system_info_struct)
{
	...
	printk("[vpq][%s][%d]contrast = %d, bri = %d, color = %d, deg_tint = %d\n", __FUNCTION__, __LINE__, contrast, bri, color, deg_tint);
	...
	//printk("[vpq][%s][%d]Input_mode = %d, YUV2RGB_TBL_Idx = %d, cmps = %d\n", __FUNCTION__, __LINE__, Input_mode, YUV2RGB_TBL_Idx, cmps);
	//printk("[vpq][%s][%d]tbl_offset_R = %d, RGB_Offset.R_offset = %d\n", __FUNCTION__, __LINE__, tbl_offset_R, drv_vipCSMatrix_t.RGB_Offset.R_offset);
	//printk("[vpq][%s][%d]tbl_offset_G = %d, RGB_Offset.G_offset = %d\n", __FUNCTION__, __LINE__, tbl_offset_G, drv_vipCSMatrix_t.RGB_Offset.G_offset);
	//printk("[vpq][%s][%d]tbl_offset_B = %d, RGB_Offset.B_offset = %d\n", __FUNCTION__, __LINE__, tbl_offset_B, drv_vipCSMatrix_t.RGB_Offset.B_offset);
...
void fwif_color_set_ColorMap_3x3_Table(unsigned char idx)
{
	...
	if(idx>=Color_Map_Table_NUM) {
		...
		//fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, ColorMap_Matrix_Apply, 1, 1, 1);
	}
}
...
void fwif_color_Icm_table_select_By_DMA(unsigned char display, unsigned short *tICM_ini, unsigned char which_table)
{
	...
	if (addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_Icm_table_select_By_DMA, addr = NULL\n");
		return;
	}
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		ret = drvif_color_icm_Write3DTable_By_DMA(icm_tab_elem_write.elem, addr, pVir_addr);
...
unsigned int TconOutputGamma_R[256]={
...
unsigned int TconOutputGamma_G[256]={
0,	16,	32,	48,	64,	80,	96,	112,	
128,	144,	160,	176,	192,	208,	224,	240,	
256,	272,	288,	304,	320,	336,	352,	368,	
384,	400,	416,	432,	448,	464,	480,	496,	
...
unsigned int TconOutputGamma_B[256]={
0,	16,	32,	48,	64,	80,	96,	112,	
128,	144,	160,	176,	192,	208,	224,	240,	
256,	272,	288,	304,	320,	336,	352,	368,	
384,	400,	416,	432,	448,	464,	480,	496,	
...
void fwif_color_gamma_data_reduce(UINT16 *Out_R, UINT16 *Out_G, UINT16 *Out_B, UINT16 *In_R, UINT16 *In_G, UINT16 *In_B)
{
	UINT16 i;
...
void fwif_set_ddomainISR_gamma(unsigned char Gamma_Mode, unsigned char Gamma_Sindex, unsigned char Gamma_Shigh, unsigned char Gamma_Slow)
{
	...
	fwif_set_gamma_multiply_remapOETF(GOut_R, GOut_G, GOut_B, GOut_R, GOut_G, GOut_B);

	/*3. process the gamma curve*/
	/*ColorSpace - curve for gamma*/
	printk(" [Gamma Flow]->set GraphicGamma,GammaBlendingFlag=%d\n",GammaBlendingFlag);
	if (GammaBlendingFlag==1) {
		drvif_color_inv_gamma_enable(SLR_MAIN_DISPLAY, 1, 1);
		fwif_set_gamma_multiply_remap(GOut_R, GOut_G, GOut_B, GOut_R, GOut_G, GOut_B);
		printk("gamma 0.45 !!!!\n");
	} else {
...
#if 0
unsigned char VIP_Skip_DI_RTNR_FRZ_ISR = 0;	// for tool control
unsigned char VIP_Skip_DI_RTNR_FRZ_TSK_P = 0;	// for tool control
char fwif_color_DI_RTNR_FRZ_ISR(void)
{
	_RPC_system_setting_info *vip_RPV_sysInfo = (_RPC_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_RPC_system_info_structure);

	if (vip_RPV_sysInfo != NULL) {
		return -1;
	}

	if (VIP_Skip_DI_RTNR_FRZ_ISR == 1)
		return 1;
	
	if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) {
		if (vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRZ_En == 1) {
			if (vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRCNT == 0) {
				rtd_printk(KERN_INFO, TAG_NAME, "DI_RTNR_FRZ, CNT=0\n");
				drvif_color_Set_di_FRZ_CNT_ISR(_MAIN_DISPLAY, 0);
				vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRZ_En = 0;
			} else {
				rtd_printk(KERN_INFO, TAG_NAME, "DI_RTNR_FRZ, En, CNT=%d,\n", vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRCNT);
				drvif_color_Set_di_FRZ_CNT_ISR(_MAIN_DISPLAY, 1);
				vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRCNT--;
			}
		}
	}
...
{
	_RPC_system_setting_info *vip_RPV_sysInfo = (_RPC_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_RPC_system_info_structure);
	unsigned int t_now, t_diff, t_smt;
	unsigned int t_th = 200 * 90; // 200 ms

	if (vip_RPV_sysInfo != NULL) {
		return -1;
	}

	if (VIP_Skip_DI_RTNR_FRZ_TSK_P == 1)
		return 1;

	t_smt = Scaler_ChangeUINT32Endian(vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.DI_RTNR_SMT_90k);
	...
	if ((t_diff > t_th) && (drvif_color_Get_di_FRZ() == 1)) {
		rtd_printk(KERN_INFO, TAG_NAME, "DI_RTNR_FRZ, freeze over than t_thm t_smt=%d,, t_now=%d,\n", t_smt, t_now);	
		down(get_DI_semaphore());
		drvif_color_Set_di_FRZ_CNT_ISR(_MAIN_DISPLAY, 0);
		up(get_DI_semaphore());
		vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRCNT = 0;
		vip_RPV_sysInfo->PQ_IP_Enable_Monitor_2.autoFRZ_En = 0;
	}
	return 0;
}
...
void fwif_color_SetDNR(unsigned char display, unsigned char level)
{
	...
	if (Scaler_Get_CinemaMode_PQ() && level == DRV_NR_OFF)
		fwif_color_SNR_flow(&(gVip_Table->Manual_NR_Table[Scaler_GetDNR_table()][DRV_NR_MID]));
}
...
void fwif_color_set_sharpness_table(unsigned char src_idx, unsigned char value)
{
	...
	memcpy(&sharp,&(gVip_Table->Ddomain_SHPTable[value]),sizeof(VIP_Sharpness_Table));
...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "ch=%d, Src=%d, nSD_HD=%d, HDMI_Range_Detect=%d, blk_lv=%d, force_run_i3ddma_en()=%d, IsHDMI()=%d, Hdmi_ColorSpace()=%d, isHDMIFullRangeYCC = %d, HDR_flag = %d, isBt2020_Vo = %d, prjID = %d\n",
	...
	if(pVOInfo != NULL){
		rtd_printk(KERN_INFO, TAG_NAME, "Hdmi_ModeInfo()=%d, HDMI_CSC_DataRange()=%d, IsRGB_YUV()=%d, EC=%d, colour_primaries=%d,InputInfo()=%d,VOFrom()=%d, Src()=%d, VO_colorspace = %d\n",
	...
	if (fwif_VIP_get_Project_ID() == VIP_Project_ID_TV006) {
		if ((VIP_RPC_system_info_structure_table->HDR_info.Ctrl_Item[TV006_HDR_En] == 0) || (channel == SLR_SUB_DISPLAY)) {		//20160107 roger, sub == non-HDR in K2L
			...
			if(get_support_vo_force_v_top(support_sdr_max_rgb)) {
				printk("MAXRGB: get_support_vo_force_v_top!!\n");
				...
				if (VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_ST2094) {
					...
					table_num = fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV030(channel, nSrcType, nSD_HD, true);
					printk("rgb2yuv table_num = %d \n",table_num);
					table_index = &(tRGB2YUV_COEF[table_num][0]);
				}
			}else{
...
#endif
	printk("rgb2yuv,display=%d,tab_num=%d,input_mode=%d,M+=%d,BOE=%d, M+_En=%d,RGB444=%d,Mode_src=%d,HDR_en=%d, HDR_csc=%d,\n",
	...
	if (NULL !=  table_index) {
...
#ifdef CONFIG_CUSTOMER_TV030
			table_num = fwif_color_get_I3DDMA_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV030(channel, nSrcType, nSD_HD, true);
		...
		if((VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_HLG)){
			if(channel == SLR_MAIN_DISPLAY)
			 	drvif_HDMI_HDR_I3DDMA_RGB2YUV_only_Matrix(tRGB2YUV_COEF[RGB2YUV_COEF_RGB_0_255_TO_YCC2020_0_255]);
		}
...
void fwif_color_set_YUV2RGB(unsigned char src_idx, unsigned char display, unsigned char which_table, unsigned char drvSkip_Flag_coefByY, unsigned char drvSkip_Flag_uvOffset)
{
	...
	unsigned char Input_mode;
	VIP_YUV2RGB_LEVEL_SELECT yuv2rgb_level = VIP_YUV2RGB_LEVEL_OFF;
	...
	if (VIP_YUV2RGB_LEVEL_HIGH == m_uv_offset_level && VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
		yuv2rgb_level = m_uv_offset_level;

	/*HDMI other use VIP_YUV2RGB_LEVEL_OFF as deault, set level by picture mode*/
	VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.RGB_Offset_Level = yuv2rgb_level;
	VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.UV_Offset_Level = yuv2rgb_level;
	VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.CoefByY_CtrlItem_Level = yuv2rgb_level;
	VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.YUV2RGB_TBL_Select = which_table;
	memcpy(&drv_vipCSMatrix_t, &(gVip_Table->YUV2RGB_CSMatrix_Table[which_table].YUV2RGB_CSMatrix[yuv2rgb_level]), sizeof(DRV_VIP_YUV2RGB_CSMatrix));
	YUV2RGB_TBL_Idx = fwif_color_get_Decide_YUV2RGB_TBL_Index(src_idx, display, Input_mode);
...
bool fwif_color_od_init(UINT8 color_depth_bits, UINT8 dma_mode, UINT8 freq)
{
	...
	if (OD_Y_RGB_mode == OD_RGB_Mode)//RGB mode
	{
		if (drvif_color_od_pqc(dispV, dispH, bit_sel, dma_mode, 6, 0, 1))
			bODInited = true;
		else
			return false;
	}
...
void fwif_set_od_setting_table(DRV_od_table_mode_t from_bin[])
{
	...
	for(i=0; i!= VIP_QUALITY_SOURCE_NUM; ++i)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]OD_setting_table[%d].type=%d,\n",i,OD_setting_table[i].type);
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]OD_setting_table[%d].gain=%d,\n",i,OD_setting_table[i].gain);
	}
...
void fwif_set_od_table(DRV_od_table_t* from_bin)
{
	...
	for(i=0; i!= OD_table_length; ++i)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]OD_table->table_deltamode[%d]=%d,\n",i,OD_table.table_deltamode[i]);
	}
	fwif_color_od_handler();
}
...
void fwif_set_od_table_test(unsigned char from_bin[])
{
	...
	for(i=0; i!= OD_table_length; ++i)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]m_pod_table_test[%d]=%d,\n",i,m_pod_table_test[i]);
	}
}
...
void fwif_set_DeMura(VIP_DeMura_TBL* from_bin)
{
	...
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_table_scale=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_table_scale);
	//rtd_printk(KERN_EMERG, TAG_NAME,"[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_block_size=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_block_size);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_rgb_table_seperate=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_rgb_table_seperate);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_en=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_en);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_r_upper_cutoff=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_r_upper_cutoff);
	//rtd_printk(KERN_EMERG, TAG_NAME,  "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_r_lower_cutoff=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_r_lower_cutoff);
	//rtd_printk(KERN_EMERG, TAG_NAME,  "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_r_m_level=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_r_m_level);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_r_l_level=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_r_l_level);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]DeMura_TBL.DeMura_CTRL_TBL.demura_r_h_level=%d,\n",DeMura_TBL.DeMura_CTRL_TBL.demura_r_h_level);
	//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH]m_pDeMura_TBL.DeMura_Adaptive_TBL=%d,\n",DeMura_TBL.DeMura_Adaptive_TBL.adaptiveScale_En);
	//for(i=0; i!= VIP_Demura_Adaptive_Scale_LV_NUM; ++i)
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]m_pDeMura_TBL.adaptiveScale[%d]=%d,\n",i,DeMura_TBL.DeMura_Adaptive_TBL.adaptiveScale[i]);
...
void fwif_set_VALC(unsigned char from_bin[6][17][17])
{
	...
	for(i=0; i!= 6; ++i)
	{
		for(j=0; j!= 17; ++j)
		{
			for(k=0; k!= 17; ++k)
			{
				...
				//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]m_pVALC_Table[i][j][k]=%d,\n",VALC_Table[i][j][k]);
			}
		}
	}
	...
	for(i=0; i!= 17; ++i)
	{
		rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]m_pVALC_Table=%d\n",VALC_Table[0][0][i]);
	}
	//R
...
void fwif_set_LEDOutGamma(unsigned short from_bin[])
{
	...
	for(i=0; i!= 257; ++i)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME, "[MHH][kernel]LEDOutGamma[%d]=%d,\n",i,LEDOutGamma[i]);
	}
...
unsigned char write_3dlut_base_table_time = 0xff;
void fwif_color_set_3dlut_table_run(void)
{
	if(write_3dlut_base_table_time == 0xff)
		return;
	
	write_3dlut_base_table_time++;
	if(write_3dlut_base_table_time > 4){
		write_3dlut_base_table_time = 0xff;
		fwif_color_WaitFor_DEN_STOP_UZUDTG();
		fwif_color_set_D_3dLUT(255, D_3DLUT_999_Reseult[0]);
...
void fwif_color_cpy_D_3DLUT_TBL(void)
{
	...
	extern RTK_VPQ_3DLUT_INDEX dlut_index;
    extern int D_3DLUT[VIP_D_3D_LUT_SIZE][VIP_8VERTEX_RGB_MAX];
	//int i=0; //add for debug

    if(dlut_index==LUT_MAX){
        printk("index=max,set linear\n");
        memcpy(D_3DLUT_999_Reseult, D_3DLUT, VIP_D_3D_LUT_SIZE * VIP_8VERTEX_RGB_MAX * sizeof(UINT32));
        //fwif_color_WaitFor_DEN_STOP_UZUDTG();
    }else{
        printk("index=%d,set 3dlut table\n",dlut_index);
	    memcpy(D_3DLUT_999_Reseult, D_3DLUT_999_Data[dlut_index], VIP_D_3D_LUT_SIZE * VIP_8VERTEX_RGB_MAX * sizeof(UINT32));
...
char fwif_color_ScalerVIP_TC_metadata_frameSync(void)
{
	...
	if (RPC_sys_info == NULL || pTC_HDR_CTRL == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "TC frame sync, point = NULL, syste, info = %lx, TC_HDR_CTRL = %lx\n", (unsigned long)RPC_sys_info, (unsigned long)pTC_HDR_CTRL);
...
void fwif_color_TC_Flow(unsigned char TC_En)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "fwif_color_TC_Flow, TC_En=%d, I3DDMA_Color_format=%d, DisplayMode_Src=%d,I3DDMA_En=%d,\n", TC_En, I3DDMA_Color_format, DisplayMode_Src, I3DDMA_En);
...
char fwif_color_set_ST2094_FrameSync_Enable(unsigned char Enable)
{
	...
	VIP_ST2094_TBL* ST2094_usingTBL = NULL;
	extern VIP_ST2094_TBL ST2094_TBL;
	extern unsigned short panel_max_luminance;
	ST2094_CTRL = drvif_fwif_color_Get_ST2094_Ctrl();
	ST2094_usingTBL = drvif_fwif_color_Get_ST2094_TBL();

	if (ST2094_CTRL == NULL ) {
		rtd_printk(KERN_EMERG, TAG_NAME, "get ST2094 CTRL error, ST2094_CTRL=%p\n",
			ST2094_CTRL);
		return -1;
	}
	...
	if (Enable == 1) {
		...
		memcpy((unsigned char*)ST2094_usingTBL,  (unsigned char*)&ST2094_TBL, sizeof(VIP_ST2094_TBL));
	}

	ST2094_usingTBL->basis_peak = panel_max_luminance*10;
	ST2094_usingTBL->product_peak = panel_max_luminance*10;
	return 0;
}
...
int fwif_color_ScalerVIP_ST2094_demo_flow(void)
{
	...
	if (ST2094_CTRL == NULL || RPC_sys_info == NULL || gVIP_Table == NULL || VIP_system_info_structure_table == NULL) {
		if ((CNT % 100) == 0)
			rtd_printk(KERN_EMERG, TAG_NAME, "ST2094 demo flow, ST2094_CTRL=%p, RPC_sys_info=%p, gVIP_Table=%p,VIP_system_info_structure_table=%p \n",
...
char fwif_color_ScalerVIP_ST2094_frameSync(void)
{
	...
	if (ST2094_CTRL == NULL || RPC_sys_info == NULL) {
		if ((CNT % 100) == 0)
			rtd_printk(KERN_EMERG, TAG_NAME, "ST2094_frameSync error, ST2094_CTRL=%p, RPC_sys_info=%p \n", ST2094_CTRL, RPC_sys_info);
		return -1;
	}
	...
	if (fwif_color_ScalerVIP_ST2094_demo_flow() == ST2094_Demo_OFF){	/* default setting, demo mode is enabled by engineer menu or tool*/
		...
		if((Debug_Log&_BIT2)!=0) {
			...
			if (CNT >= Debug_log_delay) {
				CNT = 0;
				rtd_printk(KERN_INFO, TAG_NAME, "ST2094 frame sync, 90k clock diff = %d, VGIP lint cnt diff = %d,v_sta=%d, v_len=%d, \n",
...
char fwif_color_ST2094_Ctrl_Structure_ini(void)
{
	...
	if (ST2094_CTRL == NULL || eotf_phy_addr == 0 || oetf_phy_addr == 0 || HDR_3D_LUT_phy_addr == 0 || RPC_sys_info == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "get ST2094 DMA error, ST2094_CTRL=%p, eotf_phy_addr=%x, oetf_phy_addr=%x, HDR_3D_LUT_phy_addr=%x,RPC_sys_info=%p\n",
	...
	rtd_printk(KERN_INFO, TAG_NAME, "ST2094_ini, EOTF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d, OETF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d,\n",
	...
	if (EOTF_remap == 1) {
		...
		if ((ST2094_CTRL->EOTF_DMA.phy_addr_align == 0) || (ST2094_CTRL->EOTF_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "ST2094, EOTF 96 aligned fail, addr=%x, pVir_addr=%p,\n",
				ST2094_CTRL->EOTF_DMA.phy_addr_align, ST2094_CTRL->EOTF_DMA.pVir_addr_align);
			BUG();
		}
	}
	...
	if (OETF_remap == 1) {
		...
		if ((ST2094_CTRL->OETF_DMA.phy_addr_align == 0) || (ST2094_CTRL->OETF_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "ST2094, OETF 96 aligned fail, addr=%x, pVir_addr=%p,\n",
				ST2094_CTRL->OETF_DMA.phy_addr_align, ST2094_CTRL->OETF_DMA.pVir_addr_align);
			BUG();
		}
	}

	rtd_printk(KERN_INFO, TAG_NAME, "ST2094_ini, 96B aligned, EOTF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d, OETF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d,\n",
		ST2094_CTRL->EOTF_DMA.phy_addr_align, ST2094_CTRL->EOTF_DMA.pVir_addr_align, ST2094_CTRL->EOTF_DMA.size, EOTF_remap,
		ST2094_CTRL->OETF_DMA.phy_addr_align, ST2094_CTRL->OETF_DMA.pVir_addr_align, ST2094_CTRL->OETF_DMA.size, OETF_remap);
#endif
...
char fwif_color_DM_3D_17x17x17_LUT_Set_By_DMA(unsigned char enable, unsigned short *pArray)
{
	...
	size = drvif_memory_get_data_align(VIP_DMAto3DTable_HDR_3D_LUT_SIZE, (1 << 12));
	if (addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM_3D_17x17x17_LUT_Set_By_DMA, addr = NULL\n");
		return -1;
	}

	ret = drvif_Get_VO_Active_Info(&v_sta, &v_end, &line_cnt);
	if (ret < 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM_3D_17x17x17_LUT_Set_By_DMA is not ready, ret = %d\n", ret);
		//return -1;
	} else {
		/* wait for active area to write DMA*/
		v_len = ((v_end - v_sta)<0)?(0):((v_end - v_sta));
		...
		ret = drvif_Wait_VO_start(delay1, v_len, 0x032500);
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM_3D_17x17x17_LUT_Set_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
				ret, v_end, v_sta, delay1, delay2);
			//return -1;
		}
	}
...
void fwif_color_set_OutputGamma_System(unsigned int gamma_curve_index)
{
	...
	unsigned char fmt;
	extern unsigned int LinearOutputGamma[Bin_Num_Output_InvOutput_Gamma/2];
	
	Scaler_Get_Display_info(&display, &src_idx);
	
	down(&Gamma_Semaphore);
	//protect
	if(auto_gamma_R[1023]==0||auto_gamma_G[1023]==0||auto_gamma_B[1023]==0){
		fwif_color_outgamma_extend_TableSize(auto_gamma_R,auto_gamma_G,auto_gamma_B,TconOutputGamma_R,TconOutputGamma_G,TconOutputGamma_B);
		printk("Invalid autogamma table,use linear table!!!\n");
	}
	//fwif_color_set_GainOffsetGamma(GOut_R, GOut_G, GOut_B);
	if(tconoutputgamma_en){
		fwif_color_outgamma_extend_TableSize(GOut_R,GOut_G,GOut_B,TconOutputGamma_R,TconOutputGamma_G,TconOutputGamma_B);
		fwif_set_gamma_multiply_remap_Tcon(GOut_R, GOut_G, GOut_B,GOut_R, GOut_G, GOut_B);
		fwif_color_gamma_curve_data_protect(GOut_R, GOut_G, GOut_B);		/*DO NOT mark this, must check*/
		fwif_color_gamma_data_reduce(F_Out_R,F_Out_G,F_Out_B,GOut_R, GOut_G, GOut_B);
	}else{
		fwif_color_gamma_data_reduce(F_Out_R,F_Out_G,F_Out_B,auto_gamma_R, auto_gamma_G, auto_gamma_B);
	}

	fwif_color_set_GainOffsetGamma(F_Out_R, F_Out_G, F_Out_B);

	/*6. encode gamma cuve to gamma data*/
	fwif_color_out_gamma_encode(final_OutGAMMA_R, final_OutGAMMA_G, final_OutGAMMA_B, F_Out_R, F_Out_G, F_Out_B);
...
{
	unsigned int addr_sta, addr_end, size;
	...
	if(GammaEnable ==1){
		...
		case GAMMA_CURVE_3:
				...
				*In_B = gVip_Table->tGAMMA[3].B;
				break;
		case GAMMA_CURVE_4:
				*In_R = TconOutputGamma_R;
				*In_G = TconOutputGamma_G;
				*In_B = TconOutputGamma_B;
				break;
		case GAMMA_CURVE_5:
				*In_R = gVip_Table->tGAMMA[5].R;
				*In_G = gVip_Table->tGAMMA[5].G;
...
for(i=0;i<(Bin_Num_Output_InvOutput_Gamma);i++){
	...
	Gamma_G[0]=0;
	Gamma_B[0]=0;
	Gamma_R[Bin_Num_Output_InvOutput_Gamma] = Gamma_R[Bin_Num_Output_InvOutput_Gamma - 1];
	Gamma_G[Bin_Num_Output_InvOutput_Gamma] = Gamma_G[Bin_Num_Output_InvOutput_Gamma - 1];
	Gamma_B[Bin_Num_Output_InvOutput_Gamma] = Gamma_B[Bin_Num_Output_InvOutput_Gamma - 1];
...
unsigned int fwif_color_get_colorspaceInfo(void)
{
	...
	COLOR_MATRIX_TYPE ColorSpaceType = COLOR_MATRIX_TYPE_BT709;
	...
	//printk("fwif_color_get_colorspaceInfo adobe=%d isColorSpaceIndex=%d\n",pVOInfo->isAdobeRGB,isColorSpaceIndex);
	if(isColorSpaceIndex==isBT2020)
	{	
		//printk("isBT2020\n");
		ColorSpaceType = COLOR_MATRIX_TYPE_BT2020;
	}
	/*else if(pVOInfo->isAdobeRGB)
	{	
		//printk("isAdobeRGB\n");
		ColorSpaceType = COLOR_MATRIX_TYPE_ADOBE_RGB;
	}*/
	else if(isDCIP3 == 1)
	{	
		//printk("isDCIP3\n");
		ColorSpaceType = COLOR_MATRIX_TYPE_DCTIP3;
	}
	else if(isColorSpaceIndex == is709)
	{	
		//printk("is709\n");
		ColorSpaceType = COLOR_MATRIX_TYPE_BT709;
	}
    else if(isColorSpaceIndex == isDCTIP3)
	{	
		//printk("isDCTIP3\n");
		ColorSpaceType = COLOR_MATRIX_TYPE_DCTIP3;
	}
    else if(isColorSpaceIndex == isBT601_525)
	{	
		//printk("isBT601_525\n");
		ColorSpaceType = COLORSPACE_VIDEO_BT601_525;
	}
    else if(isColorSpaceIndex == isBT601_625)
	{	
		//printk("isBT601_625\n");
		ColorSpaceType = COLORSPACE_VIDEO_BT601_625;
	}
	else
...
unsigned char fwif_get_colorspaceInfo(void)
{
	...
	*/
	...
	{
		if(pAviInfo.C == VFE_HDMI_AVI_COLORIMETRY_SMPTE170) //601
			isColorSpaceIndex=isBT601_525;
		else if(pAviInfo.C == VFE_HDMI_AVI_COLORIMETRY_ITU709) //709
			isColorSpaceIndex=is709;
		else if(pAviInfo.C == VFE_HDMI_AVI_COLORIMETRY_FUTURE)
		{
			if(pAviInfo.EC ==0)//601
				isColorSpaceIndex=is709;
			else if(pAviInfo.EC ==1)//709
				isColorSpaceIndex=is709;
			else if(pAviInfo.EC ==2)//709
				isColorSpaceIndex=is709;
			else if(pAviInfo.EC == 3 || pAviInfo.EC == 4)//RGB
				isColorSpaceIndex=isRGB;
			else if(pAviInfo.EC == 5 || pAviInfo.EC == 6)//bt2020
				...
				isColorSpaceIndex=isBT2020;
			else if(pAviInfo.EC == 7)//DCTIP3
			{
				/*
				if((0 == pAviInfo.ACE) || (1 == pAviInfo.ACE))
					isColorSpaceIndex=isDCTIP3;
				else
					*/
					isColorSpaceIndex=is709;
			}
	...
	else if(VIP_system_info_structure_table->Input_src_Type==_SRC_VO /*SrcType==_SRC_VO*/)
	{
		...
		*/
		if (pVOInfo->colour_primaries == 1)
			isColorSpaceIndex=is709;
		else if (pVOInfo->colour_primaries == 5)
			isColorSpaceIndex=isBT601_625;
		else if (pVOInfo->colour_primaries == 6)
			isColorSpaceIndex=isBT601_525;
		else if ((pVOInfo->colour_primaries == 9 || VIP_system_info_structure_table ->HDR_flag) && (VIP_system_info_structure_table ->HDR_flag != HAL_VPQ_HDR_MODE_SDR_MAX_RGB))	/* bt2020*/
			isColorSpaceIndex=isBT2020;
		else
			isColorSpaceIndex=is709;
	}
	...
	//printk("tim_li,srctype=%d,colour_primaries=%d \n",VIP_system_info_structure_table->Input_src_Type,pVOInfo->colour_primaries);
...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV030(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	...
	unsigned char VIC;
	if (fwif_color_get_force_run_i3ddma_enable(SLR_MAIN_DISPLAY) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC)) {
		fwif_color_vsc_to_scaler_src_mapping(Get_DisplayMode_Src(SLR_MAIN_DISPLAY), &nSrcType, &input_src_form);
	} else {
		nSrcType = Scaler_InputSrcGetMainChType();
		input_src_form = Scaler_InputSrcGetMainChFrom();
	}
	printk("[fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV030]srcType=%d\n",nSrcType);
	...
	if (nSrcType == _SRC_HDMI) {
		...
		video_format = drvif_Hdmi_GetColorSpace();
		printk("[Idomain]HDMI_CSC_DataRange_Mode=%d\n",HDMI_CSC_DataRange_Mode);
		printk("[Idomain]IsRGB_YUV_RANGE=%d\n",IsRGB_YUV_RANGE);
		printk("[Idomain]video_format=%d\n",video_format);
	...
	printk("[Idomain]colorFMT_info=%d,%d,%d,%d\n",colorFMT_info,pAviInfo.EC,pAviInfo.EC,pAviInfo.C);
...
unsigned char fwif_color_get_HDR_YUV2RGB_SDR_MAX_RGB_TV030(unsigned char color_format)
{
	...
	unsigned char input_src_form;
	printk("fwif_color_get_HDR_YUV2RGB_SDR_MAX_RGB_TV030\n");
	if (fwif_color_get_force_run_i3ddma_enable(SLR_MAIN_DISPLAY) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC)) {
		fwif_color_vsc_to_scaler_src_mapping(Get_DisplayMode_Src(SLR_MAIN_DISPLAY), &srcType, &input_src_form);
	} else {
		srcType = Scaler_InputSrcGetMainChType();
	...
	if (srcType == _SRC_HDMI) {
		...
		printk("data_range,_SRC_HDMI,colorFMT_info=%d\n",colorFMT_info);

		if (HDMI_CSC_DataRange_Mode == HDMI_CSC_DATA_RANGE_MODE_AUTO) {
			if (IsRGB_YUV_RANGE==MODE_RAG_FULL){
				data_range = HDMI_CSC_DATA_RANGE_MODE_FULL;
				printk("data_range,_SRC_HDMI,MODE_RAG_FULL\n");
			}
			else if (IsRGB_YUV_RANGE==MODE_RAG_LIMIT){
				data_range = HDMI_CSC_DATA_RANGE_MODE_LIMITED;
				printk("data_range,_SRC_HDMI,MODE_RAG_Limit\n");
			}
			else{
				data_range = HDMI_CSC_DATA_RANGE_MODE_AUTO;				
				printk("data_range,_SRC_HDMI,MODE_RAG_Auto\n");
			}
		} else {
			data_range = HDMI_CSC_DataRange_Mode;
			printk("data_range,_SRC_HDMI,data_range else=%d\n",HDMI_CSC_DataRange_Mode);
		}
		...
		data_range = HDMI_CSC_DATA_RANGE_MODE_LIMITED;
		printk("data_range,_SRC_VO,colorFMT_info=%d,data_range:Limit\n",colorFMT_info);
		...
		data_range = HDMI_CSC_DATA_RANGE_MODE_FULL;
		printk("data_range,_SRC_VGA,colorFMT_info=%d,ge:Full\n",colorFMT_info);
	} else {
		if (isHD == 0) {
			colorFMT_info = is601;
		} else {
		...
		data_range = HDMI_CSC_DATA_RANGE_MODE_LIMITED;
		printk("data_range,_SRC_else,colorFMT_info=%d,ge:Limit\n",colorFMT_info);		
	}
	...
	if (srcType == _SRC_VGA) {
		...
		Data_Range_HDR_YUV2RGB_TV030 = HDMI_CSC_DATA_RANGE_MODE_FULL;
		...
		Data_Range_HDR_YUV2RGB_TV030 = HDMI_CSC_DATA_RANGE_MODE_LIMITED;
		...
		if (drvif_IsHDMI() && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_MODECURR) != _MODE_NEW)) {
			if (video_format !=  0) {/*YUV444 or YUV422*/
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
				printk("data_range,HDMI_MAXRGB isHDMI YUV,data_range:Full.\n");					
					if (colorFMT_info == isBT2020) {
				...
				printk("data_range,HDMI_MAXRGB isHDMI YUV,data_range:Limit.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
				printk("data_range,HDMI_MAXRGB isHDMI RGB,data_range:Full.\n");		
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_MAXRGB isHDMI RGB,data_range:Limit.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
			...
			if (video_format !=  0) {/*YUV444 or YUV422*/
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
				printk("data_range,HDMI_MAXRGB DVI YUV,data_range:Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_MAXRGB DVI YUV,data_range:Limit.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_LIMITED) {
					printk("data_range,HDMI_MAXRGB DVI RGB,data_range:Limit.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_MAXRGB DVI RGB,data_range:Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
		...
		if (drvif_IsHDMI() && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_MODECURR) != _MODE_NEW)) {
			if (video_format !=  0) {/*YUV444 or YUV422*/
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
					printk("data_range,HDMI_NotMAXRGB isHDMI YUV,data_range:Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_NotMAXRGB isHDMI YUV,data_range:else Full.\n");
					if (colorFMT_info == isBT2020) {
				...
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
					printk("data_range,HDMI_NotMAXRGB isHDMI RGB,data_range:Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_NotMAXRGB isHDMI RGB,data_range:else Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_Limted_235_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_Limted_235_240_to_255;
			...
			if (video_format !=  0) {/*YUV444 or YUV422*/
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
					printk("data_range,HDMI_NotMAXRGB DVI YUV,data_range:Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
				...
				printk("data_range,HDMI_NotMAXRGB DVI YUV,data_range:else Full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_Limted_235_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_Limted_235_240_to_255;
				...
				if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_LIMITED) {
					printk("data_range,HDMI_NotMAXRGB DVI RGB,data_range:Limit->full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_Limted_235_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_Limted_235_240_to_255;
				...
				printk("data_range,HDMI_NotMAXRGB DVI RGB,data_range:else full.\n");
					if (colorFMT_info == isBT2020) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
					} else if (colorFMT_info == is709) {
						ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
	...
	rtd_printk(KERN_INFO, TAG_NAME, "CSC_SMR,disp=%d,src=%d,fmt=%d,ret=%d,mode=%d,\n",
		display, srcType, color_format, ret_idx, mode);

	return ret_idx;

}

unsigned char fwif_color_get_I3DDMA_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV030(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	unsigned char table_num = RGB2YUV_COEF_BYPASS_NORMAL_GAIN;
  unsigned char input_src_form;
	SLR_VOINFO* pVOInfo = NULL;
...
void fwif_color_set_ColorMatrix(unsigned char enable,unsigned int index)
{
	...
	if(enable ==1){
		if(index==COLOR_MATRIX_TYPE_BT2020)
		{	printk("[sRGB]isBT2020\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, BT2020Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
		else if(index==COLOR_MATRIX_TYPE_ADOBE_RGB)
		{	printk("[sRGB]isAdobeRGB\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, ADOBERGBTarget_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
		else if(index==COLOR_MATRIX_TYPE_DCTIP3)
		{	printk("[sRGB]isDCIP3\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, DCIP3Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
		else if(index==COLOR_MATRIX_TYPE_BT709)
		{	printk("[sRGB]is709\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, BT709Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
        else if(index==COLORSPACE_VIDEO_BT601_525)
		{	printk("[sRGB]BT601_525\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, BT601_525Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
        else if(index==COLORSPACE_VIDEO_BT601_625)
		{	printk("[sRGB]BT601_625\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, BT601_625Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
		else
		{	printk("[sRGB]the others 709\n");
			fwif_color_ColorMap_SetMatrix(SLR_MAIN_DISPLAY, BT709Target_sRGB_APPLY, 2, g_srgbForceUpdate, 0);
		}
	}else{
