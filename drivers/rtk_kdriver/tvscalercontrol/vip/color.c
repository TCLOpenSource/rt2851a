...
void drvif_color_D_3dLUT(unsigned char LUT_Ctrl, unsigned int *pArray)//flora@20150617
{
	...
	//Image may flicker during adjust colorspace cause of 3dlut en: 1->0->1;
	/*
...
unsigned char drvif_color_D_3dLUT_By_DMA(unsigned char LUT_Ctrl, unsigned int *pArray, unsigned int addr, unsigned int* vir_addr)
{
	...
	if ((addr == 0) || (vir_addr == NULL)) {
		rtd_printk(KERN_EMERG, TAG_NAME, "drvif_color_D_3dLUT_By_DMA, table NULL, addr = %x, vi_addr = %p\n",
			 addr, vir_addr);
		return false;
	}
...
char drvif_DM2_OETF_Set_By_DMA(unsigned short *OETF_table16_R, unsigned short *OETF_table16_G, unsigned short *OETF_table16_B, unsigned int addr, unsigned int* vir_addr)
{
	...
	//rtd_printk(KERN_INFO, TAG_NAME, "OETF_phys = %x, OETF_virt = %p\n", addr, vir_addr);
...
void drvif_DM2_OETF_Set_Prog_Index( VIP_HDR_OETF_Prog_Idx_TBL* OETF_prog_idx_tbl )
{
	...
	for(i = 1; i<7; i++) // data check
	{
		if(OETF_prog_idx_tbl->oetf_grid_num_th[i] < OETF_prog_idx_tbl->oetf_grid_num_th[i-1])
			{rtd_printk(KERN_DEBUG, TAG_NAME, "PQ WARNING, HDR OETF table wrong, OETF_grid_num[%d] = %d > OETF_grid_num[%d] = %d\n", i - 1, OETF_prog_idx_tbl->oetf_grid_num_th[i - 1], i, OETF_prog_idx_tbl->oetf_grid_num_th[i]);}
	}
...
void drvif_DM2_HDR_trigger_histirqen(unsigned char irpen)
{
	dm_y_histogram_ctrl_RBUS dm_y_histogram_ctrl_Reg;
	
	dm_y_histogram_ctrl_Reg.regValue = rtd_inl(DM_Y_histogram_ctrl_reg);
	dm_y_histogram_ctrl_Reg.hist_irq_en = 1;
	rtd_outl(DM_Y_histogram_ctrl_reg, dm_y_histogram_ctrl_Reg.regValue);
...
char drvif_Wait_VGIP_start(short delay0, short delay1, unsigned short timeout)
{
	...
	if ((time_end-time_str) >= timeout || timeoutcnt <= 10) {
		rtd_printk(KERN_ERR, TAG_NAME, "drvif_Wait_VGIP_start timeout=%d, timeoutcnt=%d\n", timeout, timeoutcnt);
		return -1;
	} else
		return 0;
...
char drvif_Set_DM_HDR_3dLUT_24x24x24_16(unsigned char enable, unsigned short *pArray)
{
	...
	if (timeoutcnt <= 10)
	{
		rtd_printk(KERN_EMERG, TAG_NAME, "PQ ERROR, 24x24x24 LUT can't write!\n");
		return -2;
	}
...
char drvif_Set_DM_HDR_3dLUT_17x17x17_16_By_DMA(unsigned char enable, unsigned short *pArray, unsigned int addr, unsigned int* vir_addr)
{
	...
	if ((pArray == NULL) || (addr == 0) || (vir_addr == NULL)) {
		rtd_printk(KERN_EMERG, TAG_NAME, "drvif_Set_DM_HDR_3dLUT_17x17x17_16_By_DMA, table NULL, pArray=%p, addr = %x, vi_addr = %p\n",
			pArray, addr, vir_addr);
		return -1;
	}
...
*/
		...
		case PQ_ByPass_DCC:
			tmp = (((IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg)&COLOR_D_VC_Global_CTRL_0_ms_ctrl_dcc_mask)>>COLOR_D_VC_Global_CTRL_0_ms_ctrl_icm_shift)&0x2)>>1;
			if (tmp) //sub status
		...
		case PQ_ByPass_SHP:
			tmp = (((IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg)&COLOR_D_VC_Global_CTRL_0_ms_ctrl_sharp_mask)>>COLOR_D_VC_Global_CTRL_0_ms_ctrl_icm_shift)&0x2)>>1;
			if (tmp) //sub status
		...
		case PQ_ByPass_SHP_Sub:
			tmp = ((IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg)&COLOR_D_VC_Global_CTRL_0_ms_ctrl_icm_mask)>>COLOR_D_VC_Global_CTRL_0_ms_ctrl_sharp_shift)&0x1;
			if (tmp) //main status
...
*/
		...
		case PQ_ByPass_SHP_Sub:
			ret = (((IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg)&COLOR_D_VC_Global_CTRL_0_ms_ctrl_icm_mask)>>COLOR_D_VC_Global_CTRL_0_ms_ctrl_sharp_shift)&0x2)>>1;
		break;
...
void drvif_DM_HDR_Vtop_set(unsigned char dolbyMode, unsigned char hdmi_in_mux, unsigned char dolby_ratio, unsigned char en_422to444_1, unsigned char en_TC_422to444_1)
{
	hdr_all_top_top_ctl_RBUS top_ctl_reg;
	top_ctl_reg.regValue = IoReg_Read32(HDR_ALL_TOP_TOP_CTL_reg);
...
char drvif_color_set_DeMura(DRV_DeMura_DMA_TBL* pDMA_TBL, DRV_DeMura_CTRL_TBL *ptr, unsigned char *pArray, unsigned int addr, unsigned int *vir_addr, unsigned int TBL_Line_size)
{
	...
	if ((ptr == NULL) || (pArray == NULL) || (addr == 0) || (vir_addr == NULL)) {
		rtd_printk(KERN_INFO, TAG_NAME, "drvif_color_set_DeMura tab;e NULL, ptr=%p,pArray=%p,NULL,addr=%x,vir_addr=%p\n", ptr, pArray, addr, vir_addr);
		return -1;
	}
