...
void drvif_color_ultrazoom_Adapt12tapH(unsigned char display, unsigned char H8tap_en, unsigned char hor10_table_sel)
{
	    ...
	    if (VIP_RPC_system_info_structure_table==NULL) {
			rtd_printk(KERN_INFO, TAG_NAME, "~get VIP_system_info_structure_table Error return, %s->%d, %s~\n", __FILE__, __LINE__, __FUNCTION__);
			return;
	    }
		if (display == SLR_MAIN_DISPLAY) {
		...
		if (dm_uzu_Ctrl.hor_mode_sel == 2){
			if(
              (VIP_RPC_system_info_structure_table->VIP_source==VIP_QUALITY_DTV_480I) ||
              (VIP_RPC_system_info_structure_table->VIP_source==VIP_QUALITY_DTV_576I) ||
              (VIP_RPC_system_info_structure_table->VIP_source==VIP_QUALITY_DTV_480P) ||
              (VIP_RPC_system_info_structure_table->VIP_source==VIP_QUALITY_DTV_576P)
              )
              drvif_color_ultrazoom_H12tapDeRing(6);
...
void drvif_color_ultrazoom_H12tapDeRing(unsigned char Level)
{
	int su_dering_set[7][14] = {
		...
		{1,0,120, 1,1,2,140, 0,4, 2,1,3,3,8},/*level 6*/
		};
