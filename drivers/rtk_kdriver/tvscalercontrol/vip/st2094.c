...
void drvif_fwif_color_Set_ST2094_OETF(ST2094_metadata *metadata, unsigned int sourceMaxL, unsigned int *virAddr, unsigned int phyAddr, VIP_ST2094_CTRL* ST2094_CTRL)
{
	...
	*/
	...
	if((Debug_Log&_BIT4)!=0) {
		if (CNT >= Debug_log_delay) {
			CNT = 0;
			rtd_printk(KERN_INFO, TAG_NAME, "[ST2094 OETF] Ref. peak=%d, Product peak=%d \n", referencePeak, productPeak );
			rtd_printk(KERN_INFO, TAG_NAME, "[ST2094 OETF] New knee point x=%d, y=%d \n", Kx, Ky );
			rtd_printk(KERN_INFO, TAG_NAME, "[ST2094 OETF] P[0~4] = %d, %d, %d, %d, %d\n", P_elev[0], P_elev[1], P_elev[2], P_elev[3], P_elev[4] );
			rtd_printk(KERN_INFO, TAG_NAME, "[ST2094 OETF] P[5~9] = %d, %d, %d, %d, %d\n", P_elev[5], P_elev[6], P_elev[7], P_elev[8], P_elev[9] );
			//rtd_printk(KERN_INFO, TAG_NAME, "[ST2094 OETF] P[10~13] = %d, %d, %d, %d\n", P_elev[10], P_elev[11], P_elev[12], P_elev[13] );
		}
	}
...
void drvif_fwif_color_ScalerVIP_set_ST2094_metadata2reg_frameSync(int display_Brightness, _RPC_system_setting_info *RPC_sys_info, VIP_ST2094_CTRL* ST2094_CTRL)
{
	...
	ST2094_metadata *pST2094_metadata;
	...
	pST2094_metadata = drvif_fwif_color_Get_ST2094_METADATA_Buff();

	if (drvif_Get_ST2094_EFuse_Status() == 1) {	// no ST2094
		memset(pST2094_metadata, 0 , sizeof(ST2094_metadata));
		return;
	}
	...
	if (1) {
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) {
			...
			pST2094_metadata->application_version						= ( vsif_packet->payload[0] & 0xC0 ) >> 6;
			pST2094_metadata->targeted_system_display_maximum_luminance	= (( vsif_packet->payload[0] & 0x3E ) >> 1 ) * 32;
			pST2094_metadata->average_maxrgb							= vsif_packet->payload[1] * 160;
			pST2094_metadata->maxscl[0] 								= vsif_packet->payload[10] * 160; // use 99% percentile as maxscl
			pST2094_metadata->maxscl[1] 								= vsif_packet->payload[10] * 160;
			pST2094_metadata->maxscl[2] 								= vsif_packet->payload[10] * 160;
			pST2094_metadata->num_distribution_maxrgb_percentiles			= 9;
			pST2094_metadata->distribution_maxrgb_percentages[0]			= 1;
			pST2094_metadata->distribution_maxrgb_percentages[1]			= 5;
			pST2094_metadata->distribution_maxrgb_percentages[2]			= 10;
			pST2094_metadata->distribution_maxrgb_percentages[3]			= 25;
			pST2094_metadata->distribution_maxrgb_percentages[4]			= 50;
			pST2094_metadata->distribution_maxrgb_percentages[5]			= 75;
			pST2094_metadata->distribution_maxrgb_percentages[6]			= 90;
			pST2094_metadata->distribution_maxrgb_percentages[7]			= 95;
			pST2094_metadata->distribution_maxrgb_percentages[8]			= 99;
			pST2094_metadata->distribution_maxrgb_percentiles[0]			= vsif_packet->payload[2] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[1]			= vsif_packet->payload[3] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[2]			= vsif_packet->payload[4];
			pST2094_metadata->distribution_maxrgb_percentiles[3]			= vsif_packet->payload[5] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[4]			= vsif_packet->payload[6] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[5]			= vsif_packet->payload[7] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[6]			= vsif_packet->payload[8] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[7]			= vsif_packet->payload[9] * 160;
			pST2094_metadata->distribution_maxrgb_percentiles[8]			= vsif_packet->payload[10] * 160;
			pST2094_metadata->num_bezier_curve_anchors					= (vsif_packet->payload[11] & 0xF0) >> 4;
			pST2094_metadata->tone_mapping_flag						= pST2094_metadata->num_bezier_curve_anchors > 0 ? 1 : 0;
			pST2094_metadata->knee_point_x								= ( ( ((unsigned short)vsif_packet->payload[12] & 0x0F) << 6 ) + ( (vsif_packet->payload[13] & 0xFC) >> 2 ) ) * 4;
			pST2094_metadata->knee_point_y								= ( ( ((unsigned short)vsif_packet->payload[13] & 0x03) << 8 ) + vsif_packet->payload[14] ) * 4;
			pST2094_metadata->bezier_curve_anchors[0]					= vsif_packet->payload[15] * 4;
			pST2094_metadata->bezier_curve_anchors[1]					= vsif_packet->payload[16] * 4;
			pST2094_metadata->bezier_curve_anchors[2]					= vsif_packet->payload[17] * 4;
			pST2094_metadata->bezier_curve_anchors[3]					= vsif_packet->payload[18] * 4;
			pST2094_metadata->bezier_curve_anchors[4]					= vsif_packet->payload[19] * 4;
			pST2094_metadata->bezier_curve_anchors[5]					= vsif_packet->payload[20] * 4;
			pST2094_metadata->bezier_curve_anchors[6]					= vsif_packet->payload[21] * 4;
			pST2094_metadata->bezier_curve_anchors[7]					= vsif_packet->payload[22] * 4;
			pST2094_metadata->bezier_curve_anchors[8]					= vsif_packet->payload[23] * 4;
			pST2094_metadata->bezier_curve_anchors[9]					= 0;

		} else {
			pST2094_metadata->application_version							=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.application_version						, 1);
			pST2094_metadata->targeted_system_display_maximum_luminance 	=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.targeted_system_display_maximum_luminance , 1);
			pST2094_metadata->average_maxrgb								=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.average_maxrgb							, 1);
			pST2094_metadata->maxscl[0] 									=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.maxscl[0] 								, 1);
			pST2094_metadata->maxscl[1] 									=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.maxscl[1] 								, 1);
			pST2094_metadata->maxscl[2] 									=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.maxscl[2] 								, 1);
			pST2094_metadata->num_distribution_maxrgb_percentiles			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.num_distribution_maxrgb_percentiles		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[0]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[0]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[1]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[1]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[2]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[2]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[3]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[3]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[4]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[4]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[5]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[5]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[6]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[6]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[7]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[7]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[8]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[8]		, 1);
			pST2094_metadata->distribution_maxrgb_percentages[9]			=	fwif_color_ChangeOneUINT16Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentages[9]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[0]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[0]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[1]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[1]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[2]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[2]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[3]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[3]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[4]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[4]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[5]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[5]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[6]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[6]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[7]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[7]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[8]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[8]		, 1);
			pST2094_metadata->distribution_maxrgb_percentiles[9]			=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.distribution_maxrgb_percentiles[9]		, 1);
			pST2094_metadata->tone_mapping_flag							=	RPC_sys_info->HDR_info.hdr_ST2094_metadata.tone_mapping_flag																;
			pST2094_metadata->knee_point_x 								=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.knee_point_x 								, 1);
			pST2094_metadata->knee_point_y 								=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.knee_point_y 								, 1);
			pST2094_metadata->num_bezier_curve_anchors					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.num_bezier_curve_anchors					, 1);
			pST2094_metadata->bezier_curve_anchors[0] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[0] 					, 1);
			pST2094_metadata->bezier_curve_anchors[1] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[1] 					, 1);
			pST2094_metadata->bezier_curve_anchors[2] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[2] 					, 1);
			pST2094_metadata->bezier_curve_anchors[3] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[3] 					, 1);
			pST2094_metadata->bezier_curve_anchors[4] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[4] 					, 1);
			pST2094_metadata->bezier_curve_anchors[5] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[5] 					, 1);
			pST2094_metadata->bezier_curve_anchors[6] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[6] 					, 1);
			pST2094_metadata->bezier_curve_anchors[7] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[7] 					, 1);
			pST2094_metadata->bezier_curve_anchors[8] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[8] 					, 1);
			pST2094_metadata->bezier_curve_anchors[9] 					=	fwif_color_ChangeOneUINT32Endian(RPC_sys_info->HDR_info.hdr_ST2094_metadata.bezier_curve_anchors[9] 					, 1);
	...
	else
	{
		...
		pST2094_metadata->application_version = 0;
		pST2094_metadata->targeted_system_display_maximum_luminance = 400;
		pST2094_metadata->average_maxrgb = 1052;
		pST2094_metadata->maxscl[0] = 8002;
		pST2094_metadata->maxscl[1] = 8370;
		pST2094_metadata->maxscl[2] = 7120;
		pST2094_metadata->num_distribution_maxrgb_percentiles = 10;
		pST2094_metadata->distribution_maxrgb_percentages[0] = 1;
		pST2094_metadata->distribution_maxrgb_percentages[1] = 5;
		pST2094_metadata->distribution_maxrgb_percentages[2] = 10;
		pST2094_metadata->distribution_maxrgb_percentages[3] = 25;
		...
		pST2094_metadata->distribution_maxrgb_percentages[6] = 90;
		pST2094_metadata->distribution_maxrgb_percentages[7] = 95;
		pST2094_metadata->distribution_maxrgb_percentages[8] = 98;
		pST2094_metadata->distribution_maxrgb_percentages[9] = 99;
		pST2094_metadata->distribution_maxrgb_percentiles[0] = 1;
		pST2094_metadata->distribution_maxrgb_percentiles[1] = 7;
		pST2094_metadata->distribution_maxrgb_percentiles[2] = 15;
		pST2094_metadata->distribution_maxrgb_percentiles[3] = 45;
		pST2094_metadata->distribution_maxrgb_percentiles[4] = 136;
		pST2094_metadata->distribution_maxrgb_percentiles[5] = 646;
		pST2094_metadata->distribution_maxrgb_percentiles[6] = 4919;
		pST2094_metadata->distribution_maxrgb_percentiles[7] = 7120;
		pST2094_metadata->distribution_maxrgb_percentiles[8] = 7185;
		pST2094_metadata->distribution_maxrgb_percentiles[9] = 7381;
		pST2094_metadata->tone_mapping_flag = 1;
		pST2094_metadata->knee_point_x = 572;
		pST2094_metadata->knee_point_y = 1056;
		pST2094_metadata->num_bezier_curve_anchors = 3;
		pST2094_metadata->bezier_curve_anchors[0] = 506;
		pST2094_metadata->bezier_curve_anchors[1] = 694;
		pST2094_metadata->bezier_curve_anchors[2] = 701;
		pST2094_metadata->bezier_curve_anchors[3] = 0;
		pST2094_metadata->bezier_curve_anchors[4] = 0;
		pST2094_metadata->bezier_curve_anchors[5] = 0;
		pST2094_metadata->bezier_curve_anchors[6] = 0;
		pST2094_metadata->bezier_curve_anchors[7] = 0;
		pST2094_metadata->bezier_curve_anchors[8] = 0;
		pST2094_metadata->bezier_curve_anchors[9] = 0;
	...
	if (pST2094_metadata->num_distribution_maxrgb_percentiles <= 0 || pST2094_metadata->num_distribution_maxrgb_percentiles > distribution_maxrgb_bin_num)
		percentile99_idx = distribution_maxrgb_bin_num - 1;
	else
		percentile99_idx = pST2094_metadata->num_distribution_maxrgb_percentiles - 1;
	sourceMaxL = pST2094_metadata->distribution_maxrgb_percentiles[percentile99_idx]; // 99% percentile
	...
	drvif_fwif_color_Set_ST2094_OETF(pST2094_metadata, sourceMaxL, virAddr_OETF, phyAddr_OETF, ST2094_CTRL );
	...
	if((Debug_Log&_BIT1)!=0) {
		if (CNT >= Debug_log_delay) {
			...
			rtd_printk(KERN_INFO, TAG_NAME, "ST2094_enable=%d, application_version=%d, targeted_system_display_maximum_luminance=%d, average_maxrgb=%d \n",
				ST2094_CTRL->Enable, pST2094_metadata->application_version, pST2094_metadata->targeted_system_display_maximum_luminance, pST2094_metadata->average_maxrgb);

			rtd_printk(KERN_INFO, TAG_NAME, "maxscl[0]=%d, maxscl[1]=%d, maxscl[2]=%d, num_distribution_maxrgb_percentiles=%d \n",
				pST2094_metadata->maxscl[0], pST2094_metadata->maxscl[1], pST2094_metadata->maxscl[2], pST2094_metadata->num_distribution_maxrgb_percentiles);

			rtd_printk(KERN_INFO, TAG_NAME, "distribution_maxrgb_percentages[0]~[4]=%d,%d,%d,%d,%d \n",
				pST2094_metadata->distribution_maxrgb_percentages[0], pST2094_metadata->distribution_maxrgb_percentages[1], pST2094_metadata->distribution_maxrgb_percentages[2],
				pST2094_metadata->distribution_maxrgb_percentages[3], pST2094_metadata->distribution_maxrgb_percentages[4]);

			rtd_printk(KERN_INFO, TAG_NAME, "distribution_maxrgb_percentages[5]~[9]=%d,%d,%d,%d,%d \n",
				pST2094_metadata->distribution_maxrgb_percentages[5], pST2094_metadata->distribution_maxrgb_percentages[6], pST2094_metadata->distribution_maxrgb_percentages[7],
				pST2094_metadata->distribution_maxrgb_percentages[8], pST2094_metadata->distribution_maxrgb_percentages[9]);

			rtd_printk(KERN_INFO, TAG_NAME, "distribution_maxrgb_percentiles[0]~[4]=%d,%d,%d,%d,%d \n",
				pST2094_metadata->distribution_maxrgb_percentiles[0], pST2094_metadata->distribution_maxrgb_percentiles[1], pST2094_metadata->distribution_maxrgb_percentiles[2],
				pST2094_metadata->distribution_maxrgb_percentiles[3], pST2094_metadata->distribution_maxrgb_percentiles[4]);

			rtd_printk(KERN_INFO, TAG_NAME, "distribution_maxrgb_percentiles[5]~[9]=%d,%d,%d,%d,%d \n",
				pST2094_metadata->distribution_maxrgb_percentiles[5], pST2094_metadata->distribution_maxrgb_percentiles[6], pST2094_metadata->distribution_maxrgb_percentiles[7],
				pST2094_metadata->distribution_maxrgb_percentiles[8], pST2094_metadata->distribution_maxrgb_percentiles[9]);

			rtd_printk(KERN_INFO, TAG_NAME, "tone_mapping_flag=%d, knee_point_x=%d, knee_point_y=%d, num_bezier_curve_anchors=%d \n",
				pST2094_metadata->tone_mapping_flag, pST2094_metadata->knee_point_x, pST2094_metadata->knee_point_y, pST2094_metadata->num_bezier_curve_anchors);

			rtd_printk(KERN_INFO, TAG_NAME, "bezier_curve_anchors[0]~[4]=%d,%d,%d,%d,%d \n",
				pST2094_metadata->bezier_curve_anchors[0], pST2094_metadata->bezier_curve_anchors[1], pST2094_metadata->bezier_curve_anchors[2],
				pST2094_metadata->bezier_curve_anchors[3], pST2094_metadata->bezier_curve_anchors[4]);

			rtd_printk(KERN_INFO, TAG_NAME, "bezier_curve_anchors[5]~[9]=%d,%d,%d,,%d,%d \n",
				pST2094_metadata->bezier_curve_anchors[5], pST2094_metadata->bezier_curve_anchors[6], pST2094_metadata->bezier_curve_anchors[7],
				pST2094_metadata->bezier_curve_anchors[8], pST2094_metadata->bezier_curve_anchors[9]);
