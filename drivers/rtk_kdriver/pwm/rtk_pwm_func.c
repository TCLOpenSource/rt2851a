...
int  m_track_mode_get(void)                    {    return m_tracking_mode;        }
...
{
    UINT64 dclk = 0;
    unsigned long clock_freq = CONFIG_CPU_XTAL_FREQ;
    switch(mode){
        case CLK_27MHZ:
                   ...
                   int pwm_freq_min, int mode)
{
    ...
    if (pwm_freq_hz == (pwm_freq_min*100)) {
        ...
        *totalcnt = duty_max;
        ...
        pwmFreqHz = pwm_freq_hz / 10;
        error_best = pwmFreqHz  + 1;
        for (j=255; j<4097; j++) // Start duty_max=255 to find solution
        {
            for (i=0; i<=3; i++)
            {
                N = (unsigned long) ((rtk_hw_get_clock(mode) *10) / (( 1 << i)*j*pwmFreqHz));
                if (N == 0)
                    N=1;
                ...
                ftmp_2 = (unsigned long) rtk_hw_get_clock(mode)*10 / ((1 << i)*j) ;
                ftmp_1 = ftmp_2 / N;
    ...
    if( m_ioctl_printk_get() > 0 )
        PWM_WARN("%s CLK_SRC:%ld pwm_freq_hz = %d M_best = 0x%X N_best = 0x%X totalcnt:%d duty_max:%d\n",__func__, rtk_hw_get_clock(mode),pwm_freq_hz , *M_best,*N_best ,*totalcnt,duty_max);
...
void rtk_pwm_hw_chip_init(R_CHIP_T *pwmPin, int ricindex)
{
    ...
    rtk_pwm_fixed_num_mode_w(pwmPin,(pwmPin->rtk_vsync == PWM_VSYNC_ENABLE ? 0:1));
}

void rtk_pwm_ctrl_probe_init(R_CHIP_T *pchip2,int pwm_mode,int invert)
{
    rtk_pwm_fixed_num_mode_w(pchip2,(pchip2->rtk_vsync == PWM_VSYNC_ENABLE ? 0:1));
...
void rtk_pwm_timing_freq_set(R_CHIP_T *pchip2,pwm_timing_ctrl_RBUS* pwm_timing_ctrl_reg)
{
    ...
    {
        PWM_ERR("Skip M/N Calculation MISC\n");
...
void rtk_pwm_iso_timing_freq_set(R_CHIP_T *pchip2,iso_pwm_timing_ctrl_RBUS* pwm_timing_ctrl_reg)
{
#ifdef NEW_PWM_FORMULA
    int rounding = 0;
#endif
    ...
    if(pchip2->rtk_freq_changed == false && pchip2->rtk_force_update == false)
    {
        PWM_ERR("Skip M/N Calculation ISO\n");
        ;//Skip M/N Calculation
    }
    else
    {
...
void rtk_pwm_clock_source(R_CHIP_T *pchip2)
{
    ...
    if(PWM_MISC_TYPE(pchip2)){
        ...
        iso_pwm_timing_ctrl_RBUS pwm_timing_ctrl_reg;
...
void rtk_pwm_vrr_dvsync_delay_init_enable_w(int enable){
    ppoverlay_dtg_ie_RBUS pwm_vrr_dtg_ie_reg;
    PWM_VRR_DTG_IE_REG_R(&pwm_vrr_dtg_ie_reg);
    pwm_vrr_dtg_ie_reg.dvsync_dly_int_en = enable;
    PWM_VRR_DTG_IE_REG_W(&pwm_vrr_dtg_ie_reg);
}

int rtk_pwm_vrr_dvsync_delay_init_update_r(void){
    int dvsync_dly_int_update = 0;
    ppoverlay_dtg_pending_status_RBUS pwm_vrr_dtg_pending_status_reg;
    pwm_vrr_dtg_pending_status_reg.regValue = rtd_inl(PWM_VRR_DTG_PENDING_STATUS);
    dvsync_dly_int_update = pwm_vrr_dtg_pending_status_reg.dvsync_dly_int_update;
...
int rtk_pwm_global_enable_iw(R_CHIP_T *pchip2,int enable){
    iso_pwm_db_ctrl_RBUS pwm_iso_db0_reg;
...
int rtk_pwm_db_enable_iw(R_CHIP_T *pchip2,int enable){
    iso_pwm_db_ctrl_RBUS pwm_iso_db_reg;
...
void rtk_pwm_db_wb(R_CHIP_T *pchip2){
    if(PWM_MISC_TYPE(pchip2)){
        ...
        iso_pwm_ctrl_RBUS pwm_ctrl_reg;
...
void rtk_pwm_freq_w(R_CHIP_T *pchip2){
    if(PWM_MISC_TYPE(pchip2)){
        ...
        iso_pwm_timing_ctrl_RBUS pwm_timing_ctrl_reg;
        I_PWM_TIMING_CTRL_REG_R(RTK_PWM_INDEX(pchip2),&pwm_timing_ctrl_reg);
        rtk_pwm_iso_timing_freq_set(pchip2,&pwm_timing_ctrl_reg);
        I_PWM_TIMING_CTRL_REG_W(RTK_PWM_INDEX(pchip2),&pwm_timing_ctrl_reg);
    }
}
...
void rtk_pwm_duty_w(R_CHIP_T *pchip2,int duty){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        ...
        iso_pwm_duty_set_RBUS pwm_duty_set_reg;
...
void rtk_pwm_mpluse_w(R_CHIP_T *pchip2,int mpluse){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        M_PWM_CTRL_REG_R(RTK_PWM_INDEX(pchip2),&pwm_ctrl_reg);
        pwm_ctrl_reg.pwm0_mplus_dim_duty_en = mpluse;
        M_PWM_CTRL_REG_W(RTK_PWM_INDEX(pchip2),&pwm_ctrl_reg);
    }
}
...
int rtk_pwm_mpluse_r(R_CHIP_T *pchip2){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        M_PWM_CTRL_REG_R(RTK_PWM_INDEX(pchip2),&pwm_ctrl_reg);
        mpluse_en = pwm_ctrl_reg.pwm0_mplus_dim_duty_en;
    }
...
void rtk_pwm_tracking_sw_init_en_w(R_CHIP_T *pchip2,int enable){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        if(pchip2->index >=2)
            return;
        M_PWM_TRACK_SET_REG_R(RTK_PWM_INDEX(pchip2),&pwm_mis_track_set_reg);
        pwm_mis_track_set_reg.pwm0_sw_track_int_en = enable;
        M_PWM_TRACK_SET_REG_W(RTK_PWM_INDEX(pchip2),&pwm_mis_track_set_reg);
    }
}
...
int rtk_pwm_tracking_sw_init_en_r(R_CHIP_T *pchip2){
    ...
    return pwm_mis_track_set_reg.pwm0_sw_track_int_en;
}
...
void rtk_pwm_tracking_phase_int_enable_w(R_CHIP_T *pchip2,int enable){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        ...
        pwm_mis_track_fsm_reg.pwm0_phase_finishtoint_en = enable;
...
void rtk_pwm_tracking_freq_int_enable_w(R_CHIP_T *pchip2,int enable){
    ...
    if(PWM_MISC_TYPE(pchip2)){
        M_PWM_TRACK_FSM_REG_R(RTK_PWM_INDEX(pchip2),&pwm_mis_track_fsm_reg);
        pwm_mis_track_fsm_reg.pwm0_freq_finishtoint_en = enable;
        M_PWM_TRACK_FSM_REG_W(RTK_PWM_INDEX(pchip2),&pwm_mis_track_fsm_reg);
    }
}
