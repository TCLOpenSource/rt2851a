...
//#define CONFIG_I2RND_DUAL_VO //enable i2rnd dual vo function
//#define BRING_UP_I2RND
...
#define	REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD		4096//this is for RTRN input max size
#define	REDUCE_DI_HSD_WIDTH_LIMIT_THRESHOLD		3840//this is for DI HSD input max size

#define REDUCE_DI_WIDTH_LIMIT_THRESHOLD         1920//this is for DI input max size
//[crixus]scart out
//#define DTV_SCART_OUT_ENABLE	1


#ifdef CONFIG_I2RND_DUAL_VO
#define vo_isr_rtd_inl rtd_inl
#define vo_isr_rtd_outl rtd_outl
#else
#define vo_isr_rtd_inl vo_rtd_inl
#define vo_isr_rtd_outl vo_rtd_outl
#endif
...
typedef struct _SLR_VOINFO {
	...
	unsigned int startPointShiftOffset; 

	/*
	HDR item need to keep latest item for seamless case @Crixus 20170215
	h sta and v sta will change when hdrsdr seamless case,
	so move here fo compare vo info@johnny 20170315
	*/
	unsigned int 						isVeComp;       // VE compress mode info for HDR10 DM setting
	...
	unsigned int 						src_v_freq;		///< added for true framerate
} SLR_VOINFO;
// oliver-
...
typedef struct _VO_Scaler_State {
	...
	unsigned int inputTiming_x;
	unsigned int inputTiming_y;
	unsigned int inputTiming_wid;
	unsigned int inputTiming_height;
...
enum SPECIFIC_PANEL_INDEX {
	SPECIFIC_PANEL_INDEX_NONE = 0,
	SPECIFIC_PANEL_INDEX_LGD_SPECIFIC_PANEL = 1,
	SPECIFIC_PANEL_INDEX_OLED = 2,
	SPECIFIC_PANEL_INDEX_LGD_FHD = 3,
	SPECIFIC_PANEL_INDEX_LGD_HD = 4,
};
...
typedef struct _Panel_Parameter_INFO {
	...
	unsigned int ivflip;
	unsigned int specific_panel_index;
...
typedef struct _SCALER_MEMORY_INFO {
	...
	//sub addr
	  ...
	  //main i2r addr
      unsigned int M_main_i2r_first_addr;
	unsigned int M_main_i2r_sec_addr;
      unsigned int M_main_i2r_third_addr;
	unsigned int DI_i2r_addr;
} SCALER_MEMORY_INFO;
...
typedef struct _SCLAER_ZOOM_INFO{
	//added by rika 20140628
		...
		unsigned int zoom_hal_orgInput_hpos;
		unsigned int zoom_hal_orgInput_vpos;
		unsigned int zoom_hal_orgInput_hsize;
		unsigned int zoom_hal_orgInput_vsize;
		...
		unsigned int Subwindowshape;
		unsigned int SubwindowWidth;
		unsigned int SubwindowLength;
...
enum _MODE_STATE
{
	...
	_MODE_STATE_DOLBY_VSIF,                                         // Get Dolby VSIF info
};
...
typedef struct _StructDisplayInfo{
	...
	unsigned short SrcIVFreq;				// Original Input Vertical Frequency
...
typedef struct {	//initialization in scalerstruct.c
	...
	char Sub_smooth_toggle_set_flag;//run  sub smooth toggle flag
...
//#endif
	...
	unsigned char run_dtgM2GoldenVsync_slow_tuning_flag;
	...
	unsigned char sub_adaptive_flag;//sub adaptive case
	unsigned char run_dtgM2UzuDelay_slow_tuning_flag;
	unsigned char dvo_dual_decoder_flag;
	unsigned char dvo_dual_decoder_run;
} DEBUG_SCALER_VSC_STRUCT;
...
typedef struct _SCALER_DISP_INFO {
	...
	unsigned int	input_2_pixel_mode;
} SCALER_DISP_INFO;
...
typedef struct _SMOOTH_TOGGLE_UPDATE_INFO {
	...
	unsigned int IMD_SMOOTH_FRAC_FS_REMOVE_RATIO;
	unsigned int IMD_SMOOTH_FRAC_FS_MULTIPLE_RATIO;
...
typedef enum
{
   ...
   // HDR Dolby Vision
	...
	VO_RUN_SCALER_IDX,
	DM_CONNECT_FLAG_IDX,
	SCALERIOC_SEAMLESS_CHANGE_SHAREMEMORY_INFO_SYNC_IDX,//for i3ddma change flag
	VBI_SHAREMEM_DATA_IDX,
    VO_PHOTO_UPDATE_SHAREMEM_IDX,//PHOTO
    VO_PHOTO_SHIFT_SHAREMEM_IDX,//PHOTO
	SCALERIOC_DUMP_REGISTER_COUNTER_IDX,

	//i2r sub hdr info
	SCALERIOC_SUB_SEAMLESS_CHANGE_SHAREMEMORY_INFO_SYNC_IDX,

	SCALERIOC_PANORAMA_IDX,//for panorama
  SCALERIOC_AV_SYNC_IDX,//for vdec mode to check av sync
        HDMI_DOLBY_VSIF_IDX,//dolby vsif info sharememory address
	 RPC_SHARE_MEM_INDEX_MAX,
	SCALER_MAIN_INPUT_OUTPUT_BUFFER_IDX,//For new input output hal flow. Main path share memory ring buffer start address
	SCALER_SUB_INPUT_OUTPUT_BUFFER_IDX,//For new input output hal flow. SUB path share memory ring buffer start address
	FILM_MODE_CHANGE_TYPE_IDX,
	SCALER_WIN_CALLBACK_DELAY_IDX,
	DYNAMIC_FRAMERATE_CHANGE_INFO_IDX,
	VO_FORCE_V_TOP_IDX,
	SCALERIOC_SETUP_DI_DONE_IDX,
	PC_MODE_SYNC_INFO_IDX,
	ORBIT_SHIFT_INFO_IDX,
...
#define _VSC_INPUT_TYPE_T
#endif


//Evance@20160721 add for Magnifier rect
#ifndef _VSC_SUB_SHAPE_T
...
#define _VSC_SUB_SHAPE_T
#endif

typedef struct _SCALER_AV_SYNC_INFO {//for scaler to check av sync
	unsigned int AV_SYNC_MAIN_INFO;
	unsigned int AV_SYNC_SUB_INFO;
} SCALER_AV_SYNC_INFO;

typedef struct SCALER_WIN_CALLBACK_DELAY_INFO {//for scaler to check av sync
	unsigned int validinfo;
	unsigned int uzulineCount;
	unsigned int region_x;
	unsigned int region_y;
	unsigned int region_w;
	unsigned int region_h;
} SCALER_WIN_CALLBACK_DELAY_INFO;

typedef struct FILM_SHAREMEM_PARAM {//for film mode sharememory use
	unsigned int film_mode_type_change;
	unsigned int memc_me_clock_off;
} FILM_SHAREMEM_PARAM;

typedef struct SCALER_ORBIT_SHIFT_INFO {//orbit shift xy
	int shift_range_x;
	int shift_range_y;
	int shift_x;
	int shift_y;
    int ori_input_x;
    int ori_input_y;
	unsigned int orbit_mode;
	unsigned int orbit_enable;
} SCALER_ORBIT_SHIFT_INFO;

/**
 * SCALER_VSC_INPUT_REGION_TYPE
 * INPUT_BASE_ON_TIMING ----------------rtk_hal_vsc_SetInputRegion
 * INPUT_BASE_ON_DISP_WIN --------------rtk_hal_vsc_SetInputRegion_ForAdaptive
 * INPUT_BASE_ON_HAL_ORIGINAL_INPUT ----rtk_hal_vsc_SetInputRegionEx
 */
 typedef enum
{
...
typedef struct _SCALER_DATA_FS_ALGO_RESULT {
	unsigned int ulInfoReady;
	unsigned int ulVoVStart;
	unsigned int ulVoVEnd;
	unsigned int uliv2dv_pixel;
	unsigned int uliv2dv_line;
	unsigned int ulVoHTotal;
} SCALER_DATA_FS_ALGO_RESULT;

typedef struct _SEND_DISP_SIZE_INFO{
	unsigned short output_x;
	unsigned short output_y;
	unsigned short output_w;
	unsigned short output_h;
	unsigned short rotate_type;
}SEND_DISP_SIZE_INFO;

typedef struct _AIRMODE_DISP_SIZE_INFO{
	unsigned int display;
	
    unsigned short main_output_x;
    unsigned short main_output_y;
    unsigned short main_output_w;
    unsigned short main_output_h;

	unsigned short sub_output_x;
    unsigned short sub_output_y;
    unsigned short sub_output_w;
    unsigned short sub_output_h;
}AIRMODE_DISP_SIZE_INFO;
