...
typedef struct
{
	unsigned char nVBICCLine;	/* for VBI parsing line select */

	unsigned char nHPLLEnable;	/* for enable HPLL tune */
	char nHPLLTune1;	/* for tunning HPLL */
	char nHPLLTune2;	/* for tunning HPLL */
	char nHPLLTune3;	/* for tunning HPLL */
	char nHPLLTune4;	/* for tunning HPLL */

	unsigned char nColorKillEnable;
	char nColorKillTune1;	/* for tunning Color Kill Level */
	char nColorKillTune2;	/* for tunning Color Kill Level */
	char nColorKillTune3;	/* for tunning Color Kill Level */
	char nColorKillTune4;	/* for tunning Color Kill Level */

	char nLockThreshold1;	/* for tunning Lock sensitivity */
	char nLockThreshold2;	/* for tunning Lock sensitivity */
...
typedef struct {
	unsigned char	data1;		///< CC data1
	unsigned char	data2;		///< CC data2
	unsigned char	field;		///< odd/even field indication(0:Even field, 1:Odd filed)
	unsigned char	status; 	///< status(reserved)
	unsigned short	line_number; ///< sliced vbi line number
...
#else
typedef struct {
	unsigned char *pbuffer;	// ttx data
	unsigned int length;	// data length
} KADP_VFE_AVD_TTX_DATA_T;
#endif
...
#else
typedef struct {
	unsigned char *pbuffer;	// vps data
	unsigned int length;	// data length
}KADP_VFE_AVD_VPS_DATA_T;
#endif
...
#else
typedef struct {
	unsigned char *pbuffer;	// wss data
	unsigned int length;	// data length
}KADP_VFE_AVD_WSS_DATA_T;
#endif
...
typedef struct
{
	unsigned int cgmsAPS               : 4 ;  // Header[6-bits] + {PayLoad + CRC}[14-bits]
										// CGMS-A 2-bits = CGMS20BitsVBI & (BIT6 | BIT7) >> 6
										// APS        2-bits = CGMS20BitsVBI & (BIT8 | BIT9) >> 8
	unsigned int copyRight             : 2 ;  // Copy Right		 	= CopyInfo576i576p >> 0
										// Copy Protection		= CopyInfo576i576p >> 1
	unsigned int PseudosyncColorStripe : 2 ;  // 00:no apps, 01:PSP on,cstripe off, 10:PSP on,2-line cstripe on, 11:PSP on,4-line cstripe on
	unsigned int ACPReserved           : 24;  // not used
...
typedef enum
{
    KADP_VFE_HDMI_DVI,      // DVI
    KADP_VFE_HDMI_HDMI,     // HDMI 1.4/2.0
    KADP_VFE_HDMI_HDMI21,   // HDMI 2.1
...
typedef struct
{
	...
	unsigned char dataBytes[HAL_VFE_HDMI_PACKET_DATA_LENGTH]; /**< packet data */
...
typedef struct
{
    unsigned char VendorName[HAL_VFE_HDMI_SPD_IF_VENDOR_LEN+1];  /* add space for NULL */
    unsigned char ProductDescription[HAL_VFE_HDMI_SPD_IF_DESC_LEN+1];
...
typedef struct
{
    ...
    unsigned char IEERegId[HAL_VFE_HDMI_VENDOR_SPECIFIC_REGID_LEN];	/**< HDMI VSI info */
    unsigned char PayLoad[HAL_VFE_HDMI_VENDOR_SPECIFIC_PAYLOAD_LEN];	/**< HDMI VSI info */
    KADP_HAL_VFE_HDMI_PACKET_STATUS_T packetStatus;				/**< HDMI VSI info */
    KADP_HAL_VFE_HDMI_IN_PACKET_T packet;								/**< HDMI VSI info */
...
typedef enum
{
	/*     ¾Æ·¡ enum value´Â °ü¸® ¾ÈÇÔ.  */
	KADP_VFE_HDMI_3D_FORMAT_2D,					/**< for No 3D (OFF)..... INPUT_TYPE_2DTO3D */

	/*      From UI, 3D ONÀÌ¸é¼­ ¾Æ·¡ FormatÀÌ ³Ñ¾î¿À¸é ÀÏ¹Ý 3D ONÀÌ µÊ.    */
	//interim format - half
	...
	/*    From UI, 3D ONÀÌ¸é¼­ ¾Æ·¡ FormatÀÌ ³Ñ¾î¿À¸é 2D to 3D °¡ µÊ    */
...
typedef enum
{
	KADP_VFE_HDMI_3D_LR,	//Driver¿Í ÀÏÄ¡.
	KADP_VFE_HDMI_3D_RL,
...
typedef struct
{
   	unsigned int format;	/**< update by LG */
	unsigned short hFreq;	/**< ... */
	unsigned short vFreq;	/**< ... */
	unsigned short hTotal;	/**< ... */
	unsigned short vTotal;	/**< ... */
...
{
	BOOLEAN lockStaus;
	unsigned int tmdsClockKHz;
	unsigned int tmdsBandwidthMbps;
	unsigned int phyCtleEQMinRage[MAX_NUMBER_OF_CHANNEL];
	unsigned int phyCtleEQMaxRage[MAX_NUMBER_OF_CHANNEL];
	unsigned int phyCtleEQResult[MAX_NUMBER_OF_CHANNEL];
	unsigned int phyError[MAX_NUMBER_OF_CHANNEL];
...
typedef struct
{
	...
	unsigned int video_pixel_clock;
...
typedef struct
{
	...
	unsigned char bit_ratio;
	BOOLEAN scramble_status;
	BOOLEAN clock_detect;
	BOOLEAN ch_locked[MAX_NUMBER_OF_CHANNEL];
	BOOLEAN FLT_Ready;
	unsigned short ch_error_count[MAX_NUMBER_OF_CHANNEL];
	unsigned int RS_Counter_Bits;
	BOOLEAN DSCFailed;
	unsigned char LTP_req[MAX_NUMBER_OF_CHANNEL];
	unsigned char FRLMode;
	unsigned char FFELevel;
	BOOLEAN FRL_Start;
	BOOLEAN FLT_Update;
...
#define VFE_IOC_AVD_CHECK_ATV_STABLE				_IO(VFE_IOC_MAGIC, VFE_IOC_AVD_IDX_36) 
...
typedef struct
{
	unsigned char   port;	        // HDMI Port
	PBYTES  p_edid;         // EDID
	unsigned short  len;            // length of edid
...
{
	unsigned char   port;          // HDMI Port
	unsigned char   n_vsif;        // number of vsif
...
typedef struct
{
    unsigned short   ctrl;           // command id
    unsigned short   rw:1;           // 0 : write, 1 : read 
    unsigned short   port:7;         // port id
    unsigned short   reserved:8;     // reserved for command specific usage
    unsigned int   param;          // parameter 1
}vfe_hdmi_ctrl;
...
#define VFE_IOC_HDMI_ENABLE_Z0_NP_SWITCH		_IOWR(VFE_IOC_MAGIC, VFE_IOC_HDMI_IDX_52, unsigned char)
#define VFE_IOC_HDMI_DISABLE_Z0_NP_SWITCH		_IOWR(VFE_IOC_MAGIC, VFE_IOC_HDMI_IDX_53, unsigned char)
