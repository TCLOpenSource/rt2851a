...
typedef struct
{
	...
	unsigned int  get_delay_Result;
}VSC_GET_SCALER_VIDEO_DELAY_T;
...
typedef struct
{
	...
	unsigned int  adaptivestreambonoff;
...
typedef struct
{
	...
	unsigned short scanType;	/**< ... */
	unsigned short phase;	    /**< ... */  // need to check unsigned short
...
typedef struct _VSC_OVERSCAN_INFO{
	KADP_VIDEO_WID_T wid;
	unsigned char ucInput_H_Overscan;
	unsigned char ucInput_V_Overscan;
	unsigned char ucInput_H_Shift;
	unsigned char ucInput_V_Shift;
	unsigned char ucOutput_H_Overscan;
	unsigned char ucOutput_V_Overscan;
	unsigned char ucOutput_H_Shift;
	unsigned char ucOutput_V_Shift;
}VSC_OVERSCAN_INFO;
...
typedef struct
{
	...
	unsigned short overlapSize; // overlap pixel size when split the HDMI video.
	KADP_VIDEO_RECT_T resolution; // resolution.
	unsigned short vFreq; // v frequency
...
#define VSC_IOC_SUBSCRIBE_VIDEO_DELAY 			_IOW(VSC_IOC_MAGIC, VSC_IOC_IDX_93, unsigned int)
#define VSC_IOC_SET_SDR_MAX_RGB                 _IOW(VSC_IOC_MAGIC,VSC_IOC_IDX_94, unsigned char)
