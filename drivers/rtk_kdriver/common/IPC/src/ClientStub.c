...
#endif
{
    ...
    XDR xdrs;
    char mem_ToShm0[200] = {0};// process rpc data within 200 bytes before writing to kernel
    ...
    // so we do this first.
    if(sizeof(RPC_STRUCT) + args_size > 200){
        mem_ToShm = (char *)malloc(sizeof(RPC_STRUCT) + args_size);
    }else
        mem_ToShm = mem_ToShm0;
   ...
   int        opt)
{
    RPC_STRUCT rpc;
    XDR xdrs;
    char mem_ToShm0[128] = {0};// process rpc reply data before writing to kernel
    char *mem_ToShm = mem_ToShm0;
