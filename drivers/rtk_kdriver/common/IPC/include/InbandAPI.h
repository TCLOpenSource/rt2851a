...
typedef enum
{
  ...
  /* VP8 software decode In-band command */
  INBAND_CMD_TYPE_VP8_SW_DECODE = 200,
...
typedef struct
{
  ...
  */
  ...
  unsigned int variable_frmrate;        /* indicate the video is variable_framerate */ 
...
#define MAX_WIDTH_SW  1920
#define MAX_HEIGHT_SW 1088
#define MAX_MB_ROWS_SW (((MAX_HEIGHT_SW >> 4) + 1) & (~1))
#define MAX_MB_COLS_SW (((MAX_WIDTH_SW  >> 4) + 1) & (~1))

typedef struct
{
  unsigned char  sb_inter_pred_opt;
  unsigned char  sb_uv_mode;
  unsigned char  sb_y_mode;
  unsigned char  sb_ref_idx;
  unsigned int   sb_part_info;
  unsigned int   sb_intra_mode[2];
  unsigned int   sb_ref[20];
} VP8_SIDEBAND_DATA;

typedef struct
{
  unsigned char  CoeffProbTable[2304]   __attribute__((aligned(8)));
  unsigned char  FrameType;
  unsigned char  show_frame;
  unsigned char  use_bilinear_mc_filter;
  unsigned char  MBrows;
  unsigned char  MBcols;
  ...
  char           uvdc_delta_q;
  char           uvac_delta_q;
  unsigned char  refresh_golden_frame;
  unsigned char  refresh_alt_ref_frame;
  unsigned char  copy_buffer_to_gf;
  unsigned char  copy_buffer_to_arf;
  unsigned char  refresh_last_frame;
  unsigned int   first_partition_length_in_bytes;
  unsigned short Width;
  unsigned short Height;
  unsigned int   VP8_P2_MB1[MAX_MB_COLS_SW*MAX_MB_ROWS_SW];
  VP8_SIDEBAND_DATA sb_data[MAX_MB_COLS_SW*MAX_MB_ROWS_SW];
} VP8_DECODE_PICTURE_P3;
