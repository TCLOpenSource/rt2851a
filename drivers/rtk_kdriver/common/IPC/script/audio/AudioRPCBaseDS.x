...
enum AUDIO_IO_PIN {
	...
	PCM_OUT3 = 23,
    FLASH_AUDIO_PIN_9 = 100, /*continuous flash use*/
...
struct AUDIO_RPC_PP_AGC {
    ...
    rpc_s32_t gain_rise;
    rpc_s32_t gain_drop;
    rpc_s32_t gain_abrupt_drop;
    rpc_s32_t judge_sample;
};
...
enum ENUM_AUDIO_IPT_SRC
{
	...
	AUDIO_IPT_SRC_DMIC0        = 0x8,	/* real input, mapping to register bit-setting */
	AUDIO_IPT_SRC_DMIC1        = 0x9,	/* real input, mapping to register bit-setting */
	AUDIO_IPT_SRC_ASRCIN       = 0xa,	/* real input, mapping to register bit-setting */
	...
	AUDIO_IPT_SRC_I2S_SEC_CH12 = 0xFE,	/* real input, mapping to register bit-setting */
...
struct	AUDIO_RPC_FORMAT_INFO {
	...
	rpc_s32_t	VariableBitRateFlag;	/* 1 = Variable bit rate, 0 = constant bit rate */
	rpc_s32_t	BitRate;
	u_char copyright;

	rpc_s32_t rsv_data[4];
};
...
struct AUDIO_DEC_CFG {
    rpc_s32_t scalehi;
    rpc_s32_t scalelo;
    rpc_s32_t scalehi_2ch;
    rpc_s32_t scalelo_2ch;
	u_short mode;
	u_short LFEMode;
	u_short dualmono;
	u_short comprMode;
...
struct AUDIO_RPC_HASH {
	rpc_s32_t rand_value;
};
struct AUDIO_FRAME_BOUNDARY {
	rpc_s32_t frameBeginPtr;
	rpc_s32_t writeBeginPtr;
	rpc_u32_t writeSize;
};
...
struct AUDIO_RPC_RINGBUFFER {
	rpc_s32_t instanceID;
	rpc_s32_t in_beginPAddrList[8];
	rpc_s32_t in_listSize;
	rpc_s32_t in_bufSize;
	rpc_s32_t in_pinID;
	rpc_s32_t extin_beginPAddrList[8];
	rpc_s32_t extin_listSize;
	rpc_s32_t extin_bufSize;
	rpc_s32_t extin_pinID;
	rpc_s32_t out_beginPAddrList[8];
	rpc_s32_t out_listSize;
	rpc_s32_t out_bufSize;
	rpc_s32_t out_pinID;
};

struct AUDIO_RPC_IOBUFFER {
    rpc_s32_t instanceID;
    rpc_s32_t in_ptrList[8];
    rpc_s32_t in_bufSize;
    rpc_s32_t in_listSize;
    rpc_s32_t out_ptrList[8];
    rpc_s32_t out_bufSize;
    rpc_s32_t out_listSize;
};
...
struct AUDIO_RPC_SWITCHAGENT {
	rpc_s32_t instanceID1;
	rpc_s32_t instanceID2;
...
struct LINK_BUFFER{
   rpc_u32_t magicNumber; /* 0x8088CAFE,  this is the magic sync word, just to verify the buffer */
   rpc_u32_t Reserved;
   rpc_u32_t Next;  /* The physical address of next linked buffer. If Next==0, means end of the buffer list */
   rpc_u32_t Size;  /* size of the following data payload, count as bytes. The Size does not include this header part. */
...
struct AUDIO_SOUND_EVENT{
    rpc_s32_t instanceID;
	AUDIO_PCM_FORMAT pcmFormat;
    rpc_s32_t endianType;		/* AUDIO_ENDIANTYPE */
    rpc_s32_t pBitstreamRB;      /* physical address of bitstream buffer  as of LinkBuffer structure*/
    rpc_s32_t iteration;                 /* iterations the sample should be played for */
};
...
{
	rpc_s32_t enable;	                /*0:disable ; 1:enable*/
	rpc_s32_t bandnum;                   /* enum ENUM_AUDIO_SPECTRUM_BANDNUM */
	rpc_s32_t data_addr;                 /* physical addr */
	rpc_s32_t upper_bandlimit_addr;      /* physical addr */
};
...
{
	rpc_s32_t 	instanceID;
	rpc_s32_t	ChannelStatus[6];		/* 192 / 32 = 6 */
};
...
struct	AUDIO_RPC_ENC_ELEM_FRAME_INFO
{
	enum ENUM_DVD_AUDIO_ENCODER_OUTPUT_INFO_TYPE infoType;
        u_int   frameNumber;
	rpc_u32_t	PTSH;			/* The PTS at the begin of the frame */
	rpc_u32_t	PTSL;
/*	rpc_s32_t	wptr;*/
	rpc_s32_t	frameSize;		/* since it could be VBR, so the frame size should be given explicitly. */
	rpc_s32_t	NumberOfFrame;	/* 1 is suggested, otherwise, system need to interpolated PTS later. */
};
...
struct	AUDIO_RPC_AIN_ELEM_FRAME_INFO
{
	...
	rpc_u32_t	PTSH;			/* The PTS at ptr position */
	rpc_u32_t	PTSL;
	rpc_s32_t ptr;
	rpc_s32_t leng;
};
...
struct	AUDIO_PROLOGIC2_MODE
{
	...
	rpc_s32_t   abaldisable;			/* Auto-Balance [1:0] = [disable,enable] */
	rpc_s32_t   chanconfig;			/* Output Channel Select [7:0] = [3/2,2/2,Rsd,Rsd,3/0,Rsd,Rsd,Rsd] */
	rpc_s32_t   dimset;			/* Dimension Control [7:0] = [Rsd,3(center),2,1,0,-1,-2,-3(surround)] */
	rpc_s32_t   surfiltenable;			/* Surround Shelf-Filter [1:0] = [enable,disable] */
	rpc_s32_t   modeselect;			/* Decoder Mode Select [4:0] = [Mat.,Mov.,Mus.,Vir.,PL_Emu] */
	rpc_s32_t   panoramaenable;			/* Panorama [1:0] = [enable,disable] */
	rpc_s32_t   fix_pcmscalefac;			/* PCM scale factor(1,-1) ,defualt is 1.0  */
	rpc_s32_t   rsinvenable;			/* Right Surround polarity inversion [1:0] = [enable,disable] */
	rpc_s32_t   samprateid;			/* PCM sample rate [2:0] = [32kHz,44.1kHz,48kHz] */
	rpc_s32_t   cwidthset;			/* Center Width Setting [7:0] = [90(phantom center),69,2,62,54,36,28,20,8,0] */
};
...
{
	rpc_s32_t		instanceID;
	rpc_s32_t   		mode;		/* mode =0 then less computation, mode =1 then better quality */
};

struct	AUDIO_RPC_MIXER_MODE
{
	rpc_s32_t		simpleMix;		/* if simpleMix = 1, it is supposed to mix the inputs for a rpc_s32_t time. */
					/* when simpleMix = 1, the exclusiveMode is ignored */
	rpc_s32_t		exclusiveMode;	/* the last 4 bits (LSB) represents the 4 input pins, 1 = enable, 0 = disbale */
...
struct AUDIO_PTS_MIXER {
	...
	rpc_s32_t	mixer_in_ena;
	rpc_s32_t	mixer_gain[8];
};
...
{
	rpc_s32_t		mode;		/* 0:customer designed, 1:club mode, 2:dance mode, 3:live mode, 4:pop mode, 5:rock mode,   6:loudness(powerful) */
	rpc_s32_t		gain[10];		/* for customer designed, gain of each band */
};
...
{
	rpc_s32_t		cutoffFreq;		/* 80/100/120 Hz */
	rpc_s32_t		config_mode;	/* mode : 0~7 */
	rpc_s32_t		subwoofer_ena;	/* 0:disable, 1:enable; for configuration 2 ,3, simplified */
};
...
{
	rpc_s32_t		wide_mode;	/* 0:reference mode, 1:wide mode */
};
...
struct	AUDIO_REVERB_CONFIG
{
	...
	unsigned char	ena;
	rpc_s32_t		pinID;
	rpc_s32_t		reverb_mode;	/* 0:Cathedral1, 1:Cathedral2, 2:Hall, 3:Room, 4:Basement*/
};
...
enum AUDIO_CONFIG_CMD_MSG
{
    ...
    AUDIO_CONFIG_CMD_GET_LATENCY = 139,
    AUDIO_CONFIG_CMD_DIAS_LOW_POWER = 140,
    AUDIO_CONFIG_CMD_AO_LOW_LATENCY = 141,
    AUDIO_CONFIG_CMD_AEC_SOURCE = 142
};
...
{
	rpc_s32_t		mode;		/* 0:customer designed, 1:club mode, 2:dance mode, 3:live mode, 4:pop mode, 5:rock mode,   6:loudness(powerful) */
	rpc_s32_t		delay;		/* enum AUDIO_AO_ECHO_DELAY_MODE */
	rpc_s32_t		fdbk_gain;
};
...
{
	rpc_s32_t HDMI_output_enable;
	rpc_s32_t EDID_DATA_addr;
};
...
struct AUDIO_RPC_DEC_FORMAT_INFO {
       ...
       rpc_s32_t nSamplesPerSec;
       rpc_s32_t nAvgBytesPerSec;   /* -1 for VBR ? */
       rpc_s32_t reserved[4];      /* reserved[0] for mpeg  (layer<<8)&0xFF00) | (mpegN)&0xFF */
...
enum AUDIO_ENUM_PRIVAETINFO
{
    ...
    ENUM_PRIVATEINFO_AUDIO_GET_CURR_AO_VOLUME,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_PARAM,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_PARAM,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_DIALOGUE_ENHANCER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_DIALOGUE_ENHANCER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VOLUME_LEVELER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VOLUME_LEVELER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_INTELLIGENT_EQUALIZER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_INTELLIGENT_EQUALIZER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_SURROUND_DECODER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_SURROUND_DECODER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_MEDIA_INTELLIGENCE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_MEDIA_INTELLIGENCE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_SURROUND_VIRTUALIZER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_SURROUND_VIRTUALIZER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_GRAPHICAL_EQUALIZER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_GRAPHICAL_EQUALIZER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_AUDIO_OPTIMIZER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_AUDIO_OPTIMIZER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_AUDIO_REGULATOR,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_AUDIO_REGULATOR,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_BASS_ENHANCER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_BASS_ENHANCER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VIRTUAL_BASS,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VIRTUAL_BASS,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_BASS_EXTRACTION,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_BASS_EXTRACTION,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_DMX_TYPE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_DMX_TYPE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VIRTUALIZER_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VIRTUALIZER_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_HEADPHONE_REVERB,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_HEADPHONE_REVERB,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_SPEAKER_START,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_SPEAKER_START,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_DE_DUCKING,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_DE_DUCKING,
    ENUM_PRIVATEINFO_AUDIO_HANDLE_TRUVOLUMHD_PARAM,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_PREGAIN,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_PREGAIN,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_POSTGAIN,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_POSTGAIN,
    ENUM_PRIVATEINFO_AUDIO_GET_ATMOS_INFO,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_B_VIRTUAL_BASS_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_B_VIRTUAL_BASS_ENABLE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_SURROUND_VIRTUALIZER_MODE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_SURROUND_VIRTUALIZER_MODE,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VOLUME_LEVELER_IO,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VOLUME_LEVELER_IO,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VOLUME_MODELER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VOLUME_MODELER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VOLUME_MAXIMIZER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VOLUME_MAXIMIZER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_SURROUND_COMPRESSOR,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_SURROUND_COMPRESSOR,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_PERCEPTUAL_HEIGHT_FILTER,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_PERCEPTUAL_HEIGHT_FILTER,
    ENUM_PRIVATEINFO_AUDIO_SET_DAP_VIRTUALIZER_SPEAKER_ANGLE,
    ENUM_PRIVATEINFO_AUDIO_GET_DAP_VIRTUALIZER_SPEAKER_ANGLE,
    ENUM_PRIVATEINFO_AUDIO_RELEASE_PCM_IN_PIN_BY_PROCESS_KILLED,
    ENUM_PRIVATEINFO_AUDIO_RELEASE_FLASH_PIN_BY_PROCESS_KILLED,
    ENUM_PRIVATEINFO_AUDIO_GET_GST_HW_FLASH_PIN,
    ENUM_PRIVATEINFO_AUDIO_GET_GST_SW_FLASH_PIN,
    ENUM_PRIVATEINFO_AUDIO_SKIP_DEC_FLUSH,
    ENUM_PRIVATEINFO_AUDIO_GET_ENDIAN,
    ENUM_PRIVATEINFO_AUDIO_GET_SNDOUT_LATENCY
};
...
{
	rpc_u32_t audioPassThroughType;
...
{
    rpc_u32_t command;
    rpc_u32_t usb_ai_ringbuf_addr;  /* addr of ring buffer struct AUDIO_RINGBUF_PTR */
    rpc_u32_t usb_ai_samplerate;    /* bit 0~13: 32/44.1/48/88.2/96/176.4/192/22.05/24/64/16/8/11.025/12 kHz */
    rpc_u32_t usb_ai_format;        /* ref to USB_AUDIO_IN_SAMPLE_FORMAT, always big-endian */
    rpc_u32_t usb_ai_max_chnum;     /* up to 2-ch currently */
};
...
{
    rpc_s32_t instanceID;
    rpc_u32_t     type;             /* enum USB_AUDIO_IN_TYPE */
...
struct AUDIO_RPC_LGSE_SETMODE
{
	rpc_s32_t instanceID;
	short dataOption;
	rpc_s32_t flag;       /* flag = 0 : bypass ,flag > 0 : enabled*/
};
...
struct AUDIO_RPC_LGSE_SETFN000
{
	...
	rpc_s32_t rpc_LGSE00855_set_address;
...
struct AUDIO_RPC_LGSE_SETFN001
{
	...
	rpc_s32_t rpc_LGSE00869_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN008
{
    ...
    rpc_s32_t rpc_LGSE00858_set_address;
...
struct AUDIO_RPC_LGSE_SETFN014
{
    ...
    rpc_s32_t rpc_LGSE00853_set_address;
...
struct AUDIO_RPC_LGSE_SETFN016
{
  rpc_s32_t instanceID;
  short dataOption;
  /*option = 0 ,_LGSE03520 for init*/
    rpc_s32_t rpc_LGSE03520_set_address;
  /*option = 1 ,_LGSE03521 for var*/
    rpc_s32_t rpc_LGSE03521_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN017
{
    ...
    rpc_s32_t rpc_LGSE00879_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN019
{
	...
	rpc_s32_t rpc_LGSE00856_set_address;
...
struct AUDIO_RPC_LGSE_SETFN022
{
	...
	rpc_s32_t rpc_LGSE00878_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN018
{
	...
	rpc_s32_t rpc_LGSE00862_set_address;
...
{
  rpc_s32_t _LGSE02582;
  rpc_s32_t _LGSE02590;
  rpc_s32_t _LGSE02591[2];
  rpc_s32_t _LGSE02592[3];
  rpc_s32_t _LGSE02605[2];
  rpc_s32_t _LGSE02606[3];
};
...
struct AUDIO_RPC_LGSE_SETFN026
{
    ...
    rpc_s32_t rpc_data026_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN027
{
    ...
    rpc_s32_t rpc_data027_set_address;
};
...
struct AUDIO_RPC_LGSE_SETFN029
{
    ...
    rpc_s32_t rpc_LGSE02869_set_address;
};

enum ENUM_TRUVOLUMEHD_CMD_TYPE
{
    ENUM_TRUVOLUMEHD_CMD_TYPE_SETPARAM = 0,
    ENUM_TRUVOLUMEHD_CMD_TYPE_GETPARAM
};

enum enum_dtsParamTruvolumeHDType {
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_FIRST_PARAM = 0x10016350,
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_ENABLE_I32 = ENUM_DTS_PARAM_LOUDNESS_CONTROL_FIRST_PARAM,                     /**< Bypass/in-line the loudness control algorithm. \n Type: int. \n Range: 0 - Bypass, 1 - In-line. \n Default: 0. */
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_IO_MODE_I32,                                                             /**< Input output Mode Type: dtsInt32; Range: 0 - 4 ; Default: 0. */
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_TARGET_LOUDNESS_I32,                                                     /**< level that the algorithm will attempt to achieve upon output. \n Type: int. \n Range: -40 to 0dB. Resolution: 1. \n Default: -16dB. */
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_PRESET_I32,                                                              /**< A collection of settings to achieve a loudness leveling objective. The settings do not include the Enable or Target Loudness parameters. Those parameters should therefore be set separately from selecting a preset. \n Type: dtsInt32; \n Range: 0 - Light, 1 - Medium, 2 - Aggressive; \n Default: 0. */
    ENUM_DTS_PARAM_LOUDNESS_CONTROL_CONTROLS_STU,                                                            /**< Control structure, see #_dtsLoudnessControlApiCtrl. */
...
enum ENUM_VIRTUALX_CMD_TYPE
{
	...
	ENUM_VIRTUALX_CMD_TYPE_TRUSRNDX_GETPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_SETPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_GETPARAM,
	ENUM_VIRTUALX_CMD_TYPE_MBHL_SETPARAM,
	ENUM_VIRTUALX_CMD_TYPE_MBHL_GETPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TRUSRNDX_SETALLPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TRUSRNDX_GETALLPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_SETALLPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_GETALLPARAM,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_SETCOEFF,
	ENUM_VIRTUALX_CMD_TYPE_TBHDX_GETCOEFF
};
...
enum enum_dtsParamVirtualxlib1Type
{
    ...
    ENUM_DTS_PARAM_VX_INPUT_MODE_I32,





    ENUM_DTS_PARAM_VX_OUTPUT_MODE_I32,



    ENUM_DTS_PARAM_VX_HEADROOM_GAIN_I32,




    ENUM_DTS_PARAM_VX_PROC_OUTPUT_GAIN_I32,





    ENUM_DTS_PARAM_VX_REFERENCE_LEVEL_I32,





    ENUM_DTS_PARAM_TSX_ENABLE_I32,




    ENUM_DTS_PARAM_TSX_PASSIVEMATRIXUPMIX_ENABLE_I32,
    ...
    ENUM_DTS_PARAM_TSX_HEIGHT_UPMIX_ENABLE_I32,
    ...
    ENUM_DTS_PARAM_TSX_LPR_GAIN_I32,




    ENUM_DTS_PARAM_TSX_CENTER_GAIN_I32,




    ENUM_DTS_PARAM_TSX_HORIZ_VIR_EFF_CTRL_I32,





    ENUM_DTS_PARAM_TSX_HEIGHTMIX_COEFF_I32,



    ENUM_DTS_PARAM_TSX_PROCESS_DISCARD_I32,


    ENUM_DTS_PARAM_TSX_HEIGHT_DISCARD_I32,
    ENUM_DTS_PARAM_TSX_FRNT_CTRL_I32,
    ENUM_DTS_PARAM_TSX_SRND_CTRL_I32,


    ENUM_DTS_PARAM_VX_DC_ENABLE_I32,




    ENUM_DTS_PARAM_VX_DC_CONTROL_I32,



    ENUM_DTS_PARAM_VX_DEF_ENABLE_I32,




    ENUM_DTS_PARAM_VX_DEF_CONTROL_I32,



    ENUM_DTS_PARAM_TSX_CONTROLS_STU,
...
enum enum_dtsParamTBHDxType
{
    ...
    ENUM_DTS_PARAM_TBHDX_MONO_MODE_I32,




    ENUM_DTS_PARAM_TBHDX_SPKSIZE_I32,
    ...
    ENUM_DTS_PARAM_TBHDX_MAXGAIN_I32,



    ENUM_DTS_PARAM_TBHDX_HP_ENABLE_I32,




    ENUM_DTS_PARAM_TBHDX_HPORDER_I32,



    ENUM_DTS_PARAM_TBHDX_TEMP_GAIN_I32,



    ENUM_DTS_PARAM_TBHDX_CUSTOMCOEFF_STU,

    ENUM_DTS_PARAM_TBHDX_CONTROLS_STU,
...
enum enum_dtsParamMbhlType
{
    ...
    ENUM_DTS_PARAM_MBHL_BYPASS_GAIN_I32,




    ENUM_DTS_PARAM_MBHL_REFERENCE_LEVEL_I32,
    ...
    ENUM_DTS_PARAM_MBHL_VOLUME_I32,





    ENUM_DTS_PARAM_MBHL_VOLUME_STEP_I32,
    ...
    ENUM_DTS_PARAM_MBHL_BALANCE_STEP_I32,
    ...
    ENUM_DTS_PARAM_MBHL_OUTPUT_GAIN_I32,




    ENUM_DTS_PARAM_MBHL_BOOST_I32,
    ...
    ENUM_DTS_PARAM_MBHL_THRESHOLD_I32,
    ...
    ENUM_DTS_PARAM_MBHL_SLOW_OFFSET_I32,





    ENUM_DTS_PARAM_MBHL_FAST_ATTACK_I32,
    ...
    ENUM_DTS_PARAM_MBHL_FAST_RELEASE_I32,



    ENUM_DTS_PARAM_MBHL_SLOW_ATTACK_I32,



    ENUM_DTS_PARAM_MBHL_SLOW_RELEASE_I32,



    ENUM_DTS_PARAM_MBHL_DELAY_I32,
    ...
    ENUM_DTS_PARAM_MBHL_ENVELOPE_FREQUENCY_I32,
    ...
    ENUM_DTS_PARAM_MBHL_CROSS_LOW_I32,
    ...
    ENUM_DTS_PARAM_MBHL_CROSS_MID_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_ATTACK_I32,





    ENUM_DTS_PARAM_MBHL_COMP_LOW_RELEASE_I32,





    ENUM_DTS_PARAM_MBHL_COMP_LOW_RATIO_I32,




    ENUM_DTS_PARAM_MBHL_COMP_LOW_THRESH_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_LOW_MAKEUP_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_MID_RELEASE_I32,





    ENUM_DTS_PARAM_MBHL_COMP_MID_RATIO_I32,




    ENUM_DTS_PARAM_MBHL_COMP_MID_THRESH_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_MID_MAKEUP_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_HIGH_RELEASE_I32,





    ENUM_DTS_PARAM_MBHL_COMP_HIGH_RATIO_I32,




    ENUM_DTS_PARAM_MBHL_COMP_HIGH_THRESH_I32,
    ...
    ENUM_DTS_PARAM_MBHL_COMP_HIGH_MAKEUP_I32,
    ...
    ENUM_DTS_PARAM_MBHL_GAIN_REDUCTION_I32,



    ENUM_DTS_PARAM_MBHL_COMP_GAIN_REDUCTION_I32,



    ENUM_DTS_PARAM_MBHL_COEF_STU,
    ENUM_DTS_PARAM_MBHL_CONTROLS_STU,
    ENUM_DTS_PARAM_MBHL_LAST_PARAM = ENUM_DTS_PARAM_MBHL_CONTROLS_STU,

    ENUM_DTS_PARAM_MBHL_APP_FIRST_PARAM = 0x10027300, /*DTS_PARAM_MBHL_APP_FIRST_PARAM = DTS_PARAM_MBHL_FIRST_PARAM + 0x1000*/
                                                      /*DTS_PARAM_MBHL_FIRST_PARAM = 0x10026300*/
    ENUM_DTS_PARAM_MBHL_APP_FRT_LOWCROSS_F32,

    ENUM_DTS_PARAM_MBHL_APP_FRT_MIDCROSS_F32,

    ENUM_DTS_PARAM_MBHL_APP_LAST_PARAM
};
...
enum DTS_TSX_OUTPUT_MODE
{
    kVXOut_Stereo = 0
};

enum DTS_TSX_HORIZN_EFFECT_CTRL
{
    kTSX_HorizEffct_Default,
    kTSX_HorizEffct_Mild
};

struct dtsVXApiCtrl_t
{
    int                     nVXLib1Enable;
    DTS_TSX_INPUT_MODE           nInputMode;
    DTS_TSX_OUTPUT_MODE          nOutputMode;
    int                     nHeadRoom;
    int                     nProOutputGain;
    int                     nReferenceLevel;
};

struct dtsTSXApiCtrl_t
{
    int                     nTsxEnable;
    int                     nPassiveMtrxEnable;
    int                     nHeightUpmixEnable;
    DTS_TSX_HORIZN_EFFECT_CTRL   nHorizEffectStrength;
    int                     nLpRtoCtrMixGain;
    int                     nHeightMixCoeff;
};

struct dtsVirtualXLib1SubCompApiCtrl_t
{
    int                     nEnable;
    int                     nControl;
};

struct AUDIO_VIRTUALX_TRUSRNDX_ALLPARAM_INFO
{

    dtsVXApiCtrl_t                  vxApiControls;


    dtsTSXApiCtrl_t                 tsxApiControls;


    dtsVirtualXLib1SubCompApiCtrl_t focusApiControls;


    dtsVirtualXLib1SubCompApiCtrl_t defApiControls;
};

struct dtsTBHDXCustomFilters
{
    int    LPF1stCoef[4];
    int    LPF2ndCoef[6];
    int    HPF1stCoef[4];
...
struct AUDIO_VIRTUALX_TBHDX_ALLPARAM_INFO
{
    int              nEnable;
    int              nProcMode;
    int              nSpkSize;
    ...
    int              nHpOrder;
    int              nBassLvl;
    dtsTBHDXCustomFilters stCustomFilters;
};
**/
...
struct AUDIO_RPC_TRUVOL_MODE {
    ...
    int BypassGain;   /* 0.0~1.0  for BypassGain, default is 1.0 */
    int mode;         /* 0 for light mode, 1 for normal mode, 2 for heavy mode  */
    int MaxGain;      /* 0.25~256 for volume Max gain, default is 16 */
    int NormalizerEnable; /* 1 for TRUE, 0 for FALSE  */
    int BlockSize; /* 160~464 for Volume Block Size, default is 256*/
};
...
{
    rpc_s32_t module_type;  /* AI or AO*/
    char professional;
    char data_type;
    char copyright;
    char pre_emphasis;
...
{
  rpc_s32_t                        MaxSpeed;
  rpc_s32_t                        MaxReverseSpeed;
  rpc_s32_t                        MinSkip;
  rpc_s32_t                        MaxSkip;
  rpc_s32_t                        MinReverseSkip;
  rpc_s32_t                        MaxReverseSkip;
} ;
...
struct AUDIO_RPC_SET_A2DP_CLOCK
{
    int output_pin;
    float speed;
