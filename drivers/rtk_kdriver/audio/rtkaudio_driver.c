...
void RHAL_AUDIO_HDMI_SetAudioReturnChannel(bool *onoff)
{
	*onoff = ARC_offon;
}

void RHAL_AUDIO_HDMI_SetEnhancedAudioReturnChannel(bool *onoff)
{
	*onoff = eARC_offon;
}
...
long rtkaudio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RTKAUDIO_IOC_GET:
	{
        ...
        case GET_OPTEE_CARVEOUT_MEM_INFO:
        {
            ...
            pr_debug("GET_OPTEE_CARVEOUT_MEM_INFO\n");
            ...
            pr_debug("GET_OPTEE_CARVEOUT_MEM_INFO done\n");
            break;
        }
		default:
			ret = -EFAULT;
	...
	case RTKAUDIO_IOC_OMX_CREATE_DECODER:
	{
		if (rtkaudio_dec_agent == -1) {
			pr_debug("[rtkaudio] RTKAUDIO_IOC_OMX_CREATE_DECODER\n");
	...
	{
		pr_debug("[rtkaudio] RTKAUDIO_IOC_OMX_GET_OFFLOAD_DECODER\n");
		...
		if ((!rtkaudio_dec_inuse) && (rtkaudio_dec_agent != -1)) {
			...
			pr_debug("[rtkaudio] omx decoder is using or did not create, create decoder through orignal flow.\n");
			ret = -EFAULT;
		}
		mutex_unlock(&dec_usage_lock);

		pr_debug("[rtkaudio] RTKAUDIO_IOC_OMX_GET_OFFLOAD_DECODER done\n");
		break;
	}
	case RTKAUDIO_IOC_OMX_RELEASE_OFFLOAD_DECODER:
	{
		pr_debug("[rtkaudio] RTKAUDIO_IOC_OMX_RELEASE_OFFLOAD_DECODER\n");

		mutex_lock(&dec_usage_lock);
		if (rtkaudio_dec_agent != -1) {
			rtkaudio_dec_inuse = false;
		} else {
			pr_debug("[rtkaudio] omx decoder did not create.\n");
		}
		mutex_unlock(&dec_usage_lock);

		pr_debug("[rtkaudio] RTKAUDIO_IOC_OMX_RELEASE_OFFLOAD_DECODER done\n");
		break;
	}
	...
	case RTKAUDIO_IOC_SET_DMX_MODE:
	{
		...
		pr_info("[rtkaudio] set downmix mode %d\n", dmx_mode);
	...
	const char *buf, size_t count, loff_t *f_pos)
{
	...
	if ((cmd_buf[0] == 'f') && (count == 2)) {
		...
		pr_info("[rtkaudio] set earc status %s\n", (eARC_offon ? "ON" : "OFF"));
		...
		pr_info("[rtkaudio] set arc status %s\n", (ARC_offon ? "ON" : "OFF"));
	} else if ((cmd_buf[0] == 'a') && (cmd_buf[1] == 'r') &&
			   ...
			   (cmd_buf[8] == 's')) {
		bool onoff;
		RHAL_AUDIO_HDMI_SetAudioReturnChannel(&onoff);
		pr_info("[rtkaudio] arc  status %s\n", (onoff ? "ON" : "OFF"));
		RHAL_AUDIO_HDMI_SetEnhancedAudioReturnChannel(&onoff);
		pr_info("[rtkaudio] earc status %s\n", (onoff ? "ON" : "OFF"));
	} else {
		pr_notice("%s\n", cmd_buf);
	}
	 ...
	 unsigned long *retvalue)
{
	...
	do {
		...
		if (ret == RPC_FAIL) {
			pr_err("[rtkaudio] RPC to ACPU fail!!\n");
			return -1;
		}
	...
	if (ret == RPC_ACPU_NOT_READY) {
		pr_err("[rtkaudio] wait ACPU ready timeout!!!\n");
		return -1;
	}
#else
	...
	command == ENUM_KERNEL_RPC_GET_MUTE_N_VOLUME) {
		if (*retvalue != S_OK) {
		   pr_err("[rtkaudio] RPC S_OK fail\n");
		   pr_err("[rtkaudio] retvalue %lx\n", *retvalue);
		   ret = -1;
		}
	} else {
		if (*retvalue != S_OK || ntohl(audio_ret->result) != S_OK) {
			pr_err("[rtkaudio] RPC S_OK fail\n");
			pr_err("[rtkaudio] retvalue %lx, result %x, command %lx\n",
...
static int rtkaudio_create_decoder_agent(struct platform_device *rtkaudio_dev)
{
	...
	rtkaudio_dev->dev.dma_mask = &rtkaudio_dev->dev.coherent_dma_mask;
	if(dma_set_mask(&(rtkaudio_dev->dev), DMA_BIT_MASK(32))) {
		pr_err("[rtkaudio] %d:DMA not supported\n",__LINE__);
	}

	p = dma_alloc_coherent(&(rtkaudio_dev->dev), RPC_DVR_MALLOC_UNCACHED_SIZE, &dat, GFP_ATOMIC);

	if (!p) {
		pr_err("[rtkaudio] %d alloc memory fail\n", __LINE__);
		return -ENOMEM;
	}
	...
	if (audio_send_rpc_command(RPC_AUDIO,
		...
		(unsigned long) res,
		&ret)) {
		pr_err("[rtkaudio] %s %d RPC fail\n", __func__, __LINE__);
		dma_free_coherent(&(rtkaudio_dev->dev), RPC_DVR_MALLOC_UNCACHED_SIZE, p, dat);
		return -1;
	}
	...
	dma_free_coherent(&(rtkaudio_dev->dev), RPC_DVR_MALLOC_UNCACHED_SIZE, p, dat);
	pr_debug("[rtkaudio] Create decode instance %d\n", rtkaudio_dec_agent);
	return 0;
}
...
static int rtkaudio_get_audio_caps(void)
{
	...
	if (!vir_addr) {
		pr_err("[audio] %d alloc memory fail\n", __LINE__);
		return -ENOMEM;
	}
