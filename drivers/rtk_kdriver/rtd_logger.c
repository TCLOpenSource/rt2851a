...
struct SHARE_STRUCT
{
    ...
    rtdlog_file file_info[MAX_LOGBUF_NUM];
};
...
ssize_t rtd_kernel_write_file (EnumLogbufType type, unsigned char * buf, int count, ssize_t* result)
{
    ...
    if(rtkd_file_info[type].usb_fd)
    {
        ...
        if(rv<0)
        {
            pr_err("%s %d. write usb log file %d \e[1;31mfail\e[0m! ret is %d\n",__func__,__LINE__,type,rv);
        }
    }
    ...
    if(rtkd_file_info[type].fd)
    {
        ...
        if(rv<0)
        {
            pr_err("%s %d. write log file %d \e[1;31mfail\e[0m! ret is %d\n",__func__,__LINE__,type,rv);
            error_flag=rv;
            rv=0;
        }
    }
...
void rtdlog_parse_bufsize (char * str)
{
    ...
    pr_err("%s %d.  warning! rtdlog_bufsize is not setted!\n",__func__,__LINE__);
    rtdlog_bufsize = 0;
}
...
static void dump_rtd_msg_header (rtd_msg_header * start,unsigned int len)
{
    ...
    pr_err("*****************dump_rtd_msg_header start**********************\n");
    pr_err("timestamp = 0x%016llx\n",start->timestamp);
    pr_err("len = 0x%08x\n",start->len);
    pr_err("pid = 0x%08x\n",start->pid);
    pr_err("tid = 0x%08x\n",start->tid);
    pr_err("module = 0x%08x\n",start->module);
    pr_err("text_len = 0x%08x\n",start->text_len);
    pr_err("check_sum = 0x%08x\n",start->check_sum);
    pr_err("name = %s\n",buf);
    pr_err("level = 0x%02x\n",start->level);
    pr_err("cpu_type = 0x%02x\n",start->cpu_type);
    pr_err("cpu_core = 0x%02x\n",start->cpu_core);
    pr_err("reserve0 = 0x%02x\n",start->reserve0);
    pr_err("*****************dump_rtd_msg_header end***********************\n");
}
...
static int check_msghead_valid ( rtd_msg_header * header, bool debug)
{
    if((header->len > header->text_len)&&((header->len - header->text_len) == sizeof(rtd_msg_header)))
    {
        ...
        {
            pr_err("%s %d. msg header checksum error\n",__func__,__LINE__);
            goto check_fail;
        }
        else
        {
...
FAIL:
        ...
        pr_err("type[\e[1;31m%s\e[0m] check msg header fail!\n\n\n",cpu_type_str[type]);
        return -1;
    }
    return 0;
}
...
ssize_t rtd_write_data (EnumLogbufType type)
{
    ...
    {
        pr_err("%s %d. error happen! insert miss log msg for type:%d\n",__func__,__LINE__,type);
        return -2;
    }
...
void rtdlog_sleep (EnumLogbufType type, long write_count )
{
    ...
    if( (write_count>0) && (flag++ > 0 ) )
    {
        ...
        set_freezable();
        flag=0;
        //pr_err("%s %d  sleep for a while ! type is %d \n",__func__,__LINE__, type);
    }
#else
    set_freezable();
    ...
    while(1)
    {
        for(type=ACPU1_BUF; type<MAX_LOGBUF_NUM; type++)
        {
            if(((RTDLOG_BOOT_SHARE_PARAM*) share_buffer)->flag_status==0)
            {
                ...
                pr_err("%s %d.  rtdlog flow suspended!\n",__func__,__LINE__);
                if (val)
                {
                    return val;
                }
...
static int rtdlog_mmap (struct file *file, struct vm_area_struct *vma)
{
    ...
    {
        pr_err("%s %d. cannot remap share memory for rtkd !\n",__func__,__LINE__);
        return -EAGAIN;
    }
    return 0;
...
static int init_rtd_share_buffer (void)
{
    ...
    if(NULL == rtkd_share_buffer)
    {
        pr_err("%s %d. cannot kmalloc share memory for rtkd !\n",__func__,__LINE__);
        return -1;
    }
...
static void init_rtd_buffer (logbuf_global_status * status)
{
    ...
    for(i = 0; i < MAX_LOGBUF_NUM; ++i)
    {
        ...
        //dump kfifo
        pr_err("KFIFO[%d]_INIT:buf(%08lx),size(%08lx),in(%08lx),out(%08lx) (%016lx)\n",
    ...
    *native_logbuf_saver = (unsigned long)(native_logbuf_addr - PAGE_OFFSET);
    pr_err("native logbuf addr : 0x%08lx, CONFIG_PAGE_OFFSET : 0x%08lx\n",(unsigned long)native_logbuf_addr,PAGE_OFFSET);

    share_buffer=(unsigned char *)((u32)rtd_virt_addr+RTD_LOGBUF_BOOT_SHARE_PARAM_OFFSET);
}
...
static int __init rtdlog_init_module (void)
{
    ...
    pr_err("rtdlog:get result:%d,get dev:0x%08x,major:%d\n", result, dev, rtdlog_major);
