...
atomic_t r_cnt = ATOMIC_INIT(0);
atomic_t w_cnt = ATOMIC_INIT(0);
...
int optee_tp_init(void)
{
        ...
        TP_WARNING("TP call tee_client_open_context 1\n");
        tp_ta.ctx = tee_client_open_context(NULL, optee_tp_match, NULL, &vers);
        TP_WARNING("TP call tee_client_open_context 2\n");
        if(tp_ta.ctx == NULL) {
...
void optee_tp_deinit(void)
{
        ...
        timeout = jiffies + HZ;    //timeout 1s
        do{
                mdelay(2);
        }while((atomic_read(&r_cnt) || atomic_read(&w_cnt)) && time_before(jiffies, timeout));

        if(atomic_read(&r_cnt) || atomic_read(&w_cnt))
               TP_WARNING("%s %d wait exit optee time out !! w_cnt=%d r_cnt=%d\n", __func__, __LINE__, atomic_read(&w_cnt), atomic_read(&r_cnt));
 
        TP_WARNING("%s %d enter w_cnt=%d r_cnt=%d\n", __func__, __LINE__, atomic_read(&w_cnt), atomic_read(&r_cnt));
...
static int optee_tp_set_cw(void)
{
        ...
        rc = tee_client_invoke_func(tp_ta.ctx, &arg, param);
        if (rc || arg.ret) {
                pr_err("[%s] optee_meminfo: invoke failed ret %x arg.ret %x\n", __func__, rc, arg.ret);
                ret = -EINVAL;
                goto out;
        }
...
INT32 RHAL_TP_TA_set_mem_prison(UINT32 startAddr, UINT32 endAddr, TP_TA_MEMORY_PRISON_TYPE_T memoryType, UINT8 isEnable)
{
        ...
        if(tp_ta.ctx == NULL || g_tpta_init_cnt == 0) {
                pr_err("[RHAL_TP_TA_set_mem_prison] optee_meminfo: no ta context\n");
                ret = -EINVAL;
                goto out;
        }
...
int optee_tp_test_02(UINT32 para1, UINT32 para2)
{
        ...
        if(last_value != *test_buffer_noncache) {
                TP_WARNING("last_value=0x%x data=0x%x\n", last_value, *test_buffer_noncache);
                last_value = *test_buffer_noncache;
        }
        last_value++;
