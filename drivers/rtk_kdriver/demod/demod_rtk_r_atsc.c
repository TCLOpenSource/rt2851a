	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_ATSC, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_ALTOBEAM_ATBM253:
				REALTEK_R_ATSC_INFO("Tuner id %d, TV_TUNER_ALTOBEAM_ATBM253\n", Param.id);
				pRealtekRATSC->m_pTunerOptReg = (REG_VALUE_ENTRY*) ATBM253_OPT_VALUE_ATSC;
				pRealtekRATSC->m_TunerOptRegCnt = sizeof(ATBM253_OPT_VALUE_ATSC) / sizeof(REG_VALUE_ENTRY);
				break;
			default:
...
int REALTEK_R_ATSC_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO*  pSigInfo)
{
	...
	if (pDemodKernel->m_ScanMode == CH_SCAN_MODE) DtvDemodTPOutEn(1);
