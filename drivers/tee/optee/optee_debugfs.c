...
#ifndef CONFIG_OPTEE_SUPPORT_VERSION_3_X
#define TA_RTK_SVP_UUID STATS_UUID
#else
...
#else /*CONFIG_OPTEE_SUPPORT_VERSION_3_X*/
#define STATS_CMD_MEMLEAK_STATS		2
#define STATS_CMD_GET_CARVEDOUT_ADDR 10
#endif /*CONFIG_OPTEE_SUPPORT_VERSION_3_X*/
...
static int optee_meminfo_init(struct teec_uuid uuid_in)
{
    int ret = 0, rc = 0;
    struct teec_uuid uuid = uuid_in;
...
int optee_carvedout_query(void)
{
	...
	if (optee_meminfo_init(uuid))
		goto out;

	if(meminfo_ta.ctx == NULL) {
		pr_err("%s %d: no ta context\n", __func__, __LINE__);
		ret = -EINVAL;
		goto out;
	}
	...
	svp_module[DISP_CARVEDOUT_ID_VBM][1] = carvedout_buf_query_secure(CARVEDOUT_VDEC_VBM, (void **)&svp_module[DISP_CARVEDOUT_ID_VBM][0]);
    ...
    if (shm_buffer == NULL) {
        pr_err("%s %d carvedout_query: no shm_buffer\n", __func__, __LINE__);
        ret = -ENOMEM;
        goto out;
    }
...
void optee_debugfs_init(char *name)
{
	...
	pr_debug("RTK tee debugfs install success");
    return;
}
