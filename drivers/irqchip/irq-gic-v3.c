...
static void rtk_record_irq_after(void)
{
	...
	if(record_irqs[cpu].clear == 1)
	{
		record_irqs[cpu].max_time = 0;
		record_irqs[cpu].max_irq_number = 0;
		record_irqs[cpu].max_timer_arch_timer = 0;
		record_irqs[cpu].clear = 0;
	}
}
...
void set_irq_records_clear(void)
{
	unsigned int idx = 0;
	for(idx = 0; idx <NR_CPUS; idx++)
		record_irqs[idx].clear = 1;
}
#else
...
void set_irq_records_clear(int)
{
	return;
}
#endif
