...
typedef int (*DVR_ION_FREE_CALLBACK)(unsigned long, void*);

DVR_ION_FREE_CALLBACK gFreeNotify = NULL;


int ion_dvr_free_register_callback( DVR_ION_FREE_CALLBACK cb)
{
...
EXPORT_SYMBOL(ion_dvr_free_register_callback);
...
free_mem:
	...
	dev_dbg(dev, "Release buffer %p addr:0%08x\n", buffer, page_to_phys(info->dvr_page) );

	if( gFreeNotify != NULL && (buffer->flags & ION_FLAG_FOR_VE) )
	{ 
		
		gFreeNotify(page_to_phys(info->dvr_page), info );
	}
	//pr_err("ionFree kern 0x%08x\n", page_to_phys(info->dvr_page) ) ;  //kjw test
	//dump_stack();
