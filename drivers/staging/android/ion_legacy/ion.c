...
unsigned int ion_reclaim_enable = 0;
#endif
...
#ifdef ION_REC_BUFF_DEBUG
static int debug_reclaim_set(void *data, u64 val)
{
	ion_reclaim_enable = (unsigned int)val;

	return 0;
}

static int debug_reclaim_get(void *data, u64 *val)
{
	*val = (u64)ion_reclaim_enable;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(debug_reclaim_fops, debug_reclaim_get,
			debug_reclaim_set, "%llu\n");
#endif
...
#ifdef ION_REC_BUFF_DEBUG
	if(!strcmp(heap->name,"ion_dvr")) {
		char debug_name[64];
		snprintf(debug_name, 64, "%s_cma_reclaim", heap->name);
		...
		if (!debug_file) {
			...
			path = dentry_path(dev->heaps_debug_root, buf, 256);
			pr_err("Failed to create heap cma reclaim debugfs at %s/%s\n",
				path, debug_name);
		}
	}
#endif
