							...
							unsigned long len, unsigned long flags)
{
	...
	pr_debug("Request buffer allocation len %ld\n", len);
...
//		return -EINVAL;
	...
	if (!info->dvr_page) {
		pr_err("Fail to allocate buffer\n");
		goto err;
	}
	...
	if (PageHighMem(info->dvr_page)) {
		unsigned long nr_clear_pages = PAGE_ALIGN(len) >> PAGE_SHIFT;
		struct page *page = info->dvr_page;
		...
		memset(page_address(info->dvr_page), 0, PAGE_ALIGN(len));
	}

	info->phys_addr = page_to_phys(info->dvr_page);
	dma_sync_single_for_cpu(NULL, info->phys_addr, PAGE_ALIGN(len), DMA_BIDIRECTIONAL);
	...
	if (ion_dvr_get_sgtable(info->table, NULL, info->phys_addr, len))
		goto err;
	...
	buffer->phy_address= info->phys_addr;
	
	pr_debug("dvr ion alloc 0x%lx %ld \n", info->phys_addr, len);
...
free_mem:
	...
	pr_debug("Release buffer %x, phy(0x%lx)\n", (unsigned int)buffer, (unsigned long)info->phys_addr);
	/* release memory */
	dvr_free_page_mesg(info->dvr_page);
...
static struct ion_heap *__ion_dvr_heap_create(void)
{
...
static int ion_dvr_heap_create(void)
{
	struct ion_heap *heap;

	heap = __ion_dvr_heap_create();
	if (IS_ERR(heap))
		return PTR_ERR(heap);

	heap->name = "ion_dvr_heap";

	ion_device_add_heap(heap);

	return 0;
}
device_initcall(ion_dvr_heap_create);
