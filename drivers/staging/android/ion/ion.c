...
phys_addr_t ion_to_phys(int fd)
{
	...
	buffer = dmabuf->priv;
	if (buffer == NULL) {
		pr_err("%s() no buffer, dmabuf(%x) fd(%d)\n", __func__, (unsigned int)dmabuf, fd);
		goto err;
	}

	if ((buffer->heap->type != ION_HEAP_TYPE_CARVEOUT) && 
		(buffer->heap->type != ION_HEAP_TYPE_CUSTOM)) {
		pr_err("%s() not sutable heap type(%d) fd(%d)\n", __func__, buffer->heap->type, fd);
		goto err;
	}

	if (buffer->sg_table == NULL) {
		pr_err("%s() no sg table, fd(%d)\n", __func__, fd);
		goto err;
	}

	page = sg_page(buffer->sg_table->sgl);
	if (page == NULL) {
		pr_err("%s() no page in sglist, fd(%d)\n", __func__, fd);
		goto err;
	}
	ret = PFN_PHYS(page_to_pfn(page));

	pr_debug("%s() fd(%d), addr=%x\n", __func__, fd, (unsigned int)ret);
...
RT_ION_FREE_CALLBACK gFreeNotify = NULL;
...
static void _ion_buffer_destroy(struct ion_buffer *buffer)
{
	...
	if (heap->flags & ION_HEAP_FLAG_DEFER_FREE)
		ion_heap_freelist_add(heap, buffer);
			...
			int phy_addr = buffer->phy_address;
	
	
			ion_buffer_destroy(buffer);
	
			if( gFreeNotify != NULL && ( flags & ION_FLAG_FOR_VE) )
			{
