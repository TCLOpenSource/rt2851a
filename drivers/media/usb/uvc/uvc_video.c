...
static int uvc_init_video(struct uvc_streaming *stream, gfp_t gfp_flags)
{
	...
	if (intf->num_altsetting > 1) {
		...
		/*
		*for merlin6/merlin7 EHCI iso tranfser hw bug
		*https://jira.realtek.com/browse/ML7BU-1191?filter=-1
		*https://jira.realtek.com/browse/RL6748-3301
		*/
		/*****if not ehci port, go the old flow**/
		if(udev->bus->busnum != 3) {
			...
			for (i = 0; i < intf->num_altsetting; ++i) {
				...
				/* Check if the bandwidth is high enough. */
				psize = uvc_endpoint_max_bpi(stream->dev->udev, ep);
				if (psize <= 1016 && psize >= best_psize) {
					altsetting = alts->desc.bAlternateSetting;
					best_psize = psize;
					best_ep = ep;
				}
