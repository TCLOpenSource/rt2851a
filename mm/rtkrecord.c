...
int list_all_rtk_memory_allocation_sort(list_rtk_mem_t list_func,int (*cmp_func)(const void *, const void *),void *data)
{
	...
	if (!data) {
		printk("allocated peak size at low memory: %8x \n",(unsigned int)(cma_low_alloc_peak_value << PAGE_SHIFT));
		printk("allocated peak size at high memory: %8x \n",(unsigned int)(cma_high_alloc_peak_value << PAGE_SHIFT));
	}
	...
	if(data){
		...
		for(cnt = 0;cnt < node_count; cnt++){
			...
			if (carvedout_buf_query(CARVEDOUT_BW_HIGH_CMA, NULL) > 0) {
				...
				pr_err("Error: HighMem CMA record is not in DCU2 and HIGHMEM_BW\n");
			}
#else
			total_tmp += ret;
#endif
		...
		pr_info("total allocated at HIGHMEM_BW: %8x \n\n", total_highmem_bw);
#else
		pr_info("total allocated at DCU2: %8x \n\n", total_tmp);
#endif
