...
struct rtkcma rtkcma_list[] = {
	[DEV_CMA1_DEFAULT]		= { "cma1",			NULL, NULL, NULL, 0 },	// null
	[DEV_CMA2]				= { "cma2",			NULL, NULL, NULL, 0 },	// auth
	[DEV_CMA3_GAL]			= { "cma3-gal",		NULL, NULL, NULL, 0 },	// gal
	[DEV_CMA4_CMA1_LIMIT]	= { "cma1-limit",	NULL, NULL, NULL, 0 },	// physical addr region limited
#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
	[DEV_CMA5_MC]			= { "cma5-mc",		NULL, NULL, NULL, 0 },	// mc_mmu
#endif
#ifdef CONFIG_HIGHMEM_BW_CMA_REGION
	[DEV_CMA6_BW]			= { "cma6-bw",		NULL, NULL, NULL, 0 },	// bw
#endif
};
...
void auth_sched_log_init(int size)
{
	...
	if(sched_log_buf_head) {
		...
		printk("sched log mem inited:%lx\n",(uintptr_t)sched_log_buf_head);
	}
...
static int auth_init(void)
{
	...
	if (IS_ERR(auth_dev)) {
		pr_err("auth: device %s create fail\n", "auth");
	...
	if (addr && size) {
		...
		dma_declare_contiguous(auth_dev, (unsigned int)size, (unsigned int)addr, 0);
	} else {
	...
	if (result < 0) {
		pr_err("auth: can't get ext major %d...\n", AUTH_EXT_MAJOR);
		return result;
 	}
	...
	for (minor = DEV_CMA1_DEFAULT; minor < ARRAY_SIZE(rtkcma_list); minor++) {
		...
		dev = device_create(auth_ext_class, NULL, MKDEV(AUTH_EXT_MAJOR, minor), 
		...
		} else if (strncmp("cma1-limit", rtkcma_list[minor].name, 10) == 0) {
		...
		if (addr && size) {
			pr_info("release %08lx - %08lx for %s...\n",
					addr, addr + size, rtkcma_list[minor].name);
			memblock_free(addr, size);
			dma_declare_contiguous(rtkcma_list[minor].dev, (unsigned int)size, (unsigned int)addr, 0);
            rtkcma_list[minor].exist = 1;
		} else {
			/* we create a null cma in order to avoid kernel from using the default cma instead */
//			dma_declare_null(rtkcma_list[minor].dev);
	...
	if (register_kernel_rpc(RPC_DVR_MALLOC, rpc_dvr_malloc) == 1)
        pr_err("register dvr remote malloc fail\n");
	if (register_kernel_rpc(RPC_DVR_FREE, rpc_dvr_free) == 1)
        pr_err("register dvr remote free fail\n");
#endif
...
static void auth_exit(void)
{
	...
	for (minor = DEV_CMA1_DEFAULT; minor < ARRAY_SIZE(rtkcma_list); minor++) {
        ...
        if (!IS_ERR(rtkcma_list[minor].dev)) {
            device_destroy(rtkcma_list[minor].dev);
            rtkcma_list[minor].dev = NULL;
            rtkcma_list[minor].exist = 0;
        }
	}
	class_destroy(auth_ext_class);
	unregister_chrdev(AUTH_EXT_MAJOR, AUTH_EXT_NAME);
}
