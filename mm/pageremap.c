...
unsigned long get_cma_avail_size(void)
{
	...
	if (carvedout_buf_query(CARVEDOUT_BW_HIGH_CMA, NULL) > 0) {
		if (highmem_bw_size)
			size += cma_get_avail_size(rtkcma_list[DEV_CMA6_BW].dev);
	}
#endif
...
static struct page *alloc_cache_page(int count, int flags)
{
    ...
    if (!IS_ERR(dev)) {
        ...
        if (!cache) {
            ...
            cache = dma_alloc_from_contiguous(dev, (cache_size / PAGE_SIZE), 0, DEFAULT_CMA_GFP_FLAG);
            if (!cache) {
                pr_debug("remap: can not allocate page cache on %s(%d)...\n", name, dev_idx);
        ...
        if (pageno >= (cache_size / PAGE_SIZE)) {
            pr_debug("remap: not enough cache memory for %s(%d)...\n", name, dev_idx);
            mutex_unlock(&cache_mutex);
            goto no_page;
        } else {
...
//            printk("weight: %d \n", __bitmap_weight(cache_bitmap, cache_size / PAGE_SIZE));
            ...
            page = cache + pageno;
            pr_debug("remap: alloc pageno: %d(pfn=0x%lx) count: %d for %s(%d)\n", pageno, page_to_pfn(page), count, name, dev_idx);
            mutex_unlock(&cache_mutex);
        ...
        if (page) {
            ...
            pr_info("remap: alloc (high=%d)page %p(pfn=0x%lx) flags(%x) count: %d for %s(%d)\n", PageHighMem(page), page, page_to_pfn(page), flags, count, name, dev_idx);
        } else {
            pr_debug("remap: not enough cache memory for flags(%x), dev(%x) on %s(%d)...\n", flags, dev, name, dev_idx);
            goto no_page;
        }
    }
...
static int free_cache_page(struct page *page, int count)
{
        ...
        if (in_cma_range(dev, page_to_pfn(page))) {
			...
			for (i = 0; i < count; i++) {
				if (page_count(page + i) != 1) {
					pr_err("remap: page %x on %s(%d) reference count is wrong...\n", (unsigned int)page, name, dev_idx);
					BUG();
				}
			}
			pr_debug("remap: free pageno: %ld(pfn=%lx) count: %d, dev(%x) on %s(%d)\n", page - cache, page_to_pfn(page), count, 
                    (unsigned int)dev, name, dev_idx);
    ...
    if (!in_cma) {
        if (page_count(page) != 1) {
            pr_err("remap: page %x (high=%d) reference count is wrong...\n", (unsigned int)page, PageHighMem(page));
            BUG();
        }
...
//	void *ret = 0;
	...
	if (!flags || (flags & GFP_DCU1_FIRST)) {
		...
		if (!page) {
			...
			if (!page) {
				if (carvedout_buf_query(CARVEDOUT_BW_HIGH_CMA, NULL) > 0) {
					page = dma_alloc_from_contiguous(rtkcma_list[DEV_CMA6_BW].dev, count, 0, DEFAULT_CMA_GFP_FLAG);
				}
			}
#endif
		}
		...
		page = dma_alloc_from_contiguous(rtkcma_list[DEV_CMA4_CMA1_LIMIT].dev, count, 0, DEFAULT_CMA_GFP_FLAG);
	} else if (flags & GFP_DCU2) {
		page = dma_alloc_from_contiguous(auth_dev, count, 0, DEFAULT_CMA_GFP_FLAG);
	} else if (flags & GFP_DCU2_GPU_CACHE_POOL) {
		page = dma_alloc_from_contiguous(auth_dev, count, 0, DEFAULT_CMA_GFP_FLAG);
...
int free_contiguous_memory(struct page *page, int count)
{
	...
	ret |= dma_release_from_contiguous(NULL, page, count);
	ret |= dma_release_from_contiguous(auth_dev, page, count);
#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
    ret |= dma_bitmap_release_from_contiguous(rtkcma_list[DEV_CMA5_MC].dev, page, count);
#endif
#ifdef CONFIG_HIGHMEM_BW_CMA_REGION
    ret |= dma_bitmap_release_from_contiguous(rtkcma_list[DEV_CMA6_BW].dev, page, count);
#endif

    if (rtkcma_list[DEV_CMA4_CMA1_LIMIT].exist) {
		ret |= dma_release_from_contiguous(rtkcma_list[DEV_CMA4_CMA1_LIMIT].dev, page, count);
 	}
...
int  unmap_kernel_line_addr(unsigned long phys_addr,unsigned int size )
{
        struct page *page, *tpage;
        void *va;
        int i, count, pfn_base;

        if (WARN_ON(!pfn_valid(phys_addr >> PAGE_SHIFT)))
                return 0;
        ...
        {   
                ...
                {   
                        ...
                        if(va){
                                pr_emerg("[PLI-MALLOC] before flush pfn=%d %d  %d, va=%p, ph=%lx",pfn_base, i, count, va, page_to_pfn(tpage) << PAGE_SHIFT);
...
int  remap_kernel_line_addr(unsigned long phys_addr,unsigned int size )
{
        struct page ** pages;
        struct page *tpage;
        int count, i;
        ...
        tpage = pfn_to_page(phys_addr >> PAGE_SHIFT);

        if(PageHighMem(tpage)) //high not need remap
	{
	    return 0; 		
	}

        //pr_err("free memory %lx, size=0x%x\n", phys_addr ,size);
        count = size >> PAGE_SHIFT;

        pages = vmalloc(count * sizeof(struct page *));
        if(pages == NULL)
        {   
                pr_err("Error: svp Failed to free igned long phys_addr,unsigned int size ma\n");
                return -1; 
        }   
...
int remap_list_mem(rtk_record *records)
{
	...
	if ((type & ID_MASK) == DRIVER_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by driver) %pF (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == NOSAVE_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by nosave) %pF (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == CACHE_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli cache) %pF (%ld.%03ld sec)\n", defer, addr, type & 0x00ffffff, pid, tid, caller, msec / 1000, msec % 1000);
		ret = type & 0x00ffffff;
	} else if ((type & ID_MASK) == BUDDY_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli buddy) %pF (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == BUDDY_MESG_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli buddy for user layer) %s (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, (const char *)caller, msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == DRIVER_MESG_ID) {
		pr_info(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by driver) %s (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, (const char *)caller, msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	}
	else {
