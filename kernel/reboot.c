...
#if 1
	/* For Android R, kernel config enable CONFIG_STATIC_USERMODEHELPER_PATH to NULL to disable call_usermodehelper.
		It cause the run_cmd return 0, then don't call kernel_power_off.
		So, set ret to 1 then force to run kernel_power_off for thermal shutdown. */
	ret = 1;
#endif
	if (ret && force) {
