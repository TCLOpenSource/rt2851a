...
static ssize_t pm_userresume_done_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", is_userresume_done());
}

power_attr_ro(pm_userresume_done);
...
static unsigned int pm_str_event_wait_ready_timeout = 0;
#endif
void str_event_before_suspend_peration(suspend_state_t state)
{
	...
	if(state == PM_SUSPEND_MEM)
	{
		...
		{
			str_event_log |= 0x2;
			pm_str_event_wait_ready_timeout = 1;
 ...
 */
	...
	if (state < PM_SUSPEND_MAX) {
		...
		if(pm_str_event_wait_ready_timeout == 1){
			pr_err("STR_NotifyAP : Force to standby!!!\n");
