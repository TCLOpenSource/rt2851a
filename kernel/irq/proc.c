...
#ifdef CONFIG_SUPPORT_RTK_INTERRUPT_PROFILE
...
#ifdef CONFIG_SUPPORT_RTK_INTERRUPT_PROFILE
		seq_printf(p, "Count Freq(sec) Avg.time(us) Max.time(us)");
#endif
		seq_putc(p, '\n');
	}
...
#ifdef CONFIG_SUPPORT_RTK_INTERRUPT_PROFILE
	if((any_count != 0) && (desc->rtk_reset == 0) && (desc->rtk_tot_count != 0))
	{
		u64	value, period;
		//Count
		seq_printf(p, "%10u ", desc->rtk_tot_count);

		//Freq(sec)
		value = desc->rtk_tot_count;
		period = arch_timer_read_counter() - desc->rtk_start_time;
		do_div(period, 27000);
		value *= 1000;
		do_div(value, period);
		seq_printf(p, "%10u ", (unsigned int)value);

		//Avg.time(us)
		value = desc->rtk_tot_time;
		do_div(value, desc->rtk_tot_count * 27);
		seq_printf(p, "%10u ", (unsigned int)value);

		//Max.time(us)
		value = desc->rtk_max_time;
		do_div(value, 27);
		seq_printf(p, "%10u ", (unsigned int)value);
	}
	else
