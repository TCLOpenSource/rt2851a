...
void reset_desc_rtk_interrupt_statistic(void)
{
	...
	for (i = 0; i < nr_irqs; i++)
	{
		...
		interrupt_count = kstat_irqs(i);
		action = desc->action;

		if ((!action || irq_desc_is_chained(desc)) && !interrupt_count)
			continue;
		else
			desc->rtk_reset = 1;
	}
	pr_err("reset rtk_interrupt_statistic(%d)\n", nr_irqs);
}
#else
void reset_desc_rtk_interrupt_statistic(void)
{
...
#ifdef CONFIG_SUPPORT_RTK_INTERRUPT_PROFILE
	desc->rtk_tot_time = 0;
	desc->rtk_tot_count = 0;
	desc->rtk_max_time = 0;
	desc->rtk_start_time = 0;
	desc->rtk_reset = 0;
#endif
	desc->name = NULL;
	desc->owner = owner;
